...
TYPE CONSTRAINT_TYPE 
  ...
  INTEGER(4)         :: NSTEP
  REAL(8)            :: C0FINAL
  INTEGER(4),POINTER :: IAT(:)
  INTEGER(4),POINTER :: IT(:,:)
  TYPE (CONSTRAINT_TYPE), POINTER :: NEXT
...
INTEGER(4)                    :: NCONSTRAINT
TYPE(CONSTRAINT_TYPE),POINTER :: FIRST_THIS
TYPE(CONSTRAINT_TYPE),POINTER :: THIS
END MODULE CONSTRAINTS_MODULE
!
!     ..................................................................
      SUBROUTINE CONSTRAINTS$NEW(ID)
      USE CONSTRAINTS_MODULE
      IMPLICIT NONE
      CHARACTER(*),INTENT(IN) :: ID
      TYPE(CONSTRAINT_TYPE),POINTER :: PREVIOUS
!     ******************************************************************
      IF(.NOT.TINI) THEN
        TINI=.TRUE.
        ALLOCATE(FIRST_THIS)
...
!
!     .................................................................
      SUBROUTINE CONSTRAINT$SELECT(ID)
      USE CONSTRAINTS_MODULE
      IMPLICIT NONE
      CHARACTER(*),INTENT(IN) :: ID
!     *****************************************************************
      IF(.NOT.TINI) THEN
        CALL ERROR$MSG('CREATE CONSTRAINT BEFORE SELECTING')
        CALL ERROR$STOP('CONSTRAINTS$SELECT')
      END IF
!
      IF(ID.EQ.'NONE') NULLIFY(THIS)
!
      THIS=>FIRST_THIS
      DO WHILE (THIS%ID.NE.ID) 
        IF(.NOT.ASSOCIATED(THIS%NEXT)) THEN
          CALL ERROR$MSG('CONSTRAINT  DOES NOT EXIST')
          CALL ERROR$CHVAL('ID',ID)
          CALL ERROR$STOP('CONSTRAINT$SELECT')
        END IF
        THIS=>THIS%NEXT
      ENDDO
      RETURN
...
!
!     ..................................................................
      SUBROUTINE CONSTRAINTS$SETCH(ID,VAL)
!     ******************************************************************
!     **  CONSTRAINTS$SET                                             **
!     ******************************************************************
      USE CONSTRAINTS_MODULE
...
!     ******************************************************************
      ...
      ELSE
        ...
        CALL ERROR$CHVAL('ID',ID)
        CALL ERROR$STOP(CONSTRAINTS$SETCH')
      END IF
      RETURN
      END
...
!     ******************************************************************
      ...
      INTEGER(4)            :: NMEMBER
      CHARACTER(32)         :: TYPE
      CHARACTER(256)        :: STRING,STRING1,STRING2,STRING3
...
!         == COUNT VECTOR LENGTH =======================================
          ...
          DO IAT=1,NAT
            DO I=1,3
              IND=IND+1
              IF(VEC.NE.0.D0) THIS%N1=THIS%N1+1
            ENDDO
          ENDDO
          ALLOCATE(THIS%I1(THIS%N1))
          ALLOCATE(THIS%C1(THIS%N1))
!         == FILL VECTOR ===============================================
          I1=0
          IND=9
          DO IAT=1,NAT
...
!       ==  FIXATOM   ==================================================
        STRING='FIXATOM'
        IF(TYPE.EQ.STRING(1:32)) THEN
          CALL LINKEDLIST$GET(LL_CNSTR,'ATOM',1,STRING1)
          CALL ATOMLIST$INDEX(STRING,IAT)
!
          WRITE(*,STRING)NUM
          ID='LINEAR'//TRIM(STRING)'.1'
          CALL CONSTRAINTS$NEW(ID)
          THIS%TYPE='LINEAR'
          THIS%DESCRIPTION='ATOM:'//TRIM(STRING1)//'.X'
          THIS%N1=1
          ALLOCATE(THIS%I1(1))
          ALLOCATE(THIS%C1(1))
          THIS%I1(1)=9+3*(IAT-1)+1
          THIC%C1(1)=1.D0
!
          WRITE(*,STRING)NUM
          ID='LINEAR'//TRIM(STRING)//'.2'
          CALL CONSTRAINTS$NEW(ID)
          THIS%TYPE='LINEAR'
          THIS%DESCRIPTION='ATOM:'//TRIM(STRING1)//'.Y'
          THIS%N1=1
          ALLOCATE(THIS%I1(1))
          ALLOCATE(THIS%C1(1))
          THIS%I1(1)=9+3*(IAT-1)+2
          THIC%C1(1)=1.D0
!
          WRITE(*,STRING)NUM
          ID='LINEAR'//TRIM(STRING)//'.3'
          CALL CONSTRAINTS$NEW(ID)
          THIS%TYPE='LINEAR'
          THIS%DESCRIPTION='ATOM:'//TRIM(STRING1)//'.Z'
          THIS%N1=1
          ALLOCATE(THIS%I1(1))
          ALLOCATE(THIS%C1(1))
          THIS%I1(1)=9+3*(IAT-1)+3
          THIC%C1(1)=1.D0
        END IF
!
!       ==  FIXGROUP  ==================================================
        STRING='FIXGROUP'
        IF(TYPE.EQ.STRING(1:32)) THEN
          CALL LINKEDLIST$GET(LL_CNSTR,'GROUP',1,STRING1)
          ALLOCATE(TMEMBER(NAT))
          CALL GROUPLIST$MEMBERS(STRING1,NAT,TMEMBER)
!
          NC1=0
          DO IAT=1,NAT
            IF(.NOT.TMEMBER(IAT))CYCLE
            NC1=NC1+1
!
            WRITE(*,STRING)NUM
            ID='LINEAR'//TRIM(STRING)
            WRITE(*,STRING)3*(NC1-1)+1
            ID=TRIM(ID)//'.'//TRIM(STRING)
            CALL CONSTRAINTS$NEW(ID)
            THIS%TYPE='LINEAR'
            WRITE(*,STRING)IAT
            THIS%DESCRIPTION='GROUP:'//TRIM(STRING1)//';'//TRIM(STRING)//'.X'
            THIS%N1=1
            ALLOCATE(THIS%I1(1))
            ALLOCATE(THIS%C1(1))
            THIS%I1(1)=9+3*(IAT-1)+1
...
!
            WRITE(*,STRING)NUM
            ID='LINEAR'//TRIM(STRING)
            WRITE(*,STRING)3*(NC1-1)+2
            ID=TRIM(ID)//'.'//TRIM(STRING)
            CALL CONSTRAINTS$NEW(ID)
            THIS%TYPE='LINEAR'
            WRITE(*,STRING)IAT
            THIS%DESCRIPTION='GROUP:'//TRIM(STRING1)//';'//TRIM(STRING)//'.Y'
            THIS%N1=1
            ALLOCATE(THIS%I1(1))
            ALLOCATE(THIS%C1(1))
            THIS%I1(1)=9+3*(IAT-1)+2
...
!
            WRITE(*,STRING)NUM
            ID='LINEAR'//TRIM(STRING)
            WRITE(*,STRING)3*(NC1-1)+3
            ID=TRIM(ID)//'.'//TRIM(STRING)
            CALL CONSTRAINTS$NEW(ID)
            THIS%TYPE='LINEAR'
            WRITE(*,STRING)IAT
            THIS%DESCRIPTION='GROUP:'//TRIM(STRING1)//';'//TRIM(STRING)//'.Z'
            THIS%N1=1
            ALLOCATE(THIS%I1(1))
            ALLOCATE(THIS%C1(1))
            THIS%I1(1)=9+3*(IAT-1)+3
...
!       ==  BONDLENGTH =================================================
        STRING='BONDLENGTH'
        IF(TYPE.EQ.STRING(1:32)) THEN
          CALL LINKEDLIST$GET(LL_CNSTR,'ATOM1',1,STRING1)
          CALL LINKEDLIST$GET(LL_CNSTR,'ATOM2',1,STRING2)
          WRITE(*,STRING)NUM
          ID='BOND'//TRIM(STRING)
          CALL CONSTRAINTS$NEW(ID)
          THIS%TYPE='BOND'
          THIS%DESCRIPTION='BOND:'//TRIM(STRING1)//';'//TRIM(STRING2)
          ALLOCATE(THIS%IAT(2))
          ALLOCATE(THIS%IT(3,2))
          CALL ATOMLIST$INDEXX(STRING1,IAT1,IT1)
          CALL ATOMLIST$INDEXX(STRING2,IAT2,IT2)
          THIS%IAT(1) =IAT1
          THIS%IT(:,1)=IT1(:)
          THIS%IAT(2) =IAT2
          THIS%IT(:,2)=IT2(:)
...
!       == RIGID =====================================================
        ...
        IF(TYPE.EQ.STRING(1:32)) THEN
          ...
          CALL GROUPLIST$MEMBERS(STRING1,NAT_,TMEMBER)
          NMEMBER=0
          DO IAT=1,NAT_
            IF(TMEMBER(IAT)) NMEMBER=NMEMBER+1
          ENDDO
!
!         == BUILD RIGID FRAME FROM THE FIRST THREE OR FOUR ATOMS
!
!     ..................................................................
      SUBROUTINE RIGID$SETUP(GROUP,NUM,RBAS,NAT,R,NATGROUP,ATOMID)
      CHARACTER(*),INTENT(IN) :: GROUP    ! GROUPNAME
      INTEGER(4)  ,INTENT(IN) :: NUM      ! CONSTRAINT NUMBER
      INTEGER(4)  ,INTENT(IN) :: NAT      ! #(ATOMS) IN THE CELL
      REAL(8)     ,INTENT(IN) :: RBAS(3,3)! LATTICE VECTORS
      REAL(8)     ,INTENT(IN) :: R(3,NAT) ! ATOMIC POSITIONS
      INTEGER(4)  ,INTENT(IN) :: NATGROUP ! #(ATOMS IN THE GROUP)
      CHARACTER(*),INTENT(IN) :: ATOMID   ! ATOM IDENTIFIES IN THE GROUP
      INTEGER(4)              :: FRAMEIAT(4)
      INTEGER(4)              :: FRAMEIT(3,4)
      INTEGER(4)              :: NATOFGROUP,NATFRAME
      INTEGER(4)              :: IAT1,IAT2
      CHARACTER(64)           :: STRING
      CHARACTER(64)           :: ID
!     ******************************************************************
      NATFRAME=MIN(4,NATGROUP)
      DO I=1,4
        CALL ATOMLIST$INDEXX(ATOMID(I),FRAMEIAT(I),FRAMEIT(1,I))
      ENDDO
!
!     ==================================================================
!     ==  BUILD RIGID FRAME USING BOND LENGTH CONSTRAINTS             ==
!     ==================================================================
      IC=0
      DO IAT1=1,NATFRAME
        DO IAT2=IAT1+1,NATFRAME
          WRITE(*,STRING)NUM
          ID='BOND'//TRIM(STRING)
          IC=IC+1
          WRITE(*,STRING)IC
          ID=TRIM(ID)//'.'//TRIM(STRING)
          CALL CONSTRAINTS$NEW(ID)
          THIS%DESCRIPTION='RIGID:'//TRIM(GROUP)//'.'//TRIM(STRING)
!
          THIS%TYPE='BOND'
          ALLOCATE(THIS%IAT(2))
          ALLOCATE(THIS%IT(3,2))
          THIS%IAT(1) =FRAMEIAT(IAT1)
          THIS%IT(:,1)=FRAMEIT(:,IAT1)
          THIS%IAT(2) =FRAMEIAT(IAT2)
          THIS%IT(:,2)=FRAMEIT(IAT2)
          DR(:)=R(:,THIS%IAT(2))-R(:,THIS%IAT(1))
          DO I=1,3
            DR(:)=DR(:)+RBAS(:,I)*REAL(THIS%IT(I,2)-THIS%IT(I,1),KIND=8)
          ENDDO
          THIS%C0=SQRT(DR(1)**2+DR(2)**2+DR(3)**2)
        ENDDO
      ENDDO
      IF(NATGROUP.LE.4) RETURN
!
!     ==================================================================
!     ==  CONNECT REMAINING ATOMS USING LINEAR CONSTRAINTS            ==
!     ==================================================================
      DO I=1,4
        IAT1=FRAMEIAT(I)
        IT1(:)=FRAMEIT(:,I)
        RFRAME(:,I)=R(:,IAT1)+MATMUL(RBAS,REAL(IT1))
      ENDDO          
      DO I=1,3
        AMAT(:,I)=RFRAME(:,I+1)-RFRAME(1)
      ENDDO
      CALL LINALG$INVERT(3,AMAT,AMATINV)
!
!     ==================================================================
!     ==  CONNECT REMAINING ATOMS USING LINEAR CONSTRAINTS            ==
!     ==================================================================
      DO I=5,NATGROUP
        IC=IC+1
        CALL ATOMLIST$INDEXX(ATOMID(I),IAT1,IT1)
        RI(:)=R(:,IAT1)+MATMUL(RBAS,REAL(IT1,KIND=8))
        FACTS(:)=MATMUL(AMATIN,RI(:)-RFRAME(:,1))
...
!
!     ..................................................................
      SUBROUTINE CONSTRAINTS_MOVESOLD(ICOUNT,S0,SM,SP,DELTA,NSTEP &
     &                              ,SETS,SF_,SETV,VF_)
!     ** 
!     **  MOVES CONSTRAINT TO A GIVEN VALUE OR VELOCITY               **
!     ** 
      ...
      LOGICAL(4),INTENT(IN)    :: SETV
!     ******************************************************************
      TBYDELTA=DBLE(NSTEP-ICOUNT)
!
!     ==================================================================
!     ==  AFTER ACCELERATION PHASE                                    ==
!     ==================================================================
      IF(TBYDELTA.LT.1.D0) THEN
        IF(SETS) THEN
          IF(SETV) THEN
            SP=SF_+DELTA*VF_*DBLE(ICOUNT-NSTEP)
          ELSE
...
!
      IF(SETS) THEN
        FS=2.D0*SF/(T*(T+DELTA))
        SP=SP+DELTA2*FS
        VF=VF-FS*(T+0.5D0*DELTA)
      END IF
!
      IF(SETV) THEN
        IF(SETS) THEN
          FS=-2.D0*VF*(T-DELTA)/(T*(T+DELTA))
        ELSE
          FS=2.D0*VF/(2.D0*T+DELTA)
        END IF      
        SP=SP+DELTA2*FS
      ELSE 
        FS=-2.D0*VF*(T-DELTA)/(T*(T+DELTA))
...
!     ******************************************************************
      ...
      REAL(8)                  :: SVAR
      INTEGER(4)               :: ILENG,IVEC,IAUX,NAUX,IS
      REAL(8)                  :: TAUSINGVAL
!     ******************************************************************
!
!     ==================================================================
...
!         __ CHECK LINEAR DEPENDENCE OF CONSTRAINTS_____________________
          IF(TLINDEP) THEN
            ...
            ILENG=ILENG+NC
            ALLOCATE(WORK(ILENG))
            CALL DCOPY(NC*NC,RMAT,1,WORK(1),1)
            CALL DCOPY(NC,VEC,1,WORK(IVEC),1)
            CALL DGESVF(2,WORK(1),NC,WORK(IVEC),NC,1,WORK(IS),NC,NC &
     &                 ,WORK(IAUX),NAUX)
            CALL DGESVS(WORK(1),NC,WORK(IVEC),NC,1,WORK(IS),VEC,NC,NC,NC &
     &                 ,TAUSINGVAL)
            DEALLOCATE(WORK)
!           CALL DGEF(RMAT,NC,NC,IPVT)
!           CALL DGES(RMAT,NC,NC,IPVT,VEC,0)
          ELSE
            CALL DGES(RMAT,NC,NC,IPVT,VEC,0)
          END IF
        ELSE
...
!     ******************************************************************
      ...
      IF(TOFF) THEN
        CALL FILEHANDLER$UNIT('PROT',NFILO)
        WRITE(NFILO,FMT='(''NUMBER OF SYMMETRY OPERATIONS:'')')NSYM
      END IF
      RETURN
      END
!
