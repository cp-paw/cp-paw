...
!
!     ==================================================================
!     == estimate energy by shooting outward                          ==
!     ==================================================================
...
!     ==================================================================
      ...
      DO WHILE(.NOT.CONVG)
        ...
        EOLD=E
        IF(PRESENT(DLG)) THEN
          CALL AEBOUNDSTATE(R1,DEX,NR,L,EOLD,AEZ,BOXRADIUS,POT,PHI,DLG=DLG)
        ELSE
          CALL AEBOUNDSTATE(R1,DEX,NR,L,EOLD,AEZ,BOXRADIUS,POT,PHI)
        END IF
        NNODE=0
        DO IR=1,NR-1
...
!         == estimate the energy difference to the next band
          E=E+0.5D0*((ZEFF-0.5d0)/DBLE(NNode+L+1))**2 &
     &       -0.5D0*((ZEFF-0.5d0)/DBLE(NNext+L+1))**2
          CONVG=.FALSE.
        ELSE
          CONVG=(ABS(E-EOLD).LT.TOL)
!         The fix commented out created convergence problems
!         The design is not foolproof but seems to work
!          if(eold.lt.elow) then 
!            e=0.5d0*(e+elow)
!          else  if(eold.gt.eup) then
!            e=0.5d0*(e+eup)
!          else
!            e=eold
!          end if       
e=eold
        END IF
!print*,'in fixnode',eup,elow,eold,e
...
!        PHI(IR,3)=2.D0*(POT(IR)*Y0-E)*PHI(IR,1)-2.D0*GINH(IR)
      ENDDO
      IF(THOM) THEN
        DO IR=N1,N2
