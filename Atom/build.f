...
END IF
WRITE(NFIL,FMT='("  !END")')
WRITE(NFIL,FMT='("  !VTILDE  TYPE=\'POLYNOMIAL\' RC=",F5.3 &
    &                       ," POWER=3  POT(0)=-4.0 !END ")')factor*RCOV-0.1
WRITE(NFIL,FMT='("  !PSCORE  TYPE=\'POLYNOMIAL\' RC=",F5.3 &
    &                       ," POWER=2  RHO(0)=0.0 !END ")')factor*RCOV-0.1
WRITE(NFIL,FMT='("  !COMPENSATE RC=",F5.3," !END ")')RCOV/4.d0
IF(NS.NE.0) THEN
   WRITE(NFIL,FMT='("  !WAVE L=0 N=",I2," PSTYPE=\'HBS\' RC=",F5.3 &
    &   ," LAMBDA=6 !END")')NS,FACTOR*RCOV
END IF
IF(NP.NE.0) THEN
...
!
call system('/u/blo.2/PrivatePAW/Atomic/paw_atom '//trim(file))
     ENDDO
     STOP
     END
