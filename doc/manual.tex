...
the NoS is rescaled. Attention the scaling is done for each function 
put to file individually!}
\vformat{character} 
\vrules{optional}
\vdefault{'none'}}
...
%==========================================================================
In the position trajectory ``{\it root}\_r.tra'', LEN is $9+4N$, where
$N$ is the the number of atoms.The first 9 numbers are the lattice
vectors in the form x,y,z-components of the first lattice vector,
x,y,z components of the second vector and so on.  Then the atomic
positions are written in the same manner, starting x,y,z of the first
atom, x,y,z of the second and so fourth until the last atom. Finally
the point charges of the atoms are written beginning with the first
atom.
