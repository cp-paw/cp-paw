...
Bugfix in paw_ldaplusu. For non-collinear calculations, 
the hybrid functional and LDA+U did not apply the correction to the potential. 
Problem has been fixed in paw_ldaplusu in routine ldaplusu_spindenmat. 

\subsection{May 1,  2007}

Tesselation files tess_32 tess_122 tess)482 tess_1922 for COSMO
calculations have been included in the directory ``parameters''.

\subsection{May 3,  2007}

Bugfix in paw_linkedlist.f90: With the ifc91 compiler buffer\$read did
...
the new configure process also creates a version (paw_prof.x) for
