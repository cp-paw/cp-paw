...
!
!     ..................................................................
      SUBROUTINE UPPERCASE(STRINGIN,STRING)
      USE STRINGS
      IMPLICIT NONE
...
!**  FUNCTIONS:                                                       **
!**    ERROR__MSG(MSG)                                                 **
!**    ERROR__R8VAL(NAME,VAL)                                          **
!**    ERROR__I4VAL(NAME,VAL)                                          **
!**    ERROR__L4VAL(NAME,VAL)                                          **
!**    ERROR__CHVAL(NAME,VAL)                                          **
!**    ERROR__STOP(ROUTINENAME)                                        **
!**    ERROR__NORMALSTOP                                               **
!**    ERROR__OVERFLOW  (SHOUD NOT BE USED)                            **
!**                                                                   **
!**  DEPENDECIES:                                                     **
!**    MPE(MPE__STOPALL,MPE__QUERY)                                     **
!**    FILEHANDLER(FILEHANDLER__UNIT,FILEHANDLER__CLOSEALL)             **
!**                                                                   **
!**  REMARKS:                                                         **
...
!                                                                   
!     ..................................................................
      SUBROUTINE ERROR__MSG(MESSAGE_)
!     ******************************************************************
!     ** STORE ERROR MESSAGE                                          **
!     ******************************************************************
      USE ERROR_MODULE
...
!
!     ..................................................................
      SUBROUTINE ERROR__OVERFLOW(MESSAGE_,IACTUAL,ITARGET)
!     ******************************************************************
...
!
!     ..................................................................
      SUBROUTINE ERROR__R8VAL(MESSAGE_,R8VAL)
!     ******************************************************************
...
!
!     ..................................................................
      SUBROUTINE ERROR__I4VAL(MESSAGE_,I4VAL)
!     ******************************************************************
...
!
!     ..................................................................
      SUBROUTINE ERROR__L4VAL(MESSAGE_,L4VAL)
!     ******************************************************************
!     ** STORE MESSAGE WITH INTEGER VALUE                             **
!     ******************************************************************
      USE ERROR_MODULE
...
! 
!     ..................................................................
      SUBROUTINE ERROR__CHVAL(MESSAGE_,CHVAL)
!     ******************************************************************
...
!
!     ..................................................................
      SUBROUTINE ERROR__STOP(MESSAGE_)
!     ******************************************************************
...
!     ===============================================================
      ...
      ENDDO
      IF(IEND.EQ.0) THEN
        CALL ERROR__MSG('NO END TEMPLATE FOUND')
        CALL ERROR__CHVAL('TEMPLATE NAME',TRIM(NAME))
        CALL ERROR__CHVAL('FIRSTLINE',TRIM(BUFFER(IBEGIN)))
        CALL ERROR__CHVAL('LASTLINE',TRIM(BUFFER(NLINE)))
        CALL ERROR__STOP('RESOLVE_TEMPLATES')
      END IF
!
!     ===============================================================
...
!     ===============================================================
      ...
      ENDDO
      IF(IBODY.EQ.0) THEN
        CALL ERROR__MSG('NO #BODY STATEMENT FOUND')
        CALL ERROR__STOP('RESOLVE_TEMPLATES')
      END IF
!
!     ===============================================================
...
!         ============================================================
          IF(LINE(I1-1:I1-1).EQ.'(') THEN
            IF(TSET.OR.TREP) THEN
              CALL ERROR__MSG('SYNTAX ERROR: SET ALREADY ON')
              CALL ERROR__CHVAL('LINE',TRIM(LINE))
              CALL ERROR__STOP('RESOLVETEMPLATE')
            END IF
            IF(LINE(I1:I1).EQ.'<') THEN
              ...
              IF(ISET.GT.NSETX) THEN
                CALL ERROR__MSG('#(REPLACEMENT SETS TOO LARGE')
                CALL ERROR__CHVAL('LINE',TRIM(LINE))
                CALL ERROR__STOP('RESOLVETEMPLATE')
              END IF
            ELSE IF(LINE(I1:I1).EQ.'[') THEN
              ...
              IF(IMEM.GT.NMEMX) THEN
                CALL ERROR__MSG('#(REPLACEMENT MEMBERS) TOO LARGE')
                CALL ERROR__CHVAL('LINE',TRIM(LINE))
                CALL ERROR__STOP('RESOLVETEMPLATE')
              END IF
            ELSE
              CALL ERROR__MSG('CONFUSED')
              CALL ERROR__CHVAL('LINE',TRIM(LINE))
              CALL ERROR__STOP('RESOLVETEMPLATE')
            END IF
            IREP=0
...
!         ============================================================
          IF((LINE(I1:I1).EQ.'<').AND.(.NOT.TSET)) THEN
            CALL ERROR__MSG('OUT REPLACEMENT MUST BE IN OUT ENVIRONMENT')
            CALL ERROR__CHVAL('LINE',TRIM(LINE))
            CALL ERROR__STOP('RESOLVETEMPLATE')
          END IF
          IF((LINE(I1:I1).EQ.']').AND.(.NOT.TREP)) THEN
            CALL ERROR__MSG('IN REPLACEMENT MUST BE IN IN ENVIRONMENT')
            CALL ERROR__CHVAL('LINE',TRIM(LINE))
            CALL ERROR__STOP('RESOLVETEMPLATE')
          END IF
!
          ...
          IF(LINE(I2+1:I2+1).EQ.')') THEN
            ...
            ELSE IF(TREP) THEN
              IF(IREP.NE.NREP(ISET)) THEN
                CALL ERROR__MSG('NUMBER OF REPLACEMENTS MUST BE SAME IN A SET')
                CALL ERROR__STOP('RESOLVETEMPLATE')
              END IF
              TREP=.FALSE.
            END IF
...
!         ============================================================
      ...
      IF(TREP.OR.TSET) THEN
        CALL ERROR__MSG('ENVIRONMENT NOT PROPERLY CLOSED')
        CALL ERROR__STOP('RESOLVETEMPLATE')
      END IF
!
!     ===============================================================
...
!     ===============================================================
      ...
      IF(NLINE+ISHIFT.GT.NLINEX) THEN
        CALL ERROR__MSG('BUFFER SIZE EXCEEDED')
        CALL ERROR__STOP('RESOLVE_TEMPLATE')
      END IF
!
...
!     ===============================================================
      DO I=IBODY+1,IINSTANCE-1
        ...
        IF(I2.EQ.0) THEN
          CALL ERROR__MSG('COULD NOT LOCATE SUBROUTINE NAME')
          CALL ERROR__STOP('RESOLVE_TEMPLATE')
        END IF
        I2=I2-1
        INSTANCE0=LINE(1:I2)
...
!     ****************************************************************
      ...
      DO WHILE(INDEX(COPY,'<<').NE.0)
        ...
        IF(NREP.GT.NREPX) THEN
          CALL ERROR__MSG('TOO MANY REPLACEMENTS')
          CALL ERROR__CHVAL('LINE',TRIM(COPY))
          CALL ERROR__STOP('COLLECTREPLACEMENTS')
        END IF
!     
!       ==  REPLACEMENT%OUT
        I1=INDEX(COPY,'<<')
        ...
        IF(I2.LE.I1) THEN
          CALL ERROR__MSG('SYNTAX ERROR')
          CALL ERROR__CHVAL('COPY',TRIM(COPY))
          CALL ERROR__STOP('COLLECTREPLACEMENTS')
        END IF
