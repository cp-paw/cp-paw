...
!
      DO N=1,NUP-1
        ...
        DO J=2,N+1
          A(J,1,N+1)=A(J-1,1,N)-DBLE(8*J-2)*A(J,1,N) &
     &                 +DBLE(J*(16*J+8))*A(J+1,1,N)
        ENDDO
        DO IL=2,LUP+1
          DO J=1,N+1
            A(J,IL,N+1)=-2.D0*A(J,IL-1,N+1)+DBLE(8*J)*A(J+1,IL-1,N+1)
          ENDDO
        ENDDO
      ENDDO
!
...
!     ==================================================================
      DO LR3=1,LRXX
        DO LR2=1,2*LRXX+2
          DO LR1=1,LRXX+1
            ISVAR=NINT(A(LR1,LR2,LR3))
            A(LR1,LR2,LR3)=DBLE(ISVAR)
          ENDDO
        ENDDO
      ENDDO
!
...
!     ******************************************************************
      ...
      ESELF = E00*QLM(1)**2
      LX=INT(DSQRT(DBLE(LMRX-1)+1.D-2))
      LM=1
      DO L=1,LX
