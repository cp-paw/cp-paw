...
\mbax{\key{K} \vdescr{k-point coordinates $(i_1,i_2,i_3)$ in units of
    fractions of reciprocal lattice vectors: $k_i = 1/2\ \sum_j
    G_{ij}*i_j/n_j$, where $G_{ij}$ is defined by the real space
    lattice vectors $T_{ij}$ by $\sum_k G_{ki}T_{kj}=2\pi\delta_{ij}$.
    The fractions are provided by \texttt{DIV}. The k-point
    integration weights are equally distributed.}
  \vformat{integer(3)} \vrules{multiple, optional} \vdefault{0 0 0}}
...
\vformat{real}
\vrules{optional. Not used if \texttt{K} is present.}
\vdefault{12.}}
...
\vdescr{One center expansion for the density is limited to a maximum
  angular momentum of lrhox. The actual value may be smaller if the
  LRHOX is larger than $\ell_{max}^2$ where +1) where $\ell_{max}$ is
  the maximum angular momentum in the set of partial waves}
\vformat{integer}
\vrules{Optional}
\vdefault{9}}
...
%
\mbax{\key{S}
  \vdescr{restricts the spin on which the orbitals act. The $S=0$
  defines a spin independent potential.}
  \vformat{integer}
  \vrules{optional}
  \vdefault{0}
}
...
\item a rubber sheet representation of the density on a given
  ...
  \resizebox{!}{8.0cm}{\includegraphics{h2co-rho.epsf}}
...
\begin{figure}
\centerline{
% \resizebox{!}{8.0cm}{\includegraphics{/u/blo.4/PAW/Manual/h2copdos.eps}}
 \resizebox{12.0cm}{!}{\rotatebox{-89}{\includegraphics{h2copdos.eps}}}
}
...
%==========================================================================
\newpage
\section{Specifications of unformatted output data files}
%==========================================================================
%==========================================================================
\subsection{waveplot}
%==========================================================================
...
\item[gunzip,tar] gunzip is for example part of emacs, so that you can
  simply open emacs, execute Cntl-d to open dired, set the curser to
  the file you want to uncompress, pull down ``operate$\rightarrow$
  compress or uncompress'' and say yes. This is only required if you
  have the compressed files. 
...
\item[MPI] Message Passing Interface \cite{MPI}. 
  It is not required on scalar execution.
\item[IBM Dataexplorer] The IBM Dataexplorer is used to visualize wave
  functions etc.\cite{Dataexplorer}. If this software is unavailable,
  some data produced cannot be seen. The Dataexplorer is freely
  available. 
\item[XMGR] A program to produce publication ready xy-graphics written
  largely by Paul~J.~Turner \cite{XMGR}. You can use any equivalent
