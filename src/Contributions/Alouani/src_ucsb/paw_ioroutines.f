...
!     == EPWBUCKET =====================================================
      ...
      END
!     ..................................................................
      SUBROUTINE READIN_LDA_U(LL_CNTL_)
!     ******************************************************************
!     ******************************************************************
      USE LINKEDLIST_MODULE
      USE LDAPLUSU_MODULE 
      ...
      TYPE(LL_TYPE)         :: LL_CNTL
      LOGICAL(4)            :: TCHK
      INTEGER(4)            :: PRINTOCC_ ! PRINT OCCUPATION 
      INTEGER(4)            :: POT_      ! CORRECTION TYPE 
      REAL(8)               :: XU1       ! COULOMB ENERGY     (eV)  
      REAL(8)               :: XJ1       ! EXCHANGE PARAMETER (eV) 
      REAL(8)               :: RC1       ! MT CUTOFF RADIUS  (A.U)  
!     ******************************************************************
                           CALL TRACE$PUSH('READIN_LDA_U')
      LL_CNTL=LL_CNTL_
 
      CALL LINKEDLIST$SELECT(LL_CNTL,'~')
      CALL LINKEDLIST$SELECT(LL_CNTL,'CONTROL')
      CALL LINKEDLIST$SELECT(LL_CNTL,'LDA_U')
      CALL LINKEDLIST$EXISTD(LL_CNTL,'DOLDAPU',1,TCHK) 
      IF (TCHK) THEN 
      CALL LINKEDLIST$GET(LL_CNTL,'DOLDAPU',1,TCHK)
      ENDIF 
      IF (TCHK) CALL LDAPLUSU__SET 
      WRITE(*,*) 'DOLDAPU=',TCHK 
 
!     == POT_TYPE ================================================
      CALL LINKEDLIST$EXISTD(LL_CNTL,'POT_TYPE',1,TCHK)
      IF(.NOT.TCHK) CALL LINKEDLIST$SET(LL_CNTL,'POT_TYPE',0,1)
   !  CALL LINKEDLIST$CONVERT(LL_CNTL,'POT_TYPE',1,'R(8)')
      CALL LINKEDLIST$GET(LL_CNTL,'POT_TYPE',1,POT_)
      POT_TYPE = POT_ 
 
 
 
!     == PRINTOCC ================================================
      CALL LINKEDLIST$EXISTD(LL_CNTL,'PRINTOCC',1,TCHK)
      IF(.NOT.TCHK) CALL LINKEDLIST$SET(LL_CNTL,'PRINTOCC',0,0)
   !  CALL LINKEDLIST$CONVERT(LL_CNTL,'PRINTOCC',1,'R(8)')
      CALL LINKEDLIST$GET(LL_CNTL,'PRINTOCC',1,PRINTOCC_)
      PRINTOCC = PRINTOCC_ 
 
 
!     == XU1 ========================================================
      CALL LINKEDLIST$EXISTD(LL_CNTL,'XU',1,TCHK)
      IF(.NOT.TCHK) CALL LINKEDLIST$SET(LL_CNTL,'XU',0,8.D0)
      CALL LINKEDLIST$CONVERT(LL_CNTL,'XU',1,'R(8)')
      CALL LINKEDLIST$GET(LL_CNTL,'XU',1,XU1)
      XU = XU1 
 
 
!     == XJ1 ========================================================
      CALL LINKEDLIST$EXISTD(LL_CNTL,'XJ',1,TCHK)
      IF(.NOT.TCHK) CALL LINKEDLIST$SET(LL_CNTL,'XJ',0,0.95D0)
      CALL LINKEDLIST$CONVERT(LL_CNTL,'XJ',1,'R(8)')
      CALL LINKEDLIST$GET(LL_CNTL,'XJ',1,XJ1)
      XJ = XJ1 
 
 
!     == RC_OFF ========================================================
      CALL LINKEDLIST$EXISTD(LL_CNTL,'RC_OFF',1,TCHK)
      IF(.NOT.TCHK) CALL LINKEDLIST$SET(LL_CNTL,'RC_OFF',0,-1.0D0)
      CALL LINKEDLIST$CONVERT(LL_CNTL,'RC_OFF',1,'R(8)')
      CALL LINKEDLIST$GET(LL_CNTL,'RC_OFF',1,RC1)
      RC_OFF = RC1 
...
!     == MULTIPLE TIMESTEPS?  ========================================
      ...
      END IF
      CALL LINKEDLIST$GET(LL_CNTL,'MULTIPLE',1,MULTIPLE)
      CALL CONTINUUM$SETI4('MULTIPLE',MULTIPLE)
!
...
!     ==================================================================
      CALL CONSTANTS('KB',KELVIN)
      CALL READIN_THERMOSTAT(LL_CNTL_,'CONTINUUM',DT,0.5D0*293.D0*KELVIN,10.D0)
!
                                 CALL TRACE$POP
      RETURN
      END
...
!     ******************************************************************
      ...
      CALL LINKEDLIST$SELECT(LL_CNTL,'CONTROL')
      CALL LINKEDLIST$SELECT(LL_CNTL,'ANALYSE')
      CALL LINKEDLIST$EXISTD(LL_CNTL,'OPTIC',1,TON)

! 08-04-2002 BENGONE MODIFY START
      IF (TON) THEN 
      CALL LINKEDLIST$GET(LL_CNTL,'OPTIC',1,TON)
      ENDIF 
      IF (TON) CALL OPTIC3$SETL4('ON',TON)
      WRITE(*,*) 'OPTIC =', TON 

      RETURN 
! 08-04-2002 BENGONE MODIFY END
...
!     == ESTIMATE FROM #(ELECTRONS) AND #(EMPTY BANDS) REQUESTED ========
      ...
      IF(NSPIN.EQ.2) THEN
        ...
        NOCC=NOCC+INT(0.5D0*TOTSPIN+1.D0-1.D-5)
      END IF
      NOCC=NOCC+NEMPTY
      NB=MAX(NB,NOCC)
!
...
! BENGONE MODIFY START 
        WRITE(*,*) 
        WRITE(*,*) 'ATOM=',ATOM,'TYPE=',TYPE,'ISPIN=',ISPIN,'VALUE=',VALUE &
       & ,'RC=',RC,' PWR=',PWR  
        WRITE(*,*) 
! BENGONE MODIFY END 
        CALL LINKEDLIST$SELECT(LL_STRC,'..')
