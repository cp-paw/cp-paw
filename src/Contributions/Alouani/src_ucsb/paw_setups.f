...
! DEFINE NEW VARIABLES FOR OPTIC3 ATOMS FILE 
!     INTEGER(4)              :: NAT
!     INTEGER(4)              :: LNXX
!     INTEGER(4)              :: LNXI
!     INTEGER(4),ALLOCATABLE  :: LNX(:)
!     INTEGER(4)              :: LMNXI
!     INTEGER(4),ALLOCATABLE  :: LMNX(:)
...
! GET AND WRITE  OPTIC ATOMS 
       
!     CALL ATOMLIST$NATOM(NAT)
!     CALL SETUP$LNXX(LNXX) 
!     ALLOCATE(LNX(NSP1)) 
!     ALLOCATE(LMNX(NSP1)) 
!     ALLOCATE(LOX(LNXX,NSP1)) 
!  
!      LNX(:)=0 ; LMNX(:)=0 ; LOX(:,:)=0 
!   
!     CALL SETUP$RADGRID(1,R1,DEX,NR) 
!     ALLOCATE(AEPHI(NR,LNXX,NSP1)) 
!     ALLOCATE(PSPHI(NR,LNXX,NSP1)) 
!     ALLOCATE(PRO(NR,LNXX,NSP1)) 
...
!     DO ISP=1,NSP1
!        CALL SETUP$ISELECT(ISP)
!        CALL SETUP$LNX(ISP,LNXI) 
!        LNX(ISP)=LNXI 
!        CALL SETUP$LMNX(ISP,LMNXI) 
!        LMNX(ISP)=LMNXI 
!        CALL SETUP$LOFLN(ISP,LNXI,LOX(:,ISP)) 
!        CALL SETUP$AEPARTIALWAVES(ISP,NR,LNXI,AEPHI(:,:,ISP)) 
!        CALL SETUP$PSPARTIALWAVES(ISP,NR,LNXI,PSPHI(:,:,ISP)) 
!        CALL SETUP$PROJO(ISP,NR,LNXI,PRO(:,:,ISP)) 
!     ENDDO 

!     CALL OPTIC3$ATOMS(NAT,LNXX,NSP1,NR,R1,DEX &
!    &                  ,LOX,LNX,LMNX,PRO,AEPHI,PSPHI)


!     CALL WRITE_PROJO(NAT,NR,NSP1,LNXX,LNX,PRO) 
...
!
!     ..................................................................
!     SUBROUTINE WRITE_PROJO(NAT,NR,NSP,LNXX,LNX,PRO)
!      IMPLICIT NONE 
...
!      REAL(8),INTENT(IN)     :: PRO(NR,LNXX,NSP)   
...
!      OPEN(UNIT=402,FILE='PROJO',FORM='FORMATTED',STATUS='UNKNOWN')
...
!      WRITE(402,*) (LNX(ISP),ISP=1,NSP) 

!      DO IAT=1,NAT
!         ISP=ISPECIES(IAT)
!         DO IR=1,NR 
!            WRITE(402,*)(PRO(IR,LN,ISP),LN=1,LNX(ISP)) 
!         ENDDO 
!      ENDDO 
!      CLOSE(402) 
!     END SUBROUTINE WRITE_PROJO 
...
!
!     ..................................................................
      SUBROUTINE SETUP$PROJO(ISP_,NRX_,LNX_,PRO_)
!     ******************************************************************
!     **  RETURN PS PROJO  FUNCTION  ON A RADIAL GRID                **
!     ******************************************************************
      USE SETUP_MODULE
      IMPLICIT NONE
      INTEGER(4),INTENT(IN) :: ISP_
      INTEGER(4),INTENT(IN) :: NRX_
      INTEGER(4),INTENT(IN) :: LNX_
      REAL(8)   ,INTENT(OUT):: PRO_(NRX_,LNX_)
      INTEGER(4)            :: IR,LN
!     ******************************************************************
      CALL SETUP$ISELECT(ISP_)
      DO LN=1,LNX_
        DO IR=1,NR
          PRO_(IR,LN)=THIS%PRO(IR,LN)
        ENDDO
      ENDDO
      RETURN  
      END
