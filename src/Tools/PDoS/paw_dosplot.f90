...
!     ==========================================================================
      ...
      XZEROGLOB=0.D0
      CALL LINKEDLIST$EXISTD(LL_CNTL,'XZERO[EV]',1,TCHK)
      IF(TCHK)CALL LINKEDLIST$GET(LL_CNTL,'XZERO[EV]',0,XZEROGLOB)
!
      CALL LINKEDLIST$EXISTD(LL_CNTL,'XMIN[EV]',1,TCHK)
      IF(.NOT.TCHK) THEN
        CALL ERROR$MSG('!DPCNTL:XMIN[EV] IS MANDATORY INPUT')
        CALL ERROR$STOP('READCNTL')
      END IF
      CALL LINKEDLIST$GET(LL_CNTL,'XMIN[EV]',0,XMIN)
!
      CALL LINKEDLIST$EXISTD(LL_CNTL,'XMAX[EV]',1,TCHK)
      IF(.NOT.TCHK) THEN
        CALL ERROR$MSG('!DPCNTL:XMAX[EV] IS MANDATORY INPUT')
        CALL ERROR$STOP('READCNTL')
      END IF
      CALL LINKEDLIST$GET(LL_CNTL,'XMAX[EV]',0,XMAX)

!
      CALL LINKEDLIST$EXISTD(LL_CNTL,'YMIN[EV]',1,TCHK)
      IF(.NOT.TCHK) THEN
        CALL ERROR$MSG('!DPCNTL:YMIN[EV] IS MANDATORY INPUT')
        CALL ERROR$STOP('READCNTL')
      END IF
      CALL LINKEDLIST$GET(LL_CNTL,'YMIN[EV]',0,YMIN)

!
      CALL LINKEDLIST$EXISTD(LL_CNTL,'YMAX[EV]',1,TCHK)
      IF(.NOT.TCHK) THEN
        CALL ERROR$MSG('!DPCNTL:YMAX[EV] IS MANDATORY INPUT')
        CALL ERROR$STOP('READCNTL')
      END IF
      CALL LINKEDLIST$GET(LL_CNTL,'YMAX[EV]',0,YMAX)
!
!     ==========================================================================
...
!       == GET DEFAULT SCALE FOR THIS GRAPH ===================================
        XZERO=XZEROGLOB
        CALL LINKEDLIST$EXISTD(LL_CNTL,'XZERO[EV]',1,TCHK)
        IF(TCHK)CALL LINKEDLIST$GET(LL_CNTL,'XZERO[EV]',0,XZERO)
!
!       ========================================================================
!       == LOOP OVER SETS                                                     ==
!       ========================================================================
