...
INTEGER(4),ALLOCATABLE:: LB(:)          !(NB) MAIN ANGULAR MOMENTUM
INTEGER(4),ALLOCATABLE:: NNB(:)         !(NB) NUMBER OF NODES
...
!     ==================================================================
      ...
      CALL LINKEDLIST$SELECT(LL_CNTL,'ACNTL')
      CALL LINKEDLIST$SELECT(LL_CNTL,'GENERIC')
      CALL LINKEDLIST$EXISTD(LL_CNTL,'ELEMENT',1,TCHK)
      IF(TCHK) THEN
      ...
      ELSE
        ...
        IF(.NOT.TCHK) THEN
          CALL ERROR$MSG('EITHER ELEMENT OR ATOMIC NUMBER MUST BE SPECIFIED')
          CALL ERROR$STOP('RESOLVEPAWATOMSINBUFFER')
        END IF
        CALL LINKEDLIST$GET(LL_CNTL,'Z',1,AEZ)
      END IF
      CALL LINKEDLIST$SELECT(LL_CNTL,'~')
      CALL LINKEDLIST$SELECT(LL_CNTL,'ACNTL')
...
!     ==================================================================
      AUX1=-AECORE(:)/Y0*R(:)**2
      CALL OLDRADIAL$INTEGRAL(R1,DEX,NR,AUX1,ZCORE)  
!     THE FOLLOWING LINE CREATES AN INTERFACE ERROR
!     CALL OLDRADIAL$INTEGRAL(R1,DEX,NR,-AECORE(:)/Y0*R(:)**2,ZCORE)
      ZEFF=AEZ+ZCORE
      DO IB=1,NB
...
! RETURN
      ...
      POT(:)=POT(:)-AEZ/R(:)/Y0
!
      CALL OLDRADIAL$DERIVE(R1,DEX,NR,RHO(:),GRHO)
      DO IR=1,NR
        RH=RHO(IR)*Y0
        GRHO2=(Y0*GRHO(IR))**2
      ...
      ENDDO
      CALL OLDRADIAL$DERIVE(R1,DEX,NR,GRHO(:),AUX)
...
!CALL OLDRADIAL$DERIVE(R1,DEX,NR,RHOC+RHOV,GRHO)
!PRINT*,'R,RHOTOT,GRHO'
!DO IR=1,NR
...
!     ==================================================================
      IF(TALL) THEN
        ...
        ENDDO
        CALL OLDRADIAL$INTEGRAL(R1,DEX,NR,AUX1,SVAR)
        EKINCC=EBANDCC-SVAR
      END IF
!
!     ==================================================================
...
!     ==================================================================
      ...
      ENDDO
      CALL OLDRADIAL$INTEGRAL(R1,DEX,NR,AUX1,EKINV)
      EKINV=ESUM-EKINV
!
!     ==================================================================
!     ==  CORE CORE ELECTROSTATIC
!     ==================================================================
      CALL OLDRADIAL$POISSON(R1,DEX,NR,0,RHOC,AUX3)
      DO IR=1,NR
        AUX1(IR)=RHOC(IR)*(0.5D0*AUX3(IR)*R(IR)**2-AEZ/Y0*R(IR))
      ENDDO
      CALL OLDRADIAL$INTEGRAL(R1,DEX,NR,AUX1,EELCC)
!
!     ==================================================================
...
!     ==================================================================
      ...
      ENDDO
      CALL OLDRADIAL$INTEGRAL(R1,DEX,NR,AUX1,EELVC)
!
!     ==================================================================
!     ==  VALENCE VALENCE ELECTROSTATIC
!     ==================================================================
      CALL OLDRADIAL$POISSON(R1,DEX,NR,0,RHOV,AUX3)
      DO IR=1,NR
        AUX1(IR)=RHOV(IR)*0.5D0*AUX3(IR)*R(IR)**2
      ENDDO
      CALL OLDRADIAL$INTEGRAL(R1,DEX,NR,AUX1,EELVV)
!
!     ==================================================================
!     ==  CORE CORE EXCHANGE
!     ==================================================================
      CALL OLDRADIAL$DERIVE(R1,DEX,NR,RHOC,GRHO)
      DO IR=1,NR
        GRHO2=(Y0*GRHO(IR))**2
        RH=RHOC(IR)*Y0
        CALL DFT(RH,0.D0,GRHO2,0.D0,0.D0,EXC,VXC,DUMMY1,VGXC,DUMMY2,DUMMY3)
      ...
      ENDDO
      CALL OLDRADIAL$INTEGRAL(R1,DEX,NR,AUX1,EXCCC)
!
!     ==================================================================
...
!       CALL DFT(RH,RH,GRHO2,GRHO2,GRHO2,EXC,VXC,DUMMY1,VGXC,DUMMY2,DUMMY3)
        AUX1(IR)=EXC*FOURPI*R(IR)**2
      ENDDO
      CALL OLDRADIAL$INTEGRAL(R1,DEX,NR,AUX1,EXCVV)
      EXCVV=EXCVV-EXCCC
!
!     ==================================================================
...
!     ******************************************************************
      ...
      DO IR=1,NR
        PSPOT(IR)=AEPOT(IR)
      ENDDO
      CALL OLDRADIAL$DERIVE(R1,DEX,NR,AEPOT,WORK)
      CALL OLDRADIAL$VALUE(R1,DEX,NR,WORK,RC,DFDR)
      CALL OLDRADIAL$VALUE(R1,DEX,NR,AEPOT,RC,FOFR)
...
!     ******************************************************************
      ...
      IMPLICIT NONE
      INTEGER(4),PARAMETER  :: ITRMAX=300
...
!     
!       == FIND NEW PREFACTOR  =======================================
        CALL OLDRADIAL$INTEGRATE(R1,DEX,NR,R(:)**2*PSPHI(:,1)**2*CUT(:)*Y0,AUX)
...
!       ================================================================
     ...
     &    ," TEST PARTIAL WAVE FOR L=",I2," AND E=",F10.5/72("-"))')L,E
        CALL OLDRADIAL$VALUE(R1,DEX,NR,PHI(:,1),RMT,PHI1)
        CALL OLDRADIAL$VALUE(R1,DEX,NR,PHI(:,2),RMT,PHI2)
...
!       ================================================================
     ...
     &                ,POT(:),PHITEST,AUX)
        CALL OLDRADIAL$VALUE(R1,DEX,NR,PHITEST(:,1),RMT,PHI1)
        CALL OLDRADIAL$VALUE(R1,DEX,NR,PHITEST(:,2),RMT,PHI2)
        DLGTEST=RMT*PHI2/PHI1
...
!         PRINT*,'ETEST',ETEST,EOLD,EOLD-ETEST
 ...
 1100   CONTINUE
        CALL OLDRADIAL$VALUE(R1,DEX,IRMT,PHITEST(:,1),RMT,PHI1)
        CALL OLDRADIAL$VALUE(R1,DEX,IRMT,PHITEST(:,2),RMT,PHI2)
...
!
        IF(ABS(DENL).LT.1.D-5) THEN
          ...
          ENDDO
          CALL OLDRADIAL$INTEGRAL(R1,DEX,IRAUG,AUX1,SVAR2)
...
!       == TEST BIORTHOGONALITY OF PROJECTOR AND PS-PARTIALWAVES =======
        ...
        DO N1=1,NPRO
          DO N2=1,NPRO
            CALL OLDRADIAL$INTEGRAL(R1,DEX,NR,R(:)**2*PSPHI(:,1,N1)*PRO(:,N2),SVAR)
            IF(N1.EQ.N2) THEN
...
!
          DO N2=1,NPRO
            CALL OLDRADIAL$INTEGRAL(R1,DEX,NR,R(:)**2*PHITEST(:,1)*PRO(:,N2),SVAR)
...
!     ******************************************************************
      ...
      ENDDO
      RHO(:)=0.D0
      CALL OLDRADIAL$INTEGRAL(R1,DEX,NR,(AECORE(:)-PSCORE(:))*R(:)**2,SVAR)
...
!           ============================================================
            AUX(:)=R(:)**2*PSPSI(:,1)**2
            CALL OLDRADIAL$INTEGRAL(R1,DEX,NR,AUX(:),SUM)
            DO I1=1,NWAVE1
              AUX(:)=R(:)**2*PRO1(:,I1)*PSPSI(:,1)
              CALL OLDRADIAL$INTEGRAL(R1,DEX,NR,AUX(:),PROJ(I1))
            ENDDO
            DO I1=1,NWAVE1
              DO I2=1,NWAVE1
...
!     ==================================================================
      ...
      CALL OLDRADIAL$INTEGRAL(R1,DEX,NR,AUX,SVAR)
...
!         ==  G-MOMENTS FOR KINETIC ENERGY AND MASS RENORMALIZATION  ==
          ...
          ENDDO
          WORK(:)=GMOMENT(:,1)
          CALL OLDRADIAL$INTEGRATE(G1,DEX,NG,WORK,GMOMENT(:,1))
          WORK(:)=GMOMENT(:,2)
          CALL OLDRADIAL$INTEGRATE(G1,DEX,NG,WORK,GMOMENT(:,2))
          WORK(:)=GMOMENT(:,3)
          CALL OLDRADIAL$INTEGRATE(G1,DEX,NG,WORK,GMOMENT(:,3))
...
!           ==========================================================
            DO I1=1,NWAVE1
              CALL OLDRADIAL$INTEGRAL(R1,DEX,NR,R(:)**2*PRO1(:,I1)*PSIR(1:NR),PROJ(I1))
            ENDDO
!
!           ==========================================================
!           ==  RENORMALIZE WAVE FUNCTION                           ==
!           ==========================================================
            CALL OLDRADIAL$INTEGRAL(R1,DEX,NR,(R(:)*PSIR(1:NR))**2,SVAR)
            DO I1=1,NWAVE1
              DO I2=1,NWAVE1
...
!           ============================================================
            DO I1=1,NWAVE1
              CALL OLDRADIAL$INTEGRAL(R1,DEX,NR,R(:)**2*PRO1(:,I1)*PSPSI(:,1),PROJ(I1))
            ENDDO
            DO I1=1,NWAVE1
              DO I2=1,NWAVE1
                QAUG=QAUG+PROJ(I1)*DO1(I1,I2)*PROJ(I2)*FB(IB)
...
!     == CHECK CHARGE CONSERVATION ======================================
      AUX(:)=4.D0*PI*Y0*RHO(:)*R(:)**2
      CALL OLDRADIAL$INTEGRAL(R1,DEX,NR,AUX,SVAR)
!     CALL OLDRADIAL$INTEGRAL(R1,DEX,NR,4.D0*PI*Y0*RHO(:)*R(:)**2,SVAR)
...
!     ==================================================================
      ...
      DO IR=1,30
        ...
        CALL OLDRADIAL$VALUE(R1,DEX,NR,AEPOT,RI,AEPOT1)
        CALL OLDRADIAL$VALUE(R1,DEX,NR,PSPOT,RI,PSPOT1)
        CALL OLDRADIAL$VALUE(R1,DEX,NR,VHAT,RI,VHAT1)
        CALL OLDRADIAL$VALUE(R1,DEX,NR,AECORE,RI,AECORE1)
        CALL OLDRADIAL$VALUE(R1,DEX,NR,PSCORE,RI,PSCORE1)
        AEPOT1=AEPOT1*Y0
        PSPOT1=PSPOT1*Y0
        AECORE1=AECORE1*Y0
        PSCORE1=PSCORE1*Y0
...
!     ==================================================================
      DO L=0,3
        ...
        REWIND NFIL
        DO IR=1,NR
          WRITE(NFIL,fmt='(100e20.10)')R(IR),PRO1(IR,:)
        ENDDO
        DEALLOCATE(PRO1)
      ENDDO
!
...
!     ==================================================================
      ...
      REWIND NFIL
      DO IR=1,NR
        WRITE(NFIL,fmt='(100e20.10)')R(IR),AEPOT(IR),PSPOT(IR),VHAT(IR)
      ENDDO
...
!     ==================================================================
      DO L=0,3
        ...
        REWIND NFIL
        DO IR=1,NR
          WRITE(NFIL,fmt='(100e20.10)')R(IR) &
     &          ,(AEPHI1(IR,1,IPRO),PSPHI1(IR,1,IPRO),IPRO=1,NPRO)
        ENDDO
        DEALLOCATE(AEPHI1)
        DEALLOCATE(PSPHI1)
      END DO
...
!     ==================================================================
     ...
     &                       -PSPHI(:,1,IW1)*PSPHI(:,1,IW2))*DENMAT(IW1,IW2)
      ...
      CALL OLDRADIAL$MOMENT(R1,DEX,NR,0,RHO(:),QLM)
      QLM=QLM-AEZ*Y0
!
!     ==================================================================
...
!     == XC ENERGY
      ...
      DO IR=1,NR
        ...
        AUX1(IR)=EXC*FOURPI*R(IR)**2
      ENDDO
      CALL OLDRADIAL$INTEGRAL(R1,DEX,NR,AUX1,PSEXC)
!
!     == ELECTROSTATIC ENERGY
      ...
      CALL OLDRADIAL$INTEGRAL(R1,DEX,NR,AUX1,PSEEL)
!
!     == ENERGY VHAT ===
      CALL OLDRADIAL$INTEGRAL(R1,DEX,NR,VHAT(:)*RHO(:)*R(:)**2,SVAR)
      PSEEL=PSEEL+SVAR
!
!     ==================================================================
...
!     == XC ENERGY
      ...
      DO IR=1,NR
        ...
        AUX1(IR)=EXC*FOURPI*R(IR)**2
      ENDDO
      CALL OLDRADIAL$INTEGRAL(R1,DEX,NR,AUX1,PS1EXC)
!     == ELECTROSTATIC ENERGY
      CALL GAUSSN(0,1.D0/RCSMALL**2,SVAR)
      DO IR=1,NR
        RHO(IR)=RHO(IR)+QLM*SVAR*EXP(-(R(IR)/RCSMALL)**2)
      ...
      CALL OLDRADIAL$POISSON(R1,DEX,NR,0,RHO,POT)
      AUX1(:)=0.5D0*RHO(:)*POT(:)*R(:)**2
      CALL OLDRADIAL$INTEGRAL(R1,DEX,NR,AUX1,PS1EEL)
!     == ENERGY VHAT ===
      CALL OLDRADIAL$INTEGRAL(R1,DEX,NR,VHAT(:)*RHO(:)*R(:)**2,SVAR)
      PS1EEL=PS1EEL+SVAR
!
...
!     == XC ENERGY
      ...
      DO IR=1,NR
        ...
        AUX1(IR)=EXC*FOURPI*R(IR)**2
      ENDDO
      CALL OLDRADIAL$INTEGRAL(R1,DEX,NR,AUX1,AE1EXC)
!     == ELECTROSTATIC ENERGY
      CALL OLDRADIAL$POISSON(R1,DEX,NR,0,RHO,POT)
      AUX1(:)=RHO(:)*(0.5D0*POT(:)*R(:)**2-AEZ/Y0*R(:))
      CALL OLDRADIAL$INTEGRAL(R1,DEX,NR,AUX1,AE1EEL)
!     == SUBSTRACT CORE ENERGIES 
      CALL OLDRADIAL$DERIVE(R1,DEX,NR,AECORE,GRHO)
      DO IR=1,NR
        GRHO2=(Y0*GRHO(IR))**2
      ...
      AE1EXC=AE1EXC-SVAR
!
      CALL OLDRADIAL$POISSON(R1,DEX,NR,0,AECORE,POT)
        ...
        DO IB=1,NB
          DO IPRO=1,NPRO
            IF(LB(IB).EQ.LPRO(IPRO)) THEN
              CALL OLDRADIAL$INTEGRAL(R1,DEX,NR,R(:)**2*PSPSI(:,1,IB),PROJ(IPRO,IB))
            ELSE
              PROJ(IPRO,IB)=0.D0
            END IF
          ENDDO
...
!       ================================================================
        QAUG=AEZ
        CALL OLDRADIAL$INTEGRAL(R1,DEX,NR,R(:)**2*(AECORE(:)-PSCORE(:))*Y0,SVAR)
...
!       ================================================================
     ...
     &                     ,PSPOT,NPRO,PRO1,DATH1,DO1,PAWPSI,AUX)
            DO IPRO=1,NPRO
              CALL OLDRADIAL$INTEGRAL(R1,DEX,NR &
...
!       CALL XPHASESHIFT(R1,DEX,NR,RPHASE,REFPSI,PHASEAE(IE,L+1))
        do ir=1,np
          call oldradial$value(r1,dex,nr,refpsi(1,1),rp(ir),psiarr(ir,ie))
        enddo
...
!     =========================================================
     ...
     &              ,IRlast,0.d0,AEPOT,REFPSI,AUX)
      do ir=1,np
        call oldradial$value(r1,dex,nr,refpsi(1,1),rp(ir),psib(ir,1))
      enddo
      svar=1.d0/psib(1,1)
      psib(:,1)=psib(:,1)*svar
      do ie=1,ne
