...
!     == DEFAULT VALUES
      ...
      IF(.NOT.TCHK) THEN
!       == PER DEFAULT, START FROM RESTART FILE, UNLESS NONE IS PRESENT ========
        CALL LINKEDLIST$SET(LL_CNTL,'START',0,.FALSE.) 
        CALL FILEHANDLER$INQUIRE('RESTART_IN',TCHK)
        IF(.NOT.TCHK)CALL LINKEDLIST$SET(LL_CNTL,'START',0,.TRUE.) 
      END IF
!
