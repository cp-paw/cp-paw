...
real(kind=8), save, allocatable, dimension(:)  ::  contv_rsolv

character(8), save, allocatable, dimension(:)  ::  contv_tesselation_name
 
!#########################################################
! these are the working arrays for the various routines
! we define them here and try to save them throughout
!
!  since this is all private it is only accesible from within this module
!  therfore I do not define the whole stuff inside the functions and allocate/deallocate
!  it all the time.
!
!  instead all arrays are defined here as allocatable and SAVEed and they get allocated
!  once in continuum_initialize
!  They don't get deallocated and this is bad programming style, I know :-)
!
!     FACE and related arrays
!
!   comment:
!   the major point of this code review is to get rid of the liststructure for the 
!   faces. it is kinda cool, but not practical and at most points in the old code all
!   values get remaped to staright arrays. this is good for performance and necessary
!   for parallelization. since no faces are deleted or inserted during runtime (then lists 
!   would be good) but this is handeled by the surface function it is much easier to do it that
!   way.
!   we keep one full integer array to maintain pointers to the corresponding atom to which this face belongs
!   this is memoryintense (could be done by "magic" formulas etc.) but easy and fast. 
!   we do not need to keep the area, because it is the same for all faces of one atom. we also don't need
!   to keep the edges of the vertices since they were used only for the matlab printout.
!   BUT: in order to make things fast (and easy to program :-) I will keep a complete array of the original
!   face positions (as if all atoms would sit at the origin). Every time the continuum is called the actual
!   face positions are calculated by adding the vector of the corresponding atomposition. Therfore the old
!   call to continuum$switch in paw_atoms is no longer necessary to move the spheres. They always sit were
!   the atoms are (I guess, this is just more safe for future changes etc.)
...
real(kind=8), save, allocatable, dimension(:)      ::  face_q     ! charge at t0
real(kind=8), save, allocatable, dimension(:)      ::  face_qm    ! charge at t-
real(kind=8), save, allocatable, dimension(:)      ::  face_q_keep     ! charge at t0
real(kind=8), save, allocatable, dimension(:)      ::  face_qm_keep    ! charge at t-
real(kind=8), save, allocatable, dimension(:)      ::  face_qp    ! charge at t+
real(kind=8), save, allocatable, dimension(:)      ::  face_fq    ! force on charge
real(kind=8), save, allocatable, dimension(:)      ::  face_fmem  ! force on charge memorized for MTS
integer     , save, allocatable, dimension(:)      ::  face_atom  ! pointer to the corresponding atom
real(kind=8), save, allocatable, dimension(:)      ::  face_fmem_keep  ! force on charge memorized for MTS
integer     , save, allocatable, dimension(:)      ::  face_atom_keep  ! pointer to the corresponding atom
integer     , save, allocatable, dimension(:)      ::  face_find  ! pointer of the filtered face to the original

! FACE positions

real(kind=8), save, allocatable, dimension(:,:)    ::  face_pos        ! actual positions
real(kind=8), save, allocatable, dimension(:,:)    ::  face_relpos     ! relative position to nuclei

! per atom arrays

real(kind=8), save, allocatable, dimension(:)      ::  face_area       ! area of one face (1-natoms) 
integer     , save, allocatable, dimension(:)      ::  face_peratom    ! number of faces per atom (sum is nfaces)
...
real(kind=8), save, allocatable, dimension(:)      :: atom_surf
real(kind=8), save, allocatable, dimension(:)      :: atom_charge
real(kind=8), save, allocatable, dimension(:)      :: atom_weighed_charge

! working arrays and other stuff

real(kind=8), save, allocatable, dimension(:,:,:)  :: afv              ! distance matrix nuclei <-> face
real(kind=8), save, allocatable, dimension(:,:)    :: afd              ! distance matrix nuclei <-> face
real(kind=8), save, allocatable, dimension(:,:)    :: wdff             ! weighed distance matrix face <-> face
real(kind=8), save, allocatable, dimension(:)      :: bigf             ! product of fermi functions
real(kind=8), save, allocatable, dimension(:,:)    :: fermi            ! fermi functions per atom/face
real(kind=8), save, allocatable, dimension(:,:,:)  :: dbigf            ! derivative of bigf wrt nuc. coords.
real(kind=8), save, allocatable, dimension(:)      :: png              ! potential of nuclear gaussians on the charge
real(kind=8), save, allocatable, dimension(:,:,:)  :: hfng             ! "hamiltonian" between faces and indiv. nuc. gaussians
...
!         (means simplified :-)
!         we read teh tesselations right here and allocate everything and
!         report what we've done (read_tesselations is no longer necessary)
!
!         we do not need the atompositions here cause we just read into face_relpos     
!
subroutine continuum_init

!  as opposed to the old code we read everything in anyways, because if a restart file is read
!  all values will be overwritten anyways (the stopping when reading the restart is handeled
...
real(kind=8), parameter                   :: MB = 1.D0/1024.D0/1024.D0
...
! now read the first two lines of all tesselation files for all atoms
! its a bit clumsy to close and reaopen all the files but we have to know the 
! number of faces before we can start
! (and it is only for startup :-)
do ia = 1, natoms
    call filehandler$unit(contv_tesselation_name(ia), nfil)
    rewind (nfil)
    read   (nfil,*) npoints(ia)
    read   (nfil,*) face_peratom(ia)
...
!
!  note: for simplicity I allocate all these arrays once in full size
!        here even if filtering is switched on
!
allocate(face_q(nfaces))
write (nfilprot, FMT='(A30, F8.4)')"Mem for q [MB]           : ", DBLE(nfaces*8*MB)
allocate(face_qm(nfaces))
write (nfilprot, FMT='(A30, F8.4)')"Mem for qm [MB]          : ", DBLE(nfaces*8*MB)
if (.not. contv_filter) then
   allocate(face_qp(nfaces))
   write (nfilprot, FMT='(A30, F8.4)')"Mem for qp [MB]          : ", DBLE(nfaces*8*MB)
   allocate(face_fq(nfaces))
   write (nfilprot, FMT='(A30, F8.4)')"Mem for fq [MB]          : ", DBLE(nfaces*8*MB)
end if
allocate(face_fmem(nfaces))
write (nfilprot, FMT='(A30, F8.4)')"Mem for fmem [MB]        : ", DBLE(nfaces*8*MB)
allocate(face_atom(nfaces))
write (nfilprot, FMT='(A30, F8.4)')"Mem for atom [MB]        : ", DBLE(nfaces*4*MB)

allocate(face_pos(3,nfaces))
write (nfilprot, FMT='(A30, F8.4)')"Mem for pos [MB]         : ", DBLE(nfaces*3*8*MB)
allocate(face_relpos(3,nfaces))
write (nfilprot, FMT='(A30, F8.4)')"Mem for relpos [MB]      : ", DBLE(nfaces*3*8*MB)
write (nfilprot, FMT='(A30, F8.4)')"MEM TOTAL [MB]           : ", DBLE(nfaces*(11*8+4)*MB)
...
nmem = nmem + natoms*nfaces
if (.not.contv_filter) then
   allocate(dbigf(3,natoms,nfaces))  ! deriv. of prod. of fermis per face wrt to nuc. coords
   nmem = nmem + 3*natoms*nfaces
   allocate(png(nfaces))             ! potential of nuclear gaussians
   nmem = nmem + nfaces
   allocate(hfng(ng, natoms,nfaces)) ! "hamiltonian" face <-> ng (for derivs.)
   ...
   allocate(face_find(nfaces))      ! Full INDex to "find" the charge when refiltering
   nmem = nmem + nfaces
endif
write (nfilprot, *)
write (nfilprot, FMT='(A30, F8.4)')"Mem for working arrays [MB]: ", DBLE(nmem*8*MB)


! now read the rest of the tesselation files and initialize the values
ifg = 1
do ia = 1, natoms
    call filehandler$unit(contv_tesselation_name(ia), nfil)
    rewind(nfil)
    ! advance the two lines for nfaces and npoints
    read  (nfil, *) dummy
    read  (nfil, *) dummy
    ! load the points
    ...
    ! cycle over all faces for this atom
    do i = 1, face_peratom(ia)
         ...
         ! calculate center of vertexpoints
         face_relpos(:,ifg) = points(:, vertex(1))+points(:,vertex(2))+points(:,vertex(3))
         ! normalize and scale by rsolv
         face_relpos(:,ifg) = face_relpos(:,ifg)/dsqrt(sum(face_relpos(:,ifg)**2)) * contv_rsolv(ia)
         face_q(ifg)        = 0.0D0
         face_qm(ifg)       = 0.0D0
         face_fmem(ifg)     = 0.0D0
    ...
    ! area is 4*pi*rsolv**2 divided by the number of faces
    face_area(ia) = (16.0D0 * datan(1.D0) * (contv_rsolv(ia)**2))/DBLE(face_peratom(ia))
PRINT*,'In continuum_init: ',contv_rsolv(ia),face_peratom(ia),face_area(ia)
    ! deallocate the points
    deallocate(points)
    ! close the tesselation file fo0r this atom (cause we are done)
    call filehandler$close(contv_tesselation_name(ia))
end do
...
!     because of the array structure this can be done at once, so the
!     single underscore routine s continuum_write and continuum_read
!     are gone.
!     no attempts to do anyhting in parallel here!
!hms 
!hms  We have to fix some bugs here: The separator doesn't give the correct no. of records,
!hms  and hence the restart file is not handled properly.
!hms    
!
!     ..................................................................
...
!
      ...
      KINETIC_ENERGY=0.D0
! has to be calculated here (for some magic reasons: paw.f90 overwrites it every step)
...
!RS debug
         do i = 1, nfaces 
!              face_qp(i) = (vc0*face_q(i)) + (vcm*face_qm(i)) + (vcf*face_fq(i)) 
             FACE_QP(i)=(2.D0*FACE_Q(i)-(1.D0-contv_CHARGE_FRICTION)*FACE_QM(i)+&
             &(FACE_FQ(i))/(contv_mass)*TIMESTEP**2)/&
             &(1.D0+contv_CHARGE_FRICTION)
!RS debug
if (contv_debug_print) then
 print '(4F20.16)', face_fq(i), face_q(i), face_qm(i), face_qp(i)
end if
!RS debug
!               kinetic_energy = kinetic_energy + (face_qp(i)-face_qm(i))*ve
             KINETIC_ENERGY = KINETIC_ENERGY+((FACE_QP(i)-FACE_QM(i))/&
                &(2.D0*TIMESTEP))**2*contv_MASS*0.5D0
         end do
      else
            face_qp = face_q
      endif
...
!
  ...
  real(kind=8), dimension(3,nat), intent(in)   :: r0
  real(kind=8), dimension(ng,nat), intent(in)  :: rc, qmad
  real(kind=8), dimension(3)                   :: pos, cafv
  REAL(kind=8)                                 :: cafd, cfermi, cbigf, erf
 
  integer                                      :: i, ifc, ia, ig, ia2, ifc_eff, iaf
  real(kind=8)                                 :: k_tension, factor, dbigf_scal
  logical                                      :: face_skip
...
!
  ifc_eff = 1                                                    
!   calculate the actual face positions this step
  do ifc = 1,nfaces
       face_skip = .false.
       ! this is very clumsy, I know! but I want to keep the nonfiltered algorithm
...
!    calculate bigf (product over the fermi functions)
     ...
     do ia = 1,natoms
        if (iaf == ia) then
             ! face belongs to atom
             cafv(:) = -face_relpos(:,ifc)
             cafd    = contv_rsolv(ia)
             cfermi  = 1.0D0
        else
             ! face does not belong to the atom
             cafv(:) = r0(:,ia)-pos(:)
             cafd    = dsqrt(sum(cafv*cafv))
             cfermi  = fermi_function(cafd, contv_rsolv(ia))
        end if
        if (contv_filter .and. (cfermi < 1.0D-20)) then
           face_skip = .true.
     ...
     if (.not.face_skip) then
         ...
         if (contv_filter) then
            ! "inverted" logic: find points to the subarray (is of size nafces_full)
            ! skiped charges get a zero
            face_find(ifc) = ifc_eff
         end if
         ifc_eff = ifc_eff + 1
     else
...
!    if we move the atoms:
!    calculate the derivatives dbigf of bigf with respect to the atomic positions
!
    ...
    do ifc = 1,nfaces
       do ia = 1,natoms
         if (face_atom(ifc) /= ia) then
            ! only if face does not belong to the atom
            ! grad on this atom is 
            dbigf_scal = 1.0D0
            do ia2 = 1,natoms
               if (ia == ia2) then
                  ! this is the atom itself: we need the derivative
                  dbigf_scal = dbigf_scal * dfermi_function(afd(ia,ifc), contv_rsolv(ia))
               else
                  ! this is one of the other atoms : multiply by fermi
                  dbigf_scal = dbigf_scal * fermi(ia2,ifc)
               endif
            end do
            dbigf(:,ia,ifc) = dbigf(:,ia,ifc) + dbigf_scal *(afv(:,ia,ifc)/afd(ia,ifc))
            ! the neg. gradient acts on the atom to which this face belongs
            dbigf(:,face_atom(ifc),ifc) = dbigf(:,face_atom(ifc),ifc) - dbigf(:,ia,ifc)
         end if
       end do
    end do
  endif

!   calculate the accessible surface of each atom and the surface energy
!   initial value from old code (Peter Margl)
!
!   ----- THIS IS FITTED TO R(C)=3.32AU AND R(H)=2.17AU
!hms   SURFACE_TENSION_ENER=0.0018276D0
...
!hms  atom_surf(:) now contains the number of exposed faces for each atom
  do ia = 1,natoms
     atom_surf(ia)  = atom_surf(ia) * face_area(ia)
!hms  Now we have the surface areas in atom_surf(:)
! debug
     if (contv_debug_print) then
...
!   calculate the potential (png) on the charges due to the nuclei-gaussians
!   this stays fixed during the MTS propagation of the charges 
!   (do the hfng here too for the derivs.)
!
  do ifc = 1,nfaces
    png(ifc) = 0.0D0
    do ia = 1,natoms
...
!    now if contv_keep_dff is on calculate the wdff matrix right now
...
subroutine continuum_filter(ifc_eff)
     ...
     ! this is the number of effective faces considered
     if (.not. contv_filter_init) then
        ! save all the "full" values
...
!print *, "face ", ifc, " skiped"
            if (face_q_keep(ifc) > 1.D-10) then
               write (nfilo, FMT='("charge # ", I5, " is skiped (q / qm / deltaq) ", 3F15.10)')  &
                &     ifc, face_q_keep(ifc), face_qm_keep(ifc), face_q_keep(ifc)-face_qm_keep(ifc)
            end if
         end if
     end do
...
subroutine continuum_refilter
  ...
  ! switch back to full nfaces
  nfaces = nfaces_full

  ! cleanup filtering by storing values back into the keep-list
  do ifc = 1,nfaces
     ifc_sub = face_find(ifc)
     if (ifc_sub .ne. 0) then
  ...
  ! free the memory of the arrays with variing size
  deallocate(face_q)
  deallocate(face_qm)
  deallocate(face_qp)
  deallocate(face_fq)
...
!  ===========================================================================
!
!         calculate weighed face2face distance matrix here
! comment:
!   a lot of work in this routine (actualy half of it) is spared
!   by using the symmetrical nature of the matrix
!   but the symmetry is broken if we work parallel, so for the parallel implement. 
!   this will have to be changed
!   to facilitate this I keep the whole matrix (which is not very efficient in terms
!   of the memory reqs.
...
!hms  Note that the expression for the diagonal elements A_ii in Klamt's paper
!hms    A_ii = 3.8 * f(eps) / Sqrt(s_i)
!hms  represents TWICE the self-energy of one face. This is fine if one evaluates
!hms  the whole face-face energy as a double sum and multiplies by 1/2 to account
!hms  for the double counting:
!hms    E_ff = 1/2 * Sum_ij{q_i * A_ij * q_j}
!hms  Here, however, we evaluate E_sff separately, so there is no double counting.
!hms  Hence, we have a factor 1.9 (as in PM's code).

  DO ifc1 = 1,nfaces

     ! do self interaction first
     ...
     ! he also multiplies the potential here by two (consistently the energy is
     ! evaluated without the factor 2.0, because E(face-face) = 0.5 * sum(pot*q)
     ! I stay close to the original code to check the numbers
     !
!hms      wdff(ifc1,ifc1) = self_int_fact/dsqrt(face_area(face_atom(ifc1)))
     wdff(ifc1,ifc1) = self_int_fact * bigf(ifc1)**2 / dsqrt(face_area(face_atom(ifc1)))

!hms  We include the surface fctunction here also for the self-interaction term:
!hms  No self-interaction from buried faces.
!hms  P.S.: This is a minor contribution, in the order of 0.1% of the self-interaction
!hms  energy.

     DO ifc2 = ifc1+1, nfaces
          ! now calculate the rest of the matrix
          ! in the parallel approach this ifc2 has to run from 1 to nfaces,
          ! but there are two different nfaces (one is the full length and the other just a 
          ! portion of the whole length of the matrix

!hms Put in the splining for close-lying faces (cf. parallel version)
!hms The idea is the following:
!hms The true 1/r potential is replaced by a linear function if it
!hms becomes too big. The criterion is the average of the respective self-interaction potentials of the
!hms two charges considered, expressed as a distance Rspline.
!hms The linear function is fit such that it matches the true potential in value and derivative
!hms at Rspline.
!hms We deviate here from RS by including f(eps) in the self-int. potential.
...
!PRINT *, "## The potential used is        : ", wdff(ifc2,ifc1)
          ...
          ! copy to symmetric position (skip in parallel)
...
!  ===========================================================================
!
!      calculate  the force on the gaussians (on "qmad")
!        (assumes pcont to be initialized and adds to it!!!)
!
!hms There seems to be some confusion about "potential" and "force". What we need here
!hms is actually an (electrostatic) potential, not a force. So this is the potential
!hms acting on a nuclear Gaussian due to the surface charges,
!hms   phi_I,nu = E_af/Q_I,nu
...
subroutine continuum_gforces(nat, ng, pcont)
  ...
  integer, intent(in)                            :: nat, ng
  real(kind=8), dimension(ng,nat), intent(inout) :: pcont
  integer                                        :: ifc, ia, ig
!
...
!RS  debug
!       for debugging calculate all contributions into a seperate array
...
subroutine continuum_rforces(nat, r0, ng, rc, qmad, cforce)
  ...
  real(kind=8), dimension(3,nat), intent(in)     :: r0
  real(kind=8), dimension(ng,nat), intent(in)    :: rc, qmad
  real(kind=8), dimension(3,nat), intent(inout)  :: cforce

  integer                                        :: ifc1, ifc2, ia, ia1, ia2, ia3, ig
  real(kind=8), dimension(3)                     :: vect, force
  real(kind=8)                                   :: dist, wq1, wq2, factor, bigfactor, expon
  real(kind=8)                                   :: two_o_spi, errfct 
  REAL(kind=8)                                   :: rspline, eff_dist

  ! hardness constant (from peters code)
  real(kind=8), parameter    :: K1 = 1.0D1
!   ----- THIS IS FITTED TO R(C)=3.32AU AND R(H)=2.17AU
...
!
  ...
  two_o_spi = 2.0D0/dsqrt(4.0D0*datan(1.0D0))

  ! (1)  :  face-face interaction
  !         this invlolves a double loop ofer the faces
  ...
  !               the intra-charge interaction is NOT weiged by the surface fct.
  !               which means something like an additional hardness term
  !               therfore, the intra-face term is NOT geometry dependent and does
  !               not show up here (could in principle be handled according to the hardness)

!hms  There are no forces from the self-interaction term, even if it is multiplied
!hms  by the surface function.
!hms  In the COSMO approximation, the surface area is always taken to be geometry-independent,
!hms  and hence, there is no contribution to the force acting on the nuclei.
!hms  In PAW/COSMO, the surface area is constant
!hms  anyway because we don't recalculate the surface.
!hms  Therefore, the force contribution vanishes both if the face is switched on and if it is off.
  ...
  do ifc1 = 1,nfaces
     do ifc2 = ifc1+1,nfaces
        ...
        !  -> same convention as dbigf to get the right derivative vectors (sign)
        !        vect = pos(2) - pos(1)  : points from 1 to 2
        vect(:) = face_pos(:,ifc2) - face_pos(:,ifc1)
        dist    = dsqrt(sum(vect*vect))
        if (ia1 /= ia2) then
!hms Put in splining for short face-face distances
           ...
           force(:)   = contv_scalingfactor * (wq1 * wq2 * (vect(:)/eff_dist/eff_dist/eff_dist))
            ! the gradients go with oposite signs on the corresponding atoms (to get forces)
...
!            cforce(:,ia2) = cforce(:,ia2) + force(:)
        ...
        do ia = 1,natoms
           if (ia /= ia1) then
               ! derv wrt to bigf of face 1
               force(:) = contv_scalingfactor * (wq2*face_q(ifc1)/dist) * dbigf(:,ia,ifc1)
               ! this acts on the current atom and the atom where the charge 1 sits
...
!               cforce(:,ia1)= cforce(:,ia1)+ force(:)
           ...
           if (ia /= ia2) then
               ! derv wrt to bigf of face 2
               force(:) = contv_scalingfactor * (wq1*face_q(ifc2)/dist) * dbigf(:,ia,ifc2)
               ! this acts on the current atom and the atom where the charge 2 sits
!               cforce(:,ia) = cforce(:,ia) - force(:)
!               cforce(:,ia2)= cforce(:,ia2)+ force(:)
  ...
  do ifc1 = 1,nfaces
     ...
     do ia2 = 1, natoms
       ...
       !         contribs from each gaussian
       !         only if charge NOT on the atom itself
       ...
       do ig = 1,ng
           ...
           if (ia1 /= ia2) then
             force(:) = (bigf(ifc1) * face_q(ifc1) * qmad(ig, ia2) * afv(:,ia2,ifc1)/afd(ia2,ifc1)) * &
            &      (-(errfct/(afd(ia2,ifc1)**2)) + (two_o_spi*exp(-expon**2))/(afd(ia2,ifc1)*rc(ig,ia2))) 
!             cforce(:,ia2) = cforce(:,ia2) - force(:)
!             cforce(:,ia1) = cforce(:,ia1) + force(:)
...
!  ===========================================================================
!
!       calculates the forces on the charges (means the negative of the potential) 
!         (and all the charge dependent energies)
!  
!       watch out for the signs (force or potential)
!       we take the force for fq (neg. pot)
!       but for the energy we need the potential  }8-<
!
!hms Here we need the forces acting on the surface charges (which are dynamical variables),
!hms   F_q_i = -del(E)/del(q_i)
...
!    single loop (pot from gaussians and hardness)
  do ifc1 = 1,nfaces
     ...
     ! set constant png contribution and energy
     face_fq(ifc1) = -png(ifc1)
     face_atom_ener = face_atom_ener + png(ifc1)*face_q(ifc1) !hms  png contains bigf
     ! add hardness potential and energy
     face_fq(ifc1) = face_fq(ifc1) - (2.0D0 * K1 * (1.D0 - bigf(ifc1)) *face_q(ifc1))
     hardness_ener = hardness_ener + (K1 * (1.D0 - bigf(ifc1)) * face_q(ifc1)**2 )
     !
     !  now integrate the charges and the weiged charges first atomwise
!hms  "weig(ht)ed" means multiplied by Fermi function bigf
     atom_charge(face_atom(ifc1)) = atom_charge(face_atom(ifc1)) + face_q(ifc1)
     atom_weighed_charge(face_atom(ifc1)) = atom_weighed_charge(face_atom(ifc1)) + face_q(ifc1)*bigf(ifc1)
 
  end do

!    double loop over all the charges
!        we use again the symmetry of the problem (has to be changed for a parallel implementation
!    for efficiancy we have two pieces of code, one calculates cwdff on the fly, the other uses 
!    the matrix wdffe

  IF (contv_keep_dff) THEN
     ! we use the corresponding entries of wdff
     do ifc1 = 1, nfaces
        ! do self interaction here first
!hms        cpot = wdff(ifc1,ifc1) * face_q(ifc1)
        cpot = 2 * wdff(ifc1,ifc1) * face_q(ifc1)
!hms    Factor 2 from del(E_sff)/del(q_i)

        face_fq(ifc1) = face_fq(ifc1) - cpot
        ! this has to be multiplied by 0.5 at the end
        face_self_ener = face_self_ener + cpot * face_q(ifc1)

        ! now do the "inner" loop
        do ifc2 = ifc1+1, nfaces
             cpot = wdff(ifc1, ifc2) * face_q(ifc2)
             ! pot from ifc2 on ifc1
             face_fq(ifc1) = face_fq(ifc1) - cpot
             ! pot from ifc1 on ifc2
...
!hms    We include the multiplication by the surface fct. here as well

        ! do self interaction here first
!hms         cpot = cwdff * face_q(ifc1)
        cpot = 2 * cwdff * face_q(ifc1)
!hms    Again a factor 2 for the potential
        ...
        ! now do the "inner" loop
        DO ifc2 = ifc1+1, nfaces
             ! calculate the cwdff first
             vect(:) = face_pos(:,ifc1)-face_pos(:,ifc2)
             dist    = dsqrt(sum(vect*vect))
!hms We include the splining
          rspline = 2.0D0 / (self_int_fact*(1/SQRT(face_area(face_atom(ifc1))) &
                                            + 1/SQRT(face_area(face_atom(ifc2)))))            
...
!  ===========================================================================
!
!         this bit registers the energies of the cont. in the energylists
...
!                            PROPAGATE
!
!  this is the heart of the code: it gets called from paw_isolate and does all the
!  runtime calculations!  (it is PUBLIC) 
!
!  we take it from the original code but modify it significantly
!  especially we break up the charge propagation and the calculation of the forces
!  on the nuclei and the gaussian charges 
!  ONLY multiple timestep is possible
!  -> this is ensured by the fact that MULTIPLE must be an even number (at least 2 :-)
!
      ...
      REAL(8),DIMENSION(3 ,NAT),intent(out)   :: CFORCE
      REAL(8),DIMENSION(NG,NAT),intent(out)   :: PCONT
      REAL(8),DIMENSION(3 ,NAT),intent(in)    :: R0
      REAL(8),DIMENSION(NG,NAT),intent(in)    :: QMAD
      REAL(8),DIMENSION(NG,NAT),intent(in)    :: RC
!
      real(kind=8), dimension(3, nat)         :: cforcemem
      real(kind=8), dimension(ng, nat)        :: pcontmem
...
!
           if (tfirst .and. (.not. contv_restart)) then
               exit
           end if
!
...
!
! to be consistent with peters code we have to call continuum_charge_pot again
! because the charges have been propagated and he memorizes the force from NOW
!
       call continuum_charge_pot
!RS DEBUG
           if (contv_debug_print) then
