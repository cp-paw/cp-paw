...
TYPE XXX_TYPE
  ...
  REAL(8)       :: RAD        ! ATOM RADIUS FOR PDOS ETC.
  INTEGER(4),POINTER   :: NPRO(:) ! #(PROJECTORS PER ANGULAR MOMENTUM)
  INTEGER(4),POINTER   :: NTBO(:) ! #(TB ORBITALS ANGULAR MOMENTUM)
  LOGICAL(4),POINTER   :: TORB(:) ! (LNX) SELECTOR FOR LOCAL ORBITALS
END TYPE XXX_TYPE
LOGICAL(4)            :: TINI=.FALSE.
INTEGER(4)            :: NTYPEX=0
INTEGER(4)            :: NTYPE=0
INTEGER(4)    ,SAVE  :: THISTYPE=0 ! POINTER TO THE ACTUAL TYPE OR ZERO
...
!     ******************************************************************
      ...
      DO ITYPE=1,NTYPEX
        ...
        IF(ASSOCIATED(XXX(ITYPE)%NTBO))DEALLOCATE(XXX(ITYPE)%NTBO)
...
!  
      IF(ID.EQ.'ZV') THEN
        IF(XXX(THISTYPE)%TID) THEN
          ...
          CALL ERROR$CHVAL('ATOM TYPE',XXX(THISTYPE)%NAME)
          CALL ERROR$CHVAL('ID',ID)
          CALL ERROR$STOP('ATOMTYPELIST$SETR8')
        END IF
...
!!$        END IF
      ...
      ELSE
        CALL ERROR$MSG('UNKNOWN ID')
        CALL ERROR$CHVAL('ATOM TYPE',TRIM(XXX(THISTYPE)%NAME))
        CALL ERROR$CHVAL('ID',ID)
        CALL ERROR$STOP('ATOMTYPELIST$GETR8')
      END IF
      RETURN
...
!  
      ELSE IF(ID.EQ.'NTBO') THEN
        IF(ASSOCIATED(XXX(THISTYPE)%NTBO)) THEN  !LINUX PATCH
          DEALLOCATE(XXX(THISTYPE)%NTBO)
        END IF    !LINUX PATCH (SEE NULLIFY IN ATOMTYPELIST$INITIALIZE)
        LNG1=LEN
        DO I=1,LEN
          IF(VAL(I).GT.0)LNG1=I
        ENDDO
        ALLOCATE(XXX(THISTYPE)%NTBO(LNG1))
        XXX(THISTYPE)%NTBO(:)=VAL(:LNG1)
!
      ELSE
        CALL ERROR$MSG('UNKNOWN ID')
        CALL ERROR$CHVAL('ATOM TYPE',XXX(THISTYPE)%NAME)
...
!          VAL(:)=10000
      ...
      ELSE IF(ID.EQ.'NTBO') THEN
        IF(.NOT.ASSOCIATED(XXX(THISTYPE)%NTBO)) THEN
          IF(.NOT.ASSOCIATED(XXX(THISTYPE)%NPRO)) THEN
            CALL ERROR$MSG('NEITHER NTBO NOR NPRO ARE AVAILABLE')
            CALL ERROR$CHVAL('ATOM TYPE',XXX(THISTYPE)%NAME)
            CALL ERROR$CHVAL('ID',ID)
            CALL ERROR$STOP('ATOMTYPELIST$GETI4')
          END IF
          ...
          ELSE
            VAL(:)=XXX(THISTYPE)%NPRO(1:LEN)
            CALL ERROR$MSG('INCONSISTENT ARRAY SIZE')
            CALL ERROR$MSG('FALL-BACK SOLUTION NPRO FOR NTBO')
            CALL ERROR$CHVAL('ATOM TYPE',XXX(THISTYPE)%NAME)
            CALL ERROR$CHVAL('ID',ID)
            CALL ERROR$I4VAL('ARRAY SIZE EXPECTED',LEN)
            CALL ERROR$I4VAL('ACTUAL ARRAY SIZE',LEN1)
          ...
          IF(LEN.GE.LEN1) THEN
            VAL(:)=0
            VAL(1:LEN1)=XXX(THISTYPE)%NTBO
          ELSE
            VAL(:)=XXX(THISTYPE)%NTBO(1:LEN)
            CALL ERROR$MSG('INCONSISTENT ARRAY SIZE')
            CALL ERROR$CHVAL('ATOM TYPE',XXX(THISTYPE)%NAME)
            CALL ERROR$CHVAL('ID',ID)
            CALL ERROR$I4VAL('ARRAY SIZE EXPECTED',LEN)
...
!
      ...
      ELSE IF(ID.EQ.'ID') THEN
!       == ID IDENTIFIES AN ENTRY ON THE SETUP PARAMETER FILE ===============
        VAL=XXX(THISTYPE)%ID
      ELSE
        CALL ERROR$MSG('UNKNOWN ID')
        CALL ERROR$CHVAL('ATOM TYPE',XXX(THISTYPE)%NAME)
