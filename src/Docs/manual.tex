...
\item Density of states weightd with occupations 
\end{enumerate}
...
!DCNTL
  ...
  !GRID    EMIN[EV]=-18. EMAX[EV]=0. DE[EV]=0.1 BROADENING[EV]=0.1 !END
...
\block{!DCNTL!GRID}
%----------------------------------------------------------------------
\brules{optional} \bdescr{defines the energy grid and termal energy
  broadening.  Read section~\ref{sec:thermoalbroadening} for the
  background of the thermal broadening.  }
...
\mbax{\key{BROADENING[EV]}
\vdescr{Thermal broading $k_BT$ of the density of states expressed in eV. }
\vformat{real} 
\vrules{optional, mutually exclusive with BROADENING[K]}
\vdefault{300 Kelvin/eV}}

\mbax{\key{BROADENING[K]}
\vdescr{Thermal broading $k_BT$ of the density of states with $T$
  expressed in Kelvin.}
\vformat{real} 
...
\mbax{\key{ID}
\vdescr{defines the name for this set of matrix elements. Note that
  the file name for the DoS and NoS files will becomposed from this ID
  as PREFIX//ID//'.dos' and PREFIX//ID//'.nos'. The file format is
  described in section~\ref{sec:formatdosfile}}
\vformat{character} 
\vrules{mandatory} 
\vdefault{none}}
...
\mbax{\key{SPIN}
\vdescr{specifies the spin projection for non-collinear calculations
\begin{description}
...
\end{description}}
\vformat{character} 
\vrules{optional; only used when ``type'' is present. 
       Not allowed for collinear calculations.}
\vdefault{none}}
...
eigenvalues $\epsilon_n $ and eigenstates $|\tilde \psi_n\rangle$, which are
necessary for the projections $|\langle\tilde p_{IPRO} |\tilde
\psi_i\rangle|^2$, we have to solve the Kohn-Sham equation for the pseudo wave
functions:
\begin{equation}
...
\end{equation}
(For details please refer to Peter Bloechls main publication about the paw
formalism.)\\For the calculation of the eigenvalues and eigenstates we build up
the operators $\hat{\tilde H}$ and $\hat{\tilde S}$ is a finite plane wave basis
and then solve the resulting generalized eigenvalue problem, where $\tilde H$ is
hermitian and $\tilde S$ is positive definite.\\
