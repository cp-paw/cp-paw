...
The calling sequence is
\begin{center}
\verb|paw_stpa.x| \textit{parameter file} \verb|>| \textit{out}
\end{center}
where ``\textit{parameter}'' is a keyword selecting the information to
be visualized.  \textit{file} is the file written by the paw code. It
has the ending ``\verb|stpforz|\textit{Z}\verb|.myxml|'', where
\textit{Z} is the atomic number. \textit{out} is the name of the file
keeping the output.  The format is ``x y1 y2 \ldots'' and is written
line-by-line in free format.  It can be viewed for example with the
xy-plotting tool xmgrace.
\begin{center}
\verb|xmgrace -nxy| \textit{out}
\end{center}

Here a list of the parameters recognized.
\begin{center}
\begin{tabular}{ll}
SCATTERING & \\
...
NLPHIDOT   & nodeless scattering partial waves\\ 
\end{tabular}
\end{center}
...
Values for the magnetic moment and the nuclear spin of the isotopes
can be obtained from the ``Green Book''\cite{greenbook}.
