...
%----------------------------------------------------------------------
\brules{optional} 
  \bdescr{defines the energy grid and termal energy
...
\begin{verbatim}
!DPCNTL
  XMIN[EV]=-7. XMAX[EV]=25.  YMIN[EV]=-0.0 YMAX[EV]=0.5  XZERO[EV]=14.
  !GRAPH PREFIX='DOS/'
...
\mbax{\key{XZERO[EV]}
\vdescr{shifts the energy axis of all graphs so the value given (in
  eV) is the new zero. (Often this is chosen as the highest occupied
  state, respectively the Fermi level.) may be overwritten by xzero in
  !graph and !set}
\vformat{real} 
\vrules{optional}
\vdefault{0.}}
...
\mbax{\key{YMIN[EV]}
\vdescr{lower bound in eV of the density-of-states interval shown.  A
