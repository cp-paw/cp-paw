...
\brules{optional; default is fixed unit cell, and no stress
  calculation} 
\bdescr{This option is experimental!.  Invokes
  pressure calculation, and Parrinello-Rahman dynamics of
  the unit cell, if requested. }
%
\mbax{\key{MOVE}
...
\vdescr{Instrumental broeadening FWHM (Full width at half maximum) in eV.}
\vformat{real} 
...
\mbax{\key{ID}
\vdescr{identifier for the setup type on the parameter 
file stp.cntl in the parameters directory.}
\vformat{Character}
\vrules{Optional. Used only if FILE is not specified}
\vdefault{first two letters of NAME}}
...
\vformat{Character} 
\vrules{Optional, deprecated option.} 
\vdefault{information will be created on the fly using the parameter file paramters/stp.cntl}}


\mbax{\key{ZV}
\vdescr{Number of valence electrons}
\vformat{real}
\vrules{Mandatory, if FILE has been specified. Otherwise not used}
\vdefault{None}} 
...
\mbax{\key{PS$<$G2$>$} 
  ...
  \vformat{real}
  \vrules{Optional, if FILE has been specified. Otherwise not used}
\vdefault{0}}
...
%----------------------------------------------------------------------
\brules{optional} \bdescr{sets the parameters for Hybrid calculations using
natural tight binding orbitals. \textbf{Experimental option!}}

\mbax{\key{TORB} 
   \vdescr{selects the shells of orbitals that shall be
    treated as correlated orbitals. TORB refers to NPRO in the parent
    block. The number values in TORB must be equal to the sum in NPRO.
    For a given angular momentum the correlated orbitals must come
    before the uncorrelated orbitals}
\vformat{logical(*)} 
\vrules{mandatory} 
\vdefault{none}}

\mbax{\key{LHFWEIGHT} 
 \vdescr{Overwrites the generic mixing factor for 
     exchange of the onsite terms of this atom type.}
\vformat{real} 
\vrules{Optional} 
\vdefault{none}}
...
\mbax{\key{NDDO} 
\vdescr{Exchange term with three orbitals on one site
    and one on the opposite side of a bond.}  
\vformat{logical}
\vrules{optional} 
\vdefault{true}}
...
\brules{optional} \bdescr{sets the parameters for lda+U
    calculation\cite{anisimov91_prb44_943,anisimov91_prb44_943}.. Only
    used when !CONTROL!DFT:LDA+U=T.}
...
\mbax{\key{UPAR[EV]} \vdescr{U-parameter for the orbital shell
    specified by MAINLN. The entire interaction matrix is scaled so
    that the U-parameter of the specified shell has the value UPAR.
    (The U-parameter is identical to the Slater integral $F^0$}
  \vformat{real} \vrules{optional, incompatible with DIEL}
  \vdefault{none}}

\mbax{\key{JPAR[EV]} \vdescr{J-parameter for the orbital shell
    specified by MAINLN.  The Slater integrals $F^2,F^4,\ldots$ are
    rescaled. The U and J-parameters are defined as
\begin{eqnarray*}
...
\end{eqnarray*}
and the U-tensor matrix elements are defined from the correlated
orbitals $\chi_i(\vec{r})$ as
\begin{eqnarray*}
U_{i,j,k,l}=\int d^3r\int d^3r'\;
\frac{e^2\chi_i^*(\vec{r})\chi_j^*(\vec{r'})\chi_k(\vec{r})\chi_l(\vec{r'})}
...
\mbax{\key{F4/F2} \vdescr{changes the Slater integral $F^4$ of the
    main shell relative to $F^2$. The Slater integrals are rescaled to
    leave the J-parameter untouched.}
\vformat{real} 
  \vrules{optional, incompatible with DIEL} 
\vdefault{none}}

\mbax{\key{F6/F2} \vdescr{Changes the Slater integral $F^6$ of the
    main shell relative to $F^2$. The Slater integrals are rescaled to
    leave the J-parameter untouched.}
\vformat{real} 
  \vrules{optional, incompatible with DIEL} 
...
angular momentum $\ell$ (first value) and the index (second) of this
shell. Example: MAINLN= 3 1 specifies the first correlated orbital
shell in the f-angular momentum channel.}  
\vformat{integer(2)}
\vrules{optional, mandatory with UPAR and JPAR} 
\vdefault{none}}

\mbax{\key{OLD} \vdescr{Switches to the old formulation of LDA+U
according to Bengone et al., Phys. Rev. B. 62, 16392 (1992). Specify
exactly one angular momentum shell as correlated orbitals. Specify
RCUT to the muffin tin or atomic sphere radius if you want compare to
LAPW or LMTO results. Specify UPAE[EV]. (This option is used for
comparisons only. It is not fully supported or compatible with all other options.} 
\vformat{integer(2)}
\vrules{optional, mandatory with UPAR and JPAR} 
\vdefault{none}}

%----------------------------------------------------------------------
{\block{!STRUCTURE!SPECIES!HYBRID}
%----------------------------------------------------------------------
\brules{optional} \bdescr{sets the parameters for lda+U
calculation. \textbf{Experimental option!}}
...
%\mbax{\key{RCUT} 
%\vdescr{Correlated orbitals are constructed so that 
%they have a node at the radius RCUT.}
%\vformat{real} 
%\vrules{optional} 
%\vdefault{covalent radius plus 15\%}}

\mbax{\key{CV} 
\vdescr{Switch to include core valence exchange contribution}
\vformat{logical} 
\vrules{optional} 
\vdefault{.true.}}

\mbax{\key{HFWEIGHT} 
\vdescr{weight of HF fock exchange.}
\vformat{real} 
\vrules{optional} 
\vdefault{1/4}}
...
\bdescr{Places a confining potential around the
  ...
  maximum value $V_0$ at a rsadius $r_{out,R}$ equal to twice the Van der
...
\bdescr{Supresses the angular momentum $\vec{L}$ of a group of atoms
...
One can also specify specific angular momenta such as 
'PX', 'PY', 'PZ', 'DX2-Y2', 'DXZ', 'D3Z2-R2', 'DYZ', 'DXY'.
For the f-orbitals we use the notation 'F1', \ldots, 'F7'.
}
\vformat{character}
\vrules{mandatory}
\vdefault{none}}
...
other. Within a graph several density of states data can be drawne
...
%----------------------------------------------------------------------
\brules{mandatory}
\bdescr{containes direwctions on what the tool shall do}

\mbax{\key{XZERO}
\vdescr{shifts the energy axis of all graphs so the value given is the
  new zero. (Often this is chosen as the highest occupied state,
  respectively the Fermi level.) may be overwritten by xzero in !graph
  and !set}
\vformat{real} 
\vrules{optional}
\vdefault{0.}}
...
\mbax{\key{YMIN}
\vdescr{lower bound of the density-of-states interval shown.  A
  negative value allows to also show the opposite spin direction.
  Caution: The density of states prepared by the dos tool is given in
  atomic units, even though the energies are expressed in eV.}
\vformat{real} 
\vrules{mandatory}
\vdefault{none}}

\mbax{\key{YMAX}
\vdescr{upper bound of the density-of-states interval shown. Caution:
  The density of states prepared by the dos tool is given in atomic
  units, even though the energies are expressed in eV.}
\vformat{real}
\vrules{mandatory}
\vdefault{none}}
...
\mbax{\key{XZERO}
\vdescr{shifts the energy axis of sets in this graph so the value given is the
  new zero.  May be overwritten by XZERO in !SET.}
\vformat{real} 
\vrules{optional}
...
\begin{enumerate}
\item empty and full, spin-up density of states
\item empty and full, spin-down density of states
\item full, spin-up density of states
\item full, spin-down density of states
\end{enumerate}
}
...
\mbax{\key{XZERO}
\vdescr{shifts the energy axis for this set so the value given is the
  new zero. }
\vformat{real} 
\vrules{optional}
...
\mbax{\key{FILLCOLOR} \vdescr{Fills the density of states with
...
name   &RGB &name   &RGB \\
\hline
black     & \colorbox{black     }{\textcolor{white}{(000,000,000)}}  & lblack &  \colorbox{lblack  }{(150,150,150)} \\
...
blue      & \colorbox{blue      }{\textcolor{white}{(000,000,255)}}     & lblue       & \colorbox{lblue}{(150,150,255)} \\
...
\begin{itemize}
  \item calculation of bandstructures including fat bands and projected 
    bandstructures from paw-calculations with either a simple linear 
    interpolation scheme on the k-point grid or by a direct 
    diagonalisation of the paw hamiltonian
...
For that purpose the control input file is split up into two main 
sections: BANDSTRUCTURE and PDOS. Which are connected by the fact
that both use the diagonalisation of the paw hamiltonian.\\
Non-collinear calculations are currently only partially
implemented and  are untested.
...
k-point grid by using the data in the pdos-file, which makes is very fast, 
but rather inaccurate. \\
The second one uses an explicit calculation of the 
energy eigenvalues of the paw hamiltonian at given k-points. That means that
at the chosen k-points the bandstructure is represented exactly within the
accuracy of the underlying paw calculation. This mode produces very nice 
bandstructures, but has a much higher computational demand than the 
interpolation method, because a large generalized eigenproblem has to be 
solved for every k-point. Because of the high computational demand of 
diagonalisation, there is also a mpi-parallelized variant, which is 
called ppaw\_bands.x and can be compiled separately with 
...
\mbax{\key{mode}
  ...
  possible values are 'LINEARINTERPOLATION'' and 'DIAGONALISATION', which are described below.}
\vformat{character} 
\vrules{optional}
...
%----------------------------------------------------------------------
\brules{optional,multiple}
\bdescr{defines one line seqment in k-space, i.e. a part of a bandstructure}

\mbax{\key{xk1}
\vdescr{First point of the line segment in relative coordinates 
of the reciprocal lattice vectors. kvec1=GBAS*xk1}
\vformat{real(3)} 
\vrules{mandatory (xk1 or kvec1)}
\vdefault{none}}

\mbax{\key{kvec1}
\vdescr{First point of the line segment in absolute coordinates.}
\vformat{real(3)} 
\vrules{mandatory (xk1 or kvec1)}
\vdefault{none}}

\mbax{\key{xk2}
\vdescr{Last point of the line segment in relative coordinates 
of the reciprocal lattice vectors. kvec2=GBAS*xk2}
\vformat{real(3)} 
\vrules{mandatory (xk2 or kvec2)}
\vdefault{none}}

\mbax{\key{kvec2}
\vdescr{Last point of the line segment in absolute coordinates.}
\vformat{real(3)} 
\vrules{mandatory (xk2 or kvec2)}
\vdefault{none}}
...
\mbax{\key{NKDIAG}
\vdescr{number of grid points along the line segment for diagonalisation. IF NKDIAG$<$NK the bands obtained from the diagonalisation of the paw hamiltonian will be interpolated using a 1D-FFT. NKDIAG$>$NK causes an error.}
\vformat{integer} 
\vrules{optional}
\vdefault{NK}}
...
\mbax{\key{XKPROJECT}
\vdescr{direction for the projection for 2-d band structures 
in relative coordinates of the reciprocal lattice vectors. Not yet implemented for METHOD=1.}
\vformat{real(3)} 
\vrules{optional}
\vdefault{(0.,0.,0.)}}
...
\mbax{\key{IPRO}
\vdescr{number of projector for the calculation of the projected band structure.}
\vformat{integer} 
\vrules{mandatory}
\vdefault{none}}

\mbax{\key{LINESPERBAND}
\vdescr{number of lines per band to draw in band structure in order to represent the value of the projection.}
\vformat{integer} 
\vrules{optional}
\vdefault{10}}
...
%----------------------------------------------------------------------
\brules{optional}
\bdescr{defines the calculation of energy eigenvalues and projections, i.e. the calculation of a new PDOS-file, which is compatible with the one produced from the main paw program.}
...
\mbax{\key{NB}
\vdescr{number of bands. This parameter does not influence the computational complexity, because the full spectrum of the paw hamiltonian is calculated. It only determines the number of bands in the output files.}
\vformat{integer} 
\vrules{optional}
\vdefault{20}}
...
\mbax{\key{SPACEGROUP}
\vdescr{Symmetry of the crystal used for reducing the number of k-points 
that have to be calculated. See description below.}
\vformat{integer} 
\vrules{optional}
\vdefault{1}}
...
The banddata-file contains all necessary information to build up the paw
hamiltonian $\hat {\tilde H}$ in the plane wave basis. To calculate the 
eigenvalues $\epsilon_n $
and eigenstates $|\tilde \psi_n\rangle$, which are necessary for the 
projections $|\langle\tilde p_{IPRO} |\tilde \psi_i\rangle|^2$, we 
have to solve the Kohn-Sham equation for the pseudo wave functions:
\begin{equation}
  \hat{\tilde H}|\tilde \psi_n\rangle=\epsilon_n \hat{\tilde S} |\tilde \psi_n\rangle,
\end{equation}
Here the hamilton operator is given as
\begin{equation}
\hat{\tilde H} =\frac{\hat{\vec p}^2}{2}+\tilde v+\sum_{i,j}|\tilde p_i\rangle\left(\langle \phi_i|\frac{\hat{\vec p}^2}{2}+v^1|\phi_j\rangle-\langle \tilde\phi_i|\frac{\hat{\vec p}^2}{2}+\tilde v^1|\tilde\phi_j\rangle\right)\langle \tilde p_j|
\end{equation}
and the overlap operator can be written as
\begin{equation}
  \hat{\tilde S}=1+\sum_{i,j}|\tilde p_i\rangle[\langle \phi_i|\phi_j\rangle-\langle \tilde\phi_i|\tilde\phi_j\rangle]\langle \tilde p_j|.
\end{equation}
(For details please refer to Peter Bloechls main publication about 
the paw formalism.)\\For the calculation of the eigenvalues and 
eigenstates we build up the operators 
$\hat{\tilde H}$ and $\hat{\tilde S}$ is a finite plane wave 
...
The dimension of the matrix, which has to be diagonalized depends 
quadratically on this parameter. So at the expense of precision a 
lowering of this value greatly speeds up the bandsstructure calculation 
with paw\_bands.
SPACEGROUP
...
...
\noindent Options:\\
\hspace*{1.cm}\fbox{\begin{minipage}{0.9\linewidth}
\begin{description}
\item[-h]help message
\item[?]help message
\item[-etot]prints total energy in Hartree (default)
\item[-gap]prints fundamental band gap in eV
\item[-homo]prints energy of the highest occupied state in eV
\item[-lumo]prints energy of the lowest unoccupied state in eV
...
Remark: -gap -homo and -lumo do not work with variable occupations
and will provide an error message.
...
\bigskip\fbox{{\tt paw\_murnaghan.x\ <in.dat}}\bigskip
