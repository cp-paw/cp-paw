...
\mbax{\key{ID} 
\vdescr{Identifier for the setup type. Allows to select
    parameterset defining the augmentation (e.g. partial waves and
    projector functions). The ID must be recognized by internally or
    it must identify a parameter set on a parameter file specified by
    !CNTL!FILES!FILE with name=PARMS\_STP.}
\vformat{character}
\vrules{optional. not used when !STRUCTURE!SPECIES!SETUP is present.}
\vdefault{none}}
...
\end{cases}
\end{eqnarray}
where $v(r)$ is tha radial component of the true atomic potential.
}
...
%----------------------------------------------------------------------
\brules{optional, atom cointribution is scaled to zero if not specified.}
...
\vdescr{Augmentation radius in units of the covalent radius of the
  ...
  matched to the envelope function.}
\vformat{real}
\vrules{optional} 
\vdefault{1.}}
...
\vdescr{defines the name for this set of matrix elements. Note that
  the file name for the DoS and NoS files will be composed from this ID
