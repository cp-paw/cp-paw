...
%----------------------------------------------------------------------
\brules{optional}
\bdescr{Defines band data file.}

\mbax{\key{NAME}
\vdescr{file name of band data file from paw calculation.}
\vformat{character} 
\vrules{mandatory}
...
%----------------------------------------------------------------------
\brules{mandatory}
\bdescr{defines method to calculate band structure}

\mbax{\key{ID}
\vdescr{ID=0 uses a linear interpolation of the energy bands from the paw calculation (Note: this method produces wrong results and is only included for testing purposes), ID=1 uses a diagonalisation of the paw hamiltonian.}
\vformat{integer} 
\vrules{optional}
\vdefault{1}}

\mbax{\key{METHOD\_DIAG}
\vdescr{determines the method to diagonalize the paw hamiltonian if ID=1. METHOD\_DIAG=1 is the full diagonalisation with lapack routines. Other methods are not implemented yet.}
\vformat{integer} 
\vrules{optional}
\vdefault{1}}

\mbax{\key{EPWPSI}
\vdescr{determines the plane wave cutoff for the diagonalisation in Rydberg. See !CONTROL!FOURIER!EPWPSI.}
\vformat{real} 
\vrules{optional}
\vdefault{value from banddata-file}}

%----------------------------------------------------------------------
\block{!BCNTL!LINE}
%----------------------------------------------------------------------
\brules{optional,multiple}
\bdescr{defines one line seqment in k-space}
...
If none is specified, the file is the one with extension '.bands'}}

%----------------------------------------------------------------------
\block{!BCNTL!LINE!FATBAND}
%----------------------------------------------------------------------
\brules{optional,multiple}
\bdescr{defines an orbital projected band structure (fat bands). This rudimentary interface will be extended in the future.}
...
%==========================================================================
If method=0 the bands are interpolated between the grid points using a linear
interpolation in each tetrahedron.\\
If method=1 the bands are calculated using a full diagonalisation of the paw 
hamiltonian in the basis of plane waves. The projections are evaluated from the
eigenvectors and the projectors, i.e. 
$|\langle\tilde p_\alpha |\tilde \psi_i\rangle|^2$. This means that projections
with different projectors are not comparable. The implementation has 
only been tested for spin-restricted and collinear calculations.
