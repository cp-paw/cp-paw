...
#IF DEFINED(CPPVAR_CUBLAS_ACC)
     ...
     &        WAVES_ACC_THIS_PSI0_DELETE &
     &       ,WAVES_ACC_THIS_PSIM_DELETE
#ENDIF
#IF DEFINED(CPPVAR_CUBLAS_ACC)
     ...
     &       ,CPPAW_CUBLAS_ACC_PSI0_PRINFO_RESIDENCY_ENABLED &
     &       ,CPPAW_CUBLAS_ACC_PSIM_SWITCH_RESIDENCY_ENABLED &
...
#ENDIF
      ...
      LOGICAL(4)             :: TRESIDENTPSI0PRINFO
      LOGICAL(4)             :: TRESIDENTPSIMSWITCH
      LOGICAL(4)             :: TUSE1COVBATCH
...
#ENDIF
      ...
      TRESIDENTPSI0PRINFO=.FALSE.
      TRESIDENTPSIMSWITCH=.FALSE.
#IF DEFINED(CPPVAR_CUBLAS_ACC)
...
#IF DEFINED(CPPVAR_CUBLAS_ACC)
      TRESIDENTPSIMSWITCH= &
     &    CPPAW_CUBLAS_ACC_PSIM_SWITCH_RESIDENCY_ENABLED() &
     &    .AND.TRESIDENTOVERLAP.AND.(.NOT.TSTRESS)
#ENDIF
!
!     ==========================================================================
!     == COLLECT OCCUPATIONS                                                  ==
...
#IF DEFINED(CPPVAR_CUBLAS_ACC)
     ...
     &           .AND.CPPAW_CUBLAS_ACC_SHOULD_USE_PROJECTION(PROJFLOPS)
            ENDDO
            IF(NBH.NE.NB) THEN
              ACCINVFLOPS=8.D0*REAL(NGL*NDIM,KIND=8) &
     &                    *REAL(NBH,KIND=8)*REAL(NBH,KIND=8)
              TRESIDENTOPSI=TRESIDENTOPSI &
...
!         == THIS LOOP IS DONE EXPLICIT, BECAUSE THE IFC10 COULD NOT HANDLE IT
          DO IB=1,NBH
            ...
            ENDDO
          ENDDO
          CALL WAVES_ADDPRO(MAP,GSET,NAT,R0,NGL,NDIM,NBH,NPRO,THIS%OPSI &
     &                     ,OPROJ,'OPSI')
          DEALLOCATE(OPROJ)
ELSE
          ALLOCATE(THIS%OPSI(NGL,NDIM,NBH))
...
!++++++++++++++++++++++++ TO HERE +++++++++++++++++++++++++++++++++++++++
#IF DEFINED(CPPVAR_CUBLAS_ACC)
          IF(TRESIDENTPSI0ORTHO.AND.THIS%PSI0_ACC_RESIDENT &
     &       .AND.(.NOT.TRESIDENTPSI0PRINFO)) THEN
...
!         ======================================================================
#IF DEFINED(CPPVAR_CUBLAS_ACC)
          IF(THIS%PSIM_ACC_RESIDENT.AND.(.NOT.TRESIDENTOVERLAP)) THEN
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
...
!$ACC UPDATE SELF(THIS%PSIM(1:NGL,1:NDIM,1:NBH))
            IF(.NOT.TRESIDENTPSIMSWITCH) THEN
...
#IF DEFINED(CPPVAR_CUBLAS_ACC)
     ...
     &          ('ACC_PRESENT_ORTHO_PSIM','ACC_COPY_ORTHO_PSIM_IN' &
     &          ,'ACC_COPY_ORTHO_PSIM_OUT' &
     &          ,NGL,NDIM,NBH,THIS%PSIM)
            CALL CPPAW_CUBLAS_ACC_PROFILE_PRESENT_C8_3D &
     &          ('ACC_PRESENT_ORTHO_OPSI','ACC_COPY_ORTHO_OPSI_IN' &
     &          ,NGL,NDIM,NBH,THIS%OPSI)
#ENDIF
          END IF
#ENDIF
...
!$ACC& PRESENT_OR_COPYIN(THIS%OPSI(1:NGL,1:NDIM,1:NBH)) &
!$ACC& IF(TRESIDENTOVERLAP)
...
#ENDIF
          ...
          TUSE1COVBATCH=.FALSE.
#IF DEFINED(CPPVAR_CUBLAS_ACC)
          CALL WAVES_1COVERLAP_ORTHO_CUBLAS(MAP,NDIM,NBH,NB,NPRO &
     &                                     ,THIS%PROJ,OPROJ &
     &                                     ,MAT,OMAT,OOMAT &
     &                                     ,TUSE1COVBATCH)
#ENDIF
          IF(.NOT.TUSE1COVBATCH) THEN
            CALL WAVES_1COVERLAP(MAP,NDIM,NBH,NB,NPRO,THIS%PROJ &
     &                           ,THIS%PROJ,MAT)
            CALL WAVES_1COVERLAP(MAP,NDIM,NBH,NB,NPRO,OPROJ &
     &                           ,THIS%PROJ,OMAT)
            CALL WAVES_1COVERLAP(MAP,NDIM,NBH,NB,NPRO,OPROJ &
     &                           ,OPROJ,OOMAT)
          END IF
...
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
          CALL ACCELPROFILE$NOW(ACCEL_T0)
#ENDIF
          TRESIDENTADDOPSI=TRESIDENTOVERLAP
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
          IF(TRESIDENTADDOPSI) THEN
#IF DEFINED(CPPVAR_CUBLAS_ACC)
            CALL CPPAW_CUBLAS_ACC_PROFILE_PRESENT_C8_3D_INOUT &
     &          ('ACC_PRESENT_ADDOPSI_PSIM','ACC_COPY_ADDOPSI_PSIM_IN' &
     &          ,'ACC_COPY_ADDOPSI_PSIM_OUT' &
     &          ,NGL,NDIM,NBH,THIS%PSIM)
            IF(NBH.EQ.NB) THEN
              CALL CPPAW_CUBLAS_ACC_PROFILE_PRESENT_C8_3D &
     &            ('ACC_PRESENT_ADDOPSI_OPSI','ACC_COPY_ADDOPSI_OPSI_IN' &
     &            ,NGL,NDIM,NBH,THIS%OPSI)
              CALL CPPAW_CUBLAS_ACC_PROFILE_PRESENT_C8_2D &
     &            ('ACC_PRESENT_ADDOPSI_LAM','ACC_COPY_ADDOPSI_LAM_IN' &
     &            ,NB,NB,LAMBDA)
            END IF
#ENDIF
          END IF
#ENDIF
          IF(TRESIDENTADDOPSI.AND.(NBH.EQ.NB)) THEN
!$ACC DATA COPY(THIS%PSIM(1:NGL,1:NDIM,1:NBH)) &
!$ACC& PRESENT_OR_COPYIN(THIS%OPSI(1:NGL,1:NDIM,1:NBH)) &
!$ACC& COPYIN(LAMBDA(1:NB,1:NB)) IF(TRESIDENTADDOPSI)
            CALL WAVES_ADDOPSI(NGL,NDIM,NBH,NB,THIS%PSIM,THIS%OPSI &
     &                        ,LAMBDA)
!$ACC END DATA
          ELSE IF(TRESIDENTADDOPSI) THEN
!$ACC DATA COPY(THIS%PSIM(1:NGL,1:NDIM,1:NBH))
            CALL WAVES_ADDOPSI(NGL,NDIM,NBH,NB,THIS%PSIM,THIS%OPSI &
     &                        ,LAMBDA)
!$ACC END DATA
...
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
            CALL ACCELPROFILE$ADD('ACC_COPY_ORTHO_PSIM_OUT' &
     &          ,INT(NGL,KIND=8),INT(NDIM,KIND=8),INT(NBH,KIND=8) &
     &          ,0_8,0.D0,16.D0*REAL(NGL,KIND=8) &
     &          *REAL(NDIM,KIND=8)*REAL(NBH,KIND=8),0.D0)
#ENDIF
...
#ENDIF
!$ACC DATA PRESENT_OR_COPYIN(OPSI(1:NGLNDIM,1:NBH)) &
!$ACC& COPYIN(LAMBDA1(1:NBH,1:NBH),LAMBDA2(1:NBH,1:NBH) &
!$ACC&       ,MINUSGACC(1:NGL)) &
...
#IF DEFINED(CPPVAR_CUBLAS_ACC)
     ...
     &       ,CPPAW_CUBLAS_ACC_INVERSION_RESIDENT_COPY &
     &       ,CPPAW_CUBLAS_ACC_GAMMA_CORRECTION_RESIDENT &
    ...
    &        ,NGL*NDIM,NBH,PSI1,NBH,PSI2,TMAT,ACCEL_CUBLAS_USED)
       ...
       IF(ACCEL_CUBLAS_USED) THEN
         ...
         IF(TSUPER) THEN
           ...
           TMAT(:,:)=2.D0*TMAT(:,:)
           IF(NGAMMA.NE.0) THEN
             CALL CPPAW_CUBLAS_ACC_GAMMA_CORRECTION_RESIDENT &
    &            (NGL,NDIM,NBH,PSI1,NBH,PSI2,NGAMMA,TMAT)
           END IF
         END IF
         TMAT(:,:)=TMAT(:,:)*GWEIGHT
       ELSE
...
#IF DEFINED(CPPVAR_CUBLAS_ACC)
       ...
       IF(TRESIDENTOVERLAP) THEN
         ...
         CALL CPPAW_CUBLAS_ACC_INVERSION_RESIDENT_COPY &
    &        (NGL,NDIM,NBH,PSI1,NBH,PSI2,MINUSGACC,TMAT &
    &        ,ACCEL_CUBLAS_USED)
       END IF
       IF(ACCEL_CUBLAS_USED) THEN
         CALL PLANEWAVE$GETR8('GWEIGHT',GWEIGHT)
...
#IF DEFINED(CPPVAR_CUBLAS_ACC)
      USE CPPAW_CUBLAS_ACC_MODULE, ONLY: &
     &        CPPAW_CUBLAS_ACC_1COVERLAP_ENABLED &
     &       ,CPPAW_CUBLAS_ACC_SCALARPRODUCT_PRESENT
#ENDIF
      IMPLICIT NONE
...
#IF DEFINED(CPPVAR_CUBLAS_ACC)
      COMPLEX(8)  ,ALLOCATABLE  :: PROJ1M(:,:)
      COMPLEX(8)  ,ALLOCATABLE  :: PROJ2M(:,:)
      COMPLEX(8)  ,ALLOCATABLE  :: PROJ1CM(:,:)
#ENDIF
      ...
      LOGICAL(4)                :: TINV
#IF DEFINED(CPPVAR_CUBLAS_ACC)
      INTEGER(4)                :: IPACK,NPACK
      REAL(8)                   :: ONEC_FLOPS,ONEC_BYTES
      LOGICAL(4)                :: TUSECUBLAS1C
#ENDIF
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
      REAL(8)                   :: ACCEL_T0
      REAL(8)                   :: ACCEL_T1
...
#IF DEFINED(CPPVAR_CUBLAS_ACC)
      NPACK=NDIM*NPROX
      ONEC_FLOPS=8.D0*REAL(NPACK,KIND=8)*REAL(NBH,KIND=8) &
     &          *REAL(NBH,KIND=8)
      TUSECUBLAS1C=NPROX.GT.0
      IF(TUSECUBLAS1C) THEN
        TUSECUBLAS1C=CPPAW_CUBLAS_ACC_1COVERLAP_ENABLED(ONEC_FLOPS)
      END IF
      IF(TUSECUBLAS1C) THEN
        ALLOCATE(PROJ1M(NPACK,NBH))
        ALLOCATE(PROJ2M(NPACK,NBH))
        ONEC_BYTES=16.D0*(2.D0*REAL(NPACK,KIND=8)*REAL(NBH,KIND=8) &
     &       +REAL(NBH,KIND=8)*REAL(NBH,KIND=8))
        IF(.NOT.TINV) THEN
          MAT(:,:)=(0.D0,0.D0)
!$ACC DATA COPYIN(PROJ1A(1:NDIM,1:NBH,1:NPROX) &
!$ACC&             ,PROJ2A(1:NDIM,1:NBH,1:NPROX)) &
!$ACC& CREATE(PROJ1M(1:NPACK,1:NBH),PROJ2M(1:NPACK,1:NBH)) &
!$ACC& COPYOUT(MAT(1:NBH,1:NBH))
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
          CALL ACCELPROFILE$NOW(ACCEL_CUBLAS_T0)
#ENDIF
!$ACC PARALLEL LOOP COLLAPSE(3) PRIVATE(IPACK) &
!$ACC& PRESENT(PROJ1A,PROJ2A,PROJ1M,PROJ2M)
          DO IBH1=1,NBH
            DO IPRO1=1,NPROX
              DO IDIM=1,NDIM
                IPACK=(IPRO1-1)*NDIM+IDIM
                PROJ1M(IPACK,IBH1)=PROJ1A(IDIM,IBH1,IPRO1)
                PROJ2M(IPACK,IBH1)=PROJ2A(IDIM,IBH1,IPRO1)
              ENDDO
            ENDDO
          ENDDO
!$ACC END PARALLEL LOOP
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
          CALL ACCELPROFILE$NOW(ACCEL_CUBLAS_T1)
          CALL ACCELPROFILE$ADD('ACC_PACK_CUBLAS_1COV' &
     &       ,INT(NPACK,KIND=8),INT(NBH,KIND=8),INT(NPROX,KIND=8),0_8 &
     &       ,0.D0,16.D0*2.D0*REAL(NPACK,KIND=8)*REAL(NBH,KIND=8) &
     &       ,ACCEL_CUBLAS_T1-ACCEL_CUBLAS_T0)
          CALL ACCELPROFILE$NOW(ACCEL_CUBLAS_T0)
#ENDIF
          CALL CPPAW_CUBLAS_ACC_SCALARPRODUCT_PRESENT(.FALSE. &
     &       ,NPACK,NBH,PROJ1M,NBH,PROJ2M,MAT)
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
          CALL ACCELPROFILE$NOW(ACCEL_CUBLAS_T1)
          CALL ACCELPROFILE$ADD('CUBLAS_ZGEMM_1COV_CMAT1' &
     &       ,INT(NPACK,KIND=8),INT(NBH,KIND=8),INT(NBH,KIND=8),0_8 &
     &       ,ONEC_FLOPS,ONEC_BYTES,ACCEL_CUBLAS_T1-ACCEL_CUBLAS_T0)
#ENDIF
!$ACC END DATA
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
          CALL ACCELPROFILE$ADD('ACC_COPY_1COV_PROJ_IN' &
     &       ,INT(NPACK,KIND=8),INT(NBH,KIND=8),INT(NPROX,KIND=8),0_8 &
     &       ,0.D0,16.D0*2.D0*REAL(NPACK,KIND=8)*REAL(NBH,KIND=8),0.D0)
          CALL ACCELPROFILE$ADD('ACC_COPY_1COV_MAT_OUT' &
     &       ,INT(NBH,KIND=8),INT(NBH,KIND=8),1_8,0_8 &
     &       ,0.D0,16.D0*REAL(NBH,KIND=8)*REAL(NBH,KIND=8),0.D0)
#ENDIF
          CALL MPE$COMBINE('K','+',MAT)
        ELSE
          ALLOCATE(PROJ1CM(NPACK,NBH))
          CMAT1(:,:)=(0.D0,0.D0)
          CMAT2(:,:)=(0.D0,0.D0)
!$ACC DATA COPYIN(PROJ1A(1:NDIM,1:NBH,1:NPROX) &
!$ACC&             ,PROJ2A(1:NDIM,1:NBH,1:NPROX)) &
!$ACC& CREATE(PROJ1M(1:NPACK,1:NBH),PROJ1CM(1:NPACK,1:NBH) &
!$ACC&       ,PROJ2M(1:NPACK,1:NBH)) &
!$ACC& COPYOUT(CMAT1(1:NBH,1:NBH),CMAT2(1:NBH,1:NBH))
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
          CALL ACCELPROFILE$NOW(ACCEL_CUBLAS_T0)
#ENDIF
!$ACC PARALLEL LOOP COLLAPSE(3) PRIVATE(IPACK) &
!$ACC& PRESENT(PROJ1A,PROJ2A,PROJ1M,PROJ1CM,PROJ2M)
          DO IBH1=1,NBH
            DO IPRO1=1,NPROX
              DO IDIM=1,NDIM
                IPACK=(IPRO1-1)*NDIM+IDIM
                PROJ1M(IPACK,IBH1)=PROJ1A(IDIM,IBH1,IPRO1)
                PROJ1CM(IPACK,IBH1)=CONJG(PROJ1A(IDIM,IBH1,IPRO1))
                PROJ2M(IPACK,IBH1)=PROJ2A(IDIM,IBH1,IPRO1)
              ENDDO
            ENDDO
          ENDDO
...
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
          ...
          CALL ACCELPROFILE$ADD('ACC_PACK_CUBLAS_1COV' &
     &       ,INT(NPACK,KIND=8),INT(NBH,KIND=8),INT(NPROX,KIND=8),0_8 &
     &       ,0.D0,16.D0*3.D0*REAL(NPACK,KIND=8)*REAL(NBH,KIND=8) &
     &       ,ACCEL_CUBLAS_T1-ACCEL_CUBLAS_T0)
          CALL ACCELPROFILE$NOW(ACCEL_CUBLAS_T0)
#ENDIF
          CALL CPPAW_CUBLAS_ACC_SCALARPRODUCT_PRESENT(.FALSE. &
     &       ,NPACK,NBH,PROJ1M,NBH,PROJ2M,CMAT1)
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
          CALL ACCELPROFILE$NOW(ACCEL_CUBLAS_T1)
          CALL ACCELPROFILE$ADD('CUBLAS_ZGEMM_1COV_CMAT1' &
     &       ,INT(NPACK,KIND=8),INT(NBH,KIND=8),INT(NBH,KIND=8),0_8 &
...
#ENDIF
          CALL CPPAW_CUBLAS_ACC_SCALARPRODUCT_PRESENT(.FALSE. &
     &       ,NPACK,NBH,PROJ1CM,NBH,PROJ2M,CMAT2)
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
          CALL ACCELPROFILE$NOW(ACCEL_CUBLAS_T1)
          CALL ACCELPROFILE$ADD('CUBLAS_ZGEMM_1COV_CMAT2' &
     &       ,INT(NPACK,KIND=8),INT(NBH,KIND=8),INT(NBH,KIND=8),0_8 &
     &       ,ONEC_FLOPS,ONEC_BYTES,ACCEL_CUBLAS_T1-ACCEL_CUBLAS_T0)
#ENDIF
!$ACC END DATA
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
          CALL ACCELPROFILE$ADD('ACC_COPY_1COV_PROJ_IN' &
     &       ,INT(NPACK,KIND=8),INT(NBH,KIND=8),INT(NPROX,KIND=8),0_8 &
     &       ,0.D0,16.D0*3.D0*REAL(NPACK,KIND=8)*REAL(NBH,KIND=8),0.D0)
          CALL ACCELPROFILE$ADD('ACC_COPY_1COV_CMAT_OUT' &
     &       ,INT(NBH,KIND=8),INT(NBH,KIND=8),2_8,0_8 &
     &       ,0.D0,16.D0*2.D0*REAL(NBH,KIND=8)*REAL(NBH,KIND=8),0.D0)
#ENDIF
...
!      .................................................................
#IF DEFINED(CPPVAR_CUBLAS_ACC)
      SUBROUTINE WAVES_1COVERLAP_ORTHO_CUBLAS(MAP,NDIM,NBH,NB,NPRO &
     &                                      ,PROJ,OPROJ,MAT,OMAT,OOMAT &
     &                                      ,USED)
!     ******************************************************************
!     **  BATCHED GPU PATH FOR THE THREE 1-CENTER OVERLAPS USED BY     **
!     **  WAVES$ORTHOGONALIZE.  THE LEGACY ROUTINE REMAINS THE         **
!     **  FALLBACK; THIS PATH IS OPT-IN THROUGH CPPAW_GPU_1COVERLAP_BATCH.
!     ******************************************************************
      USE MPE_MODULE
      USE WAVES_MODULE, ONLY: MAP_TYPE
      USE CPPAW_CUBLAS_ACC_MODULE, ONLY: &
     &        CPPAW_CUBLAS_ACC_1COVERLAP_BATCH_ENABLED &
     &       ,CPPAW_CUBLAS_ACC_SCALARPRODUCT_PRESENT
      IMPLICIT NONE
      TYPE(MAP_TYPE),INTENT(IN) :: MAP
      INTEGER(4)  ,INTENT(IN)   :: NDIM
      ...
      COMPLEX(8)  ,INTENT(IN)   :: OPROJ(NDIM,NBH,NPRO)
      COMPLEX(8)  ,INTENT(OUT)  :: MAT(NB,NB)
      COMPLEX(8)  ,INTENT(OUT)  :: OMAT(NB,NB)
      COMPLEX(8)  ,INTENT(OUT)  :: OOMAT(NB,NB)
      LOGICAL(4)  ,INTENT(OUT)  :: USED
      REAL(8)     ,ALLOCATABLE  :: DOVER(:,:)
      COMPLEX(8)  ,ALLOCATABLE  :: PROJA(:,:,:)
      COMPLEX(8)  ,ALLOCATABLE  :: OPROJA(:,:,:)
      COMPLEX(8)  ,ALLOCATABLE  :: DPROJA(:,:,:)
      COMPLEX(8)  ,ALLOCATABLE  :: DOPROJA(:,:,:)
      COMPLEX(8)  ,ALLOCATABLE  :: PROJM(:,:)
      COMPLEX(8)  ,ALLOCATABLE  :: OPROJM(:,:)
      COMPLEX(8)  ,ALLOCATABLE  :: DPROJM(:,:)
      COMPLEX(8)  ,ALLOCATABLE  :: DOPROM(:,:)
      COMPLEX(8)  ,ALLOCATABLE  :: PROJCM(:,:)
      COMPLEX(8)  ,ALLOCATABLE  :: OPROJCM(:,:)
      COMPLEX(8)  ,ALLOCATABLE  :: CMAT1(:,:),CMAT2(:,:)
      COMPLEX(8)  ,ALLOCATABLE  :: COMAT1(:,:),COMAT2(:,:)
      COMPLEX(8)  ,ALLOCATABLE  :: COOMAT1(:,:),COOMAT2(:,:)
      INTEGER(4)                :: NTASKS,THISTASK
      INTEGER(4)                :: NPROX,NPACK
      INTEGER(4)                :: IAT,ISP,LMNX,LNX
      INTEGER(4)                :: LN1,LN2,L1,L2
      INTEGER(4)                :: IPRO,IPROX,IPRO1,IPRO2
      INTEGER(4)                :: IBH1,IBH2,IDIM,IPACK
      INTEGER(4)                :: IB1,IB2
      COMPLEX(8)                :: CSVAR1,CSVAR2
      LOGICAL(4)                :: TINV
      REAL(8)                   :: ONEC_FLOPS
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
      ...
      REAL(8)                   :: ACCEL_PACK_T,ACCEL_CONTRACT_T
      REAL(8)                   :: ACCEL_UNRAVEL_T
      REAL(8)                   :: ACCEL_COPY_IN_BYTES
      REAL(8)                   :: ACCEL_COPY_OUT_BYTES
#ENDIF
!     **************************************************************************
      USED=.FALSE.
      TINV=NB.NE.NBH
      IF(NDIM.LE.0.OR.NBH.LE.0.OR.NB.LE.0.OR.NPRO.LE.0) RETURN
      ONEC_FLOPS=8.D0*REAL(NDIM*NPRO,KIND=8)*REAL(NBH,KIND=8) &
     &          *REAL(NBH,KIND=8)
      IF(TINV) ONEC_FLOPS=2.D0*ONEC_FLOPS
      ONEC_FLOPS=3.D0*ONEC_FLOPS
      IF(.NOT.CPPAW_CUBLAS_ACC_1COVERLAP_BATCH_ENABLED(ONEC_FLOPS)) &
     &  RETURN
      USED=.TRUE.
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
      ACCEL_PACK_T=0.D0
      ACCEL_CONTRACT_T=0.D0
      ACCEL_UNRAVEL_T=0.D0
      ACCEL_COPY_IN_BYTES=0.D0
      ACCEL_COPY_OUT_BYTES=0.D0
      CALL ACCELPROFILE$NOW(ACCEL_T0)
#ENDIF
      CALL MPE$QUERY('K',NTASKS,THISTASK)
      IF(NTASKS.EQ.1) THEN
      ...
      NPACK=NDIM*NPROX
      IF(NPROX.GT.0) THEN
        ALLOCATE(PROJA(NDIM,NBH,NPROX))
        ALLOCATE(OPROJA(NDIM,NBH,NPROX))
        ALLOCATE(DPROJA(NDIM,NBH,NPROX))
        ALLOCATE(DOPROJA(NDIM,NBH,NPROX))
        PROJA(:,:,:)=(0.D0,0.D0)
        OPROJA(:,:,:)=(0.D0,0.D0)
        DPROJA(:,:,:)=(0.D0,0.D0)
        DOPROJA(:,:,:)=(0.D0,0.D0)
        ...
        DO IAT=1,MAP%NAT
          ...
          END IF
          LMNX=MAP%LMNX(ISP)
          PROJA(:,:,IPROX+1:IPROX+LMNX) &
     &      =PROJ(:,:,IPRO+1:IPRO+LMNX)
          OPROJA(:,:,IPROX+1:IPROX+LMNX) &
     &      =OPROJ(:,:,IPRO+1:IPRO+LMNX)
          LNX=MAP%LNX(ISP)
          ALLOCATE(DOVER(LNX,LNX))
          CALL SETUP$ISELECT(ISP)
          CALL SETUP$GETR8A('DO',LNX*LNX,DOVER)
          ...
          DO LN1=1,LNX
            ...
            DO LN2=1,LNX
              L2=MAP%LOX(LN2,ISP)
              IF(L1.EQ.L2) THEN
                DPROJA(:,:,IPRO1+1:IPRO1+2*L1+1) &
     &            =DPROJA(:,:,IPRO1+1:IPRO1+2*L1+1) &
     &            +DOVER(LN1,LN2)*PROJ(:,:,IPRO2+1:IPRO2+2*L1+1)
                DOPROJA(:,:,IPRO1+1:IPRO1+2*L1+1) &
     &            =DOPROJA(:,:,IPRO1+1:IPRO1+2*L1+1) &
     &            +DOVER(LN1,LN2)*OPROJ(:,:,IPRO2+1:IPRO2+2*L1+1)
              END IF
              IPRO2=IPRO2+2*L2+1
            ENDDO
            IPRO1=IPRO1+2*L1+1
...
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
        ...
        CALL ACCELPROFILE$NOW(ACCEL_T0)
#ENDIF
        ALLOCATE(PROJM(NPACK,NBH))
        ALLOCATE(OPROJM(NPACK,NBH))
        ALLOCATE(DPROJM(NPACK,NBH))
        ALLOCATE(DOPROM(NPACK,NBH))
        IF(.NOT.TINV) THEN
!$ACC DATA COPYIN(PROJA(1:NDIM,1:NBH,1:NPROX) &
!$ACC&             ,OPROJA(1:NDIM,1:NBH,1:NPROX) &
!$ACC&             ,DPROJA(1:NDIM,1:NBH,1:NPROX) &
!$ACC&             ,DOPROJA(1:NDIM,1:NBH,1:NPROX)) &
!$ACC& CREATE(PROJM(1:NPACK,1:NBH),OPROJM(1:NPACK,1:NBH) &
!$ACC&       ,DPROJM(1:NPACK,1:NBH),DOPROM(1:NPACK,1:NBH)) &
!$ACC& COPYOUT(MAT(1:NBH,1:NBH),OMAT(1:NBH,1:NBH) &
!$ACC&        ,OOMAT(1:NBH,1:NBH))
!$ACC PARALLEL LOOP COLLAPSE(3) PRIVATE(IPACK) &
!$ACC& PRESENT(PROJA,OPROJA,DPROJA,DOPROJA,PROJM,OPROJM,DPROJM,DOPROM)
          DO IBH1=1,NBH
            DO IPRO1=1,NPROX
              DO IDIM=1,NDIM
                IPACK=(IPRO1-1)*NDIM+IDIM
                PROJM(IPACK,IBH1)=PROJA(IDIM,IBH1,IPRO1)
                OPROJM(IPACK,IBH1)=OPROJA(IDIM,IBH1,IPRO1)
                DPROJM(IPACK,IBH1)=DPROJA(IDIM,IBH1,IPRO1)
                DOPROM(IPACK,IBH1)=DOPROJA(IDIM,IBH1,IPRO1)
              ENDDO
            ENDDO
          ENDDO
!$ACC END PARALLEL LOOP
          CALL CPPAW_CUBLAS_ACC_SCALARPRODUCT_PRESENT(.FALSE. &
     &       ,NPACK,NBH,PROJM,NBH,DPROJM,MAT)
          CALL CPPAW_CUBLAS_ACC_SCALARPRODUCT_PRESENT(.FALSE. &
     &       ,NPACK,NBH,OPROJM,NBH,DPROJM,OMAT)
          CALL CPPAW_CUBLAS_ACC_SCALARPRODUCT_PRESENT(.FALSE. &
     &       ,NPACK,NBH,OPROJM,NBH,DOPROM,OOMAT)
!$ACC END DATA
        ELSE
          ALLOCATE(PROJCM(NPACK,NBH))
          ALLOCATE(OPROJCM(NPACK,NBH))
          ALLOCATE(CMAT1(NBH,NBH),CMAT2(NBH,NBH))
          ALLOCATE(COMAT1(NBH,NBH),COMAT2(NBH,NBH))
          ALLOCATE(COOMAT1(NBH,NBH),COOMAT2(NBH,NBH))
          CMAT1(:,:)=(0.D0,0.D0)
          CMAT2(:,:)=(0.D0,0.D0)
          COMAT1(:,:)=(0.D0,0.D0)
          COMAT2(:,:)=(0.D0,0.D0)
...
!$ACC& CREATE(PROJM(1:NPACK,1:NBH),OPROJM(1:NPACK,1:NBH) &
!$ACC&       ,DPROJM(1:NPACK,1:NBH),DOPROM(1:NPACK,1:NBH) &
!$ACC&       ,PROJCM(1:NPACK,1:NBH),OPROJCM(1:NPACK,1:NBH)) &
!$ACC& COPYOUT(CMAT1(1:NBH,1:NBH),CMAT2(1:NBH,1:NBH) &
!$ACC&        ,COMAT1(1:NBH,1:NBH),COMAT2(1:NBH,1:NBH) &
!$ACC&        ,COOMAT1(1:NBH,1:NBH),COOMAT2(1:NBH,1:NBH))
!$ACC PARALLEL LOOP COLLAPSE(3) PRIVATE(IPACK) &
!$ACC& PRESENT(PROJA,OPROJA,DPROJA,DOPROJA,PROJM,OPROJM,DPROJM,DOPROM &
!$ACC&        ,PROJCM,OPROJCM)
          DO IBH1=1,NBH
            DO IPRO1=1,NPROX
              DO IDIM=1,NDIM
                ...
                DPROJM(IPACK,IBH1)=DPROJA(IDIM,IBH1,IPRO1)
                DOPROM(IPACK,IBH1)=DOPROJA(IDIM,IBH1,IPRO1)
                PROJCM(IPACK,IBH1)=CONJG(PROJA(IDIM,IBH1,IPRO1))
                OPROJCM(IPACK,IBH1)=CONJG(OPROJA(IDIM,IBH1,IPRO1))
              ENDDO
            ENDDO
          ENDDO
!$ACC END PARALLEL LOOP
          CALL CPPAW_CUBLAS_ACC_SCALARPRODUCT_PRESENT(.FALSE. &
     &       ,NPACK,NBH,PROJM,NBH,DPROJM,CMAT1)
          CALL CPPAW_CUBLAS_ACC_SCALARPRODUCT_PRESENT(.FALSE. &
     &       ,NPACK,NBH,PROJCM,NBH,DPROJM,CMAT2)
          CALL CPPAW_CUBLAS_ACC_SCALARPRODUCT_PRESENT(.FALSE. &
     &       ,NPACK,NBH,OPROJM,NBH,DPROJM,COMAT1)
          CALL CPPAW_CUBLAS_ACC_SCALARPRODUCT_PRESENT(.FALSE. &
     &       ,NPACK,NBH,OPROJCM,NBH,DPROJM,COMAT2)
          CALL CPPAW_CUBLAS_ACC_SCALARPRODUCT_PRESENT(.FALSE. &
     &       ,NPACK,NBH,OPROJM,NBH,DOPROM,COOMAT1)
          CALL CPPAW_CUBLAS_ACC_SCALARPRODUCT_PRESENT(.FALSE. &
     &       ,NPACK,NBH,OPROJCM,NBH,DOPROM,COOMAT2)
!$ACC END DATA
          DEALLOCATE(PROJCM,OPROJCM)
        END IF
        DEALLOCATE(PROJM,OPROJM,DPROJM,DOPROM)
        DEALLOCATE(PROJA,OPROJA,DPROJA,DOPROJA)
      ELSE
        IF(TINV) THEN
          ALLOCATE(CMAT1(NBH,NBH),CMAT2(NBH,NBH))
          ALLOCATE(COMAT1(NBH,NBH),COMAT2(NBH,NBH))
...
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
      CALL ACCELPROFILE$NOW(ACCEL_T1)
      ACCEL_CONTRACT_T=ACCEL_T1-ACCEL_T0
      ACCEL_COPY_IN_BYTES=16.D0*4.D0*REAL(NPACK,KIND=8) &
     &                  *REAL(NBH,KIND=8)
      IF(TINV) THEN
        ACCEL_COPY_OUT_BYTES=16.D0*6.D0*REAL(NBH,KIND=8) &
     &                     *REAL(NBH,KIND=8)
      ELSE
        ACCEL_COPY_OUT_BYTES=16.D0*3.D0*REAL(NBH,KIND=8) &
     &                     *REAL(NBH,KIND=8)
      END IF
      CALL ACCELPROFILE$ADD('ACC_COPY_1COV_BATCH_PROJ_IN' &
     &    ,INT(NPACK,KIND=8),INT(NBH,KIND=8),4_8,0_8 &
     &    ,0.D0,ACCEL_COPY_IN_BYTES,0.D0)
      CALL ACCELPROFILE$ADD('ACC_COPY_1COV_BATCH_MAT_OUT' &
     &    ,INT(NBH,KIND=8),INT(NBH,KIND=8),MERGE(6_8,3_8,TINV),0_8 &
     &    ,0.D0,ACCEL_COPY_OUT_BYTES,0.D0)
      CALL ACCELPROFILE$ADD('CUBLAS_ZGEMM_1COV_BATCH' &
     &    ,INT(NPACK,KIND=8),INT(NBH,KIND=8),MERGE(6_8,3_8,TINV),0_8 &
     &    ,ONEC_FLOPS,ACCEL_COPY_IN_BYTES+ACCEL_COPY_OUT_BYTES &
     &    ,ACCEL_CONTRACT_T)
#ENDIF
      IF(TINV) THEN
        CALL MPE$COMBINE('K','+',CMAT1)
        CALL MPE$COMBINE('K','+',CMAT2)
        CALL MPE$COMBINE('K','+',COMAT1)
        CALL MPE$COMBINE('K','+',COMAT2)
        CALL MPE$COMBINE('K','+',COOMAT1)
        CALL MPE$COMBINE('K','+',COOMAT2)
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
        CALL ACCELPROFILE$NOW(ACCEL_T0)
#ENDIF
        DO IBH2=THISTASK,NBH,NTASKS
          IB2=2*(IBH2-1)
          DO IBH1=1,NBH
            IB1=2*(IBH1-1)
            CSVAR1=0.5D0*(CMAT1(IBH1,IBH2)+CMAT2(IBH1,IBH2))
            ...
            MAT(IB1+2,IB2+1)=-AIMAG(CSVAR2)
            MAT(IB1+2,IB2+2)=REAL(CSVAR2,KIND=8)
            CSVAR1=0.5D0*(COMAT1(IBH1,IBH2)+COMAT2(IBH1,IBH2))
            CSVAR2=0.5D0*(COMAT1(IBH1,IBH2)-COMAT2(IBH1,IBH2))
            OMAT(IB1+1,IB2+1)=REAL(CSVAR1,KIND=8)
            OMAT(IB1+1,IB2+2)=AIMAG(CSVAR1)
            OMAT(IB1+2,IB2+1)=-AIMAG(CSVAR2)
            OMAT(IB1+2,IB2+2)=REAL(CSVAR2,KIND=8)
            CSVAR1=0.5D0*(COOMAT1(IBH1,IBH2)+COOMAT2(IBH1,IBH2))
            CSVAR2=0.5D0*(COOMAT1(IBH1,IBH2)-COOMAT2(IBH1,IBH2))
            OOMAT(IB1+1,IB2+1)=REAL(CSVAR1,KIND=8)
            OOMAT(IB1+1,IB2+2)=AIMAG(CSVAR1)
            OOMAT(IB1+2,IB2+1)=-AIMAG(CSVAR2)
            OOMAT(IB1+2,IB2+2)=REAL(CSVAR2,KIND=8)
          ENDDO
        ENDDO
        CALL MPE$COMBINE('K','+',MAT)
        CALL MPE$COMBINE('K','+',OMAT)
        CALL MPE$COMBINE('K','+',OOMAT)
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
        CALL ACCELPROFILE$NOW(ACCEL_T1)
        ACCEL_UNRAVEL_T=ACCEL_T1-ACCEL_T0
#ENDIF
        DEALLOCATE(CMAT1,CMAT2,COMAT1,COMAT2,COOMAT1,COOMAT2)
      ELSE
        CALL MPE$COMBINE('K','+',MAT)
        CALL MPE$COMBINE('K','+',OMAT)
        CALL MPE$COMBINE('K','+',OOMAT)
      END IF
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
      CALL ACCELPROFILE$ADD('PAW_1COVERLAP_BATCH_PACK' &
     &    ,INT(NB,KIND=8),INT(NBH,KIND=8),INT(NPRO,KIND=8) &
     &    ,INT(MAP%NAT,KIND=8),0.D0,0.D0,ACCEL_PACK_T)
      CALL ACCELPROFILE$ADD('PAW_1COVERLAP_BATCH_CONTRACT' &
     &    ,INT(NB,KIND=8),INT(NBH,KIND=8),INT(NPRO,KIND=8) &
     &    ,INT(MAP%NAT,KIND=8),0.D0,0.D0,ACCEL_CONTRACT_T)
      IF(TINV) THEN
        CALL ACCELPROFILE$ADD('PAW_1COVERLAP_BATCH_UNRAVEL' &
     &      ,INT(NB,KIND=8),INT(NBH,KIND=8),INT(NPRO,KIND=8) &
     &      ,INT(MAP%NAT,KIND=8),0.D0,0.D0,ACCEL_UNRAVEL_T)
      END IF
      CALL ACCELPROFILE$ADD('PAW_1COVERLAP_BATCH_TOTAL' &
     &    ,INT(NB,KIND=8),INT(NBH,KIND=8),INT(NPRO,KIND=8) &
     &    ,INT(MAP%NAT,KIND=8),0.D0,0.D0 &
     &    ,ACCEL_PACK_T+ACCEL_CONTRACT_T+ACCEL_UNRAVEL_T)
#ENDIF
      RETURN
      END SUBROUTINE WAVES_1COVERLAP_ORTHO_CUBLAS
#ENDIF
!
!      .................................................................
...
#IF DEFINED(CPPVAR_CUBLAS_FP64_EMULATION)
      USE CPPAW_CUBLAS_ACC_MODULE, ONLY: &
     &       CPPAW_CUBLAS_ACC_FP64_CHECK_ENABLED
#ENDIF
#IF DEFINED(CPPVAR_CUSOLVER_ACC)
      USE CPPAW_CUSOLVER_ACC_MODULE, ONLY: &
...
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
      ...
      REAL(8)                  :: ACCEL_ORTHOX_T2
      REAL(8)                  :: ACCEL_ORTHOX_T3
      REAL(8)                  :: ACCEL_ORTHOX_RESIDUAL_T
      REAL(8)                  :: ACCEL_ORTHOX_RESIDUAL_MATMUL_T
      REAL(8)                  :: ACCEL_ORTHOX_RESIDUAL_CHECK_T
      REAL(8)                  :: ACCEL_ORTHOX_UPDATE_T
      REAL(8)                  :: ACCEL_ORTHOX_UPDATE_TRANSFORM_T
      REAL(8)                  :: ACCEL_ORTHOX_UPDATE_SCALE_T
      REAL(8)                  :: ACCEL_ORTHOX_UPDATE_BACKTRANSFORM_T
      REAL(8)                  :: ACCEL_ORTHOX_UPDATE_APPLY_T
      REAL(8)                  :: ACCEL_ORTHOX_UPDATE_SYM_T
      REAL(8)                  :: ACCEL_ORTHOX_CUBLAS_RESIDUAL_T
      REAL(8)                  :: ACCEL_ORTHOX_CUBLAS_TRANSFORM_T
      REAL(8)                  :: ACCEL_ORTHOX_CUBLAS_BACKTRANSFORM_T
#ENDIF
#IF DEFINED(CPPVAR_CUBLAS_ACC)
...
#IF DEFINED(CPPVAR_CUSOLVER_ACC)
      LOGICAL(4)               :: ACCEL_CUSOLVER_DIAG_USED
      INTEGER(4)               :: ACCEL_CUSOLVER_INFO
#ENDIF
!     **************************************************************************
...
#IF DEFINED(CPPVAR_CUSOLVER_ACC)
      ACCEL_CUSOLVER_DIAG_USED=.FALSE.
      ACCEL_CUSOLVER_INFO=0
#ENDIF
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
      ACCEL_ORTHOX_ITER=0
      ACCEL_ORTHOX_RESIDUAL_T=0.D0
      ACCEL_ORTHOX_RESIDUAL_MATMUL_T=0.D0
      ACCEL_ORTHOX_RESIDUAL_CHECK_T=0.D0
      ACCEL_ORTHOX_UPDATE_T=0.D0
      ACCEL_ORTHOX_UPDATE_TRANSFORM_T=0.D0
      ACCEL_ORTHOX_UPDATE_SCALE_T=0.D0
      ACCEL_ORTHOX_UPDATE_BACKTRANSFORM_T=0.D0
      ACCEL_ORTHOX_UPDATE_APPLY_T=0.D0
      ACCEL_ORTHOX_UPDATE_SYM_T=0.D0
      ACCEL_ORTHOX_CUBLAS_RESIDUAL_T=0.D0
      ACCEL_ORTHOX_CUBLAS_TRANSFORM_T=0.D0
      ACCEL_ORTHOX_CUBLAS_BACKTRANSFORM_T=0.D0
#ENDIF
!
!     ==========================================================================
!     ==  CALCULATE  PSIPSI(I,J)= <PSIBAR(I)|PSIBAR(J)>-1                     ==
...
!$ACC ENTER DATA COPYIN(CHIPSI(1:NB,1:NB)) &
!$ACC& CREATE(EIG(1:NB),U(1:NB,1:NB))
        CALL CPPAW_CUSOLVER_ACC_DSYEVD_PRESENT(NB,CHIPSI,EIG,U &
     &      ,ACCEL_CUSOLVER_DIAG_USED,ACCEL_CUSOLVER_INFO)
        IF(.NOT.ACCEL_CUSOLVER_DIAG_USED) THEN
!$ACC EXIT DATA DELETE(CHIPSI(1:NB,1:NB),EIG(1:NB),U(1:NB,1:NB))
        END IF
      END IF
      IF(.NOT.ACCEL_CUSOLVER_DIAG_USED) THEN
#ENDIF
...
#IF DEFINED(CPPVAR_CUBLAS_ACC)
        IF(TRESIDENTORTHOX) THEN
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
          CALL ACCELPROFILE$NOW(ACCEL_ORTHOX_T2)
#ENDIF
          CALL CPPAW_CUBLAS_ACC_DGEMM_MATMUL_PRESENT(NB,NB,NB &
     &        ,CHICHI,LAMBDA,HAUX)
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
          CALL ACCELPROFILE$NOW(ACCEL_ORTHOX_T3)
          ACCEL_ORTHOX_CUBLAS_RESIDUAL_T &
     &        =ACCEL_ORTHOX_CUBLAS_RESIDUAL_T &
     &        +ACCEL_ORTHOX_T3-ACCEL_ORTHOX_T2
#ENDIF
...
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
          CALL ACCELPROFILE$NOW(ACCEL_ORTHOX_T3)
          ACCEL_ORTHOX_CUBLAS_TRANSFORM_T &
     &        =ACCEL_ORTHOX_CUBLAS_TRANSFORM_T &
     &        +ACCEL_ORTHOX_T3-ACCEL_ORTHOX_T2
          CALL ACCELPROFILE$NOW(ACCEL_ORTHOX_T2)
#ENDIF
...
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
          CALL ACCELPROFILE$NOW(ACCEL_ORTHOX_T3)
          ACCEL_ORTHOX_CUBLAS_BACKTRANSFORM_T &
     &        =ACCEL_ORTHOX_CUBLAS_BACKTRANSFORM_T &
     &        +ACCEL_ORTHOX_T3-ACCEL_ORTHOX_T2
          CALL ACCELPROFILE$NOW(ACCEL_ORTHOX_T2)
#ENDIF
...
#ENDIF
#IF DEFINED(CPPVAR_CUSOLVER_ACC)
      IF(ACCEL_CUSOLVER_DIAG_USED) THEN
!$ACC EXIT DATA DELETE(CHIPSI(1:NB,1:NB),EIG(1:NB),U(1:NB,1:NB))
      END IF
#ENDIF
      IF(TCONVERGED) GOTO 9000
...
9000  CONTINUE
#IF DEFINED(CPPVAR_CUBLAS_FP64_EMULATION)
      IF(CPPAW_CUBLAS_ACC_FP64_CHECK_ENABLED()) THEN
        WRITE(*,FMT='("CPPAW FP64 ORTHO RESIDUAL ",ES24.16)') DIGAM
      END IF
#ENDIF
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
     ...
     &    ,0.D0,0.D0,ACCEL_ORTHOX_RESIDUAL_T)
      CALL ACCELPROFILE$ADD('PAW_ORTHO_X_RESIDUAL_MATMUL' &
     &    ,INT(NB,KIND=8),INT(ACCEL_ORTHOX_ITER,KIND=8),0_8,0_8 &
     &    ,0.D0,0.D0,ACCEL_ORTHOX_RESIDUAL_MATMUL_T)
      CALL ACCELPROFILE$ADD('PAW_ORTHO_X_RESIDUAL_CHECK' &
     &    ,INT(NB,KIND=8),INT(ACCEL_ORTHOX_ITER,KIND=8),0_8,0_8 &
     &    ,0.D0,0.D0,ACCEL_ORTHOX_RESIDUAL_CHECK_T)
      CALL ACCELPROFILE$ADD('PAW_ORTHO_X_UPDATE' &
     &    ,INT(NB,KIND=8),INT(ACCEL_ORTHOX_ITER,KIND=8),0_8,0_8 &
     &    ,0.D0,0.D0,ACCEL_ORTHOX_UPDATE_T)
      CALL ACCELPROFILE$ADD('PAW_ORTHO_X_UPDATE_TRANSFORM' &
     &    ,INT(NB,KIND=8),INT(ACCEL_ORTHOX_ITER,KIND=8),0_8,0_8 &
     &    ,0.D0,0.D0,ACCEL_ORTHOX_UPDATE_TRANSFORM_T)
      CALL ACCELPROFILE$ADD('PAW_ORTHO_X_UPDATE_SCALE' &
     &    ,INT(NB,KIND=8),INT(ACCEL_ORTHOX_ITER,KIND=8),0_8,0_8 &
     &    ,0.D0,0.D0,ACCEL_ORTHOX_UPDATE_SCALE_T)
      CALL ACCELPROFILE$ADD('PAW_ORTHO_X_UPDATE_BACKTRANSFORM' &
     &    ,INT(NB,KIND=8),INT(ACCEL_ORTHOX_ITER,KIND=8),0_8,0_8 &
     &    ,0.D0,0.D0,ACCEL_ORTHOX_UPDATE_BACKTRANSFORM_T)
      CALL ACCELPROFILE$ADD('PAW_ORTHO_X_UPDATE_APPLY' &
     &    ,INT(NB,KIND=8),INT(ACCEL_ORTHOX_ITER,KIND=8),0_8,0_8 &
     &    ,0.D0,0.D0,ACCEL_ORTHOX_UPDATE_APPLY_T)
      CALL ACCELPROFILE$ADD('PAW_ORTHO_X_UPDATE_SYM' &
     &    ,INT(NB,KIND=8),INT(ACCEL_ORTHOX_ITER,KIND=8),0_8,0_8 &
     &    ,0.D0,0.D0,ACCEL_ORTHOX_UPDATE_SYM_T)
      IF(TRESIDENTORTHOX) THEN
        CALL ACCELPROFILE$ADD('CUBLAS_DGEMM_ORTHOX_RESIDUAL' &
     &      ,INT(NB,KIND=8),INT(ACCEL_ORTHOX_ITER,KIND=8),0_8,0_8 &
     &      ,4.D0*REAL(ACCEL_ORTHOX_ITER,KIND=8)*REAL(NB,KIND=8)**3 &
     &      ,48.D0*REAL(ACCEL_ORTHOX_ITER,KIND=8) &
     &       *REAL(NB,KIND=8)*REAL(NB,KIND=8) &
     &      ,ACCEL_ORTHOX_CUBLAS_RESIDUAL_T)
        CALL ACCELPROFILE$ADD('CUBLAS_DGEMM_ORTHOX_TRANSFORM' &
     &      ,INT(NB,KIND=8),INT(ACCEL_ORTHOX_ITER,KIND=8),0_8,0_8 &
     &      ,4.D0*REAL(ACCEL_ORTHOX_ITER,KIND=8)*REAL(NB,KIND=8)**3 &
     &      ,48.D0*REAL(ACCEL_ORTHOX_ITER,KIND=8) &
     &       *REAL(NB,KIND=8)*REAL(NB,KIND=8) &
     &      ,ACCEL_ORTHOX_CUBLAS_TRANSFORM_T)
        CALL ACCELPROFILE$ADD('CUBLAS_DGEMM_ORTHOX_BACKTRANS' &
     &      ,INT(NB,KIND=8),INT(ACCEL_ORTHOX_ITER,KIND=8),0_8,0_8 &
     &      ,4.D0*REAL(ACCEL_ORTHOX_ITER,KIND=8)*REAL(NB,KIND=8)**3 &
     &      ,48.D0*REAL(ACCEL_ORTHOX_ITER,KIND=8) &
     &       *REAL(NB,KIND=8)*REAL(NB,KIND=8) &
...
!     ******************************************************************
      TRESIDENTGRAM=.FALSE.
      TPSIRESIDENTIN=.FALSE.
#IF DEFINED(CPPVAR_CUBLAS_ACC)
      TRESIDENTGRAM=CPPAW_CUBLAS_ACC_RESIDENCY_ENABLED()
      IF(TRESIDENTGRAM) TPSIRESIDENTIN=ACC_IS_PRESENT(PSI)
#ENDIF
!
!     ==================================================================
!     ==  CALCULATE PROJECTIONS FOR THE NEW POSITIONS                 ==
...
#IF DEFINED(CPPVAR_CUBLAS_ACC)
      IF(TRESIDENTGRAM) THEN
        ...
        ACC_COPY_PSI_IN='ACC_COPY_GRAM_'//TRIM(PROFILE_ID)//'_PSI_IN'
        ACC_COPY_PSI_OUT='ACC_COPY_GRAM_'//TRIM(PROFILE_ID)//'_PSI_OUT'
        CALL CPPAW_CUBLAS_ACC_PROFILE_PRESENT_C8_3D_INOUT &
...
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
      ...
      CALL ACCELPROFILE$ADD('PAW_GRAM_SETUP' &
     &   ,INT(NGL,KIND=8),INT(NDIM,KIND=8),INT(NBH,KIND=8) &
     &   ,INT(NB,KIND=8),0.D0,0.D0,ACCEL_GRAM_T1-ACCEL_GRAM_T0)
      CALL ACCELPROFILE$NOW(ACCEL_GRAM_T0)
#ENDIF
!$ACC DATA PRESENT_OR_COPY(PSI(1:NGL,1:NDIM,1:NBH)) IF(TRESIDENTGRAM)
      CALL WAVES_PROJECTIONS(MAP,GSET,NAT,R,NGL,NDIM,NBH,NPRO,PSI &
     &                       ,PROJ,'GRAM_'//TRIM(PROFILE_ID))
      CALL MPE$COMBINE('K','+',PROJ)
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
      CALL ACCELPROFILE$NOW(ACCEL_GRAM_T1)
      CALL ACCELPROFILE$ADD('PAW_GRAM_PROJECTIONS' &
     &   ,INT(NGL,KIND=8),INT(NDIM,KIND=8),INT(NBH,KIND=8) &
     &   ,INT(NPRO,KIND=8),0.D0,0.D0,ACCEL_GRAM_T1-ACCEL_GRAM_T0)
      CALL ACCELPROFILE$NOW(ACCEL_GRAM_T0)
#ENDIF
!     
!     ==================================================================
...
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
      CALL ACCELPROFILE$NOW(ACCEL_GRAM_T1)
      CALL ACCELPROFILE$ADD('PAW_GRAM_1COVERLAP' &
     &   ,INT(NDIM,KIND=8),INT(NBH,KIND=8),INT(NB,KIND=8) &
     &   ,INT(NPRO,KIND=8),0.D0,0.D0,ACCEL_GRAM_T1-ACCEL_GRAM_T0)
      CALL ACCELPROFILE$NOW(ACCEL_GRAM_T0)
#ENDIF
      DO J=1,NB
...
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
      CALL ACCELPROFILE$NOW(ACCEL_GRAM_T1)
      CALL ACCELPROFILE$ADD('PAW_GRAM_OVERLAP_ADD' &
     &   ,INT(NB,KIND=8),INT(NB,KIND=8),0_8,0_8 &
...
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
      CALL ACCELPROFILE$NOW(ACCEL_GRAM_T1)
      CALL ACCELPROFILE$ADD('PAW_GRAM_OVERLAP_CLOSE' &
     &   ,INT(NGL,KIND=8),INT(NDIM,KIND=8),INT(NBH,KIND=8) &
     &   ,INT(NB,KIND=8),0.D0,0.D0,ACCEL_GRAM_T1-ACCEL_GRAM_T0)
      CALL ACCELPROFILE$NOW(ACCEL_GRAM_T0)
#ENDIF
...
#IF DEFINED(CPPVAR_CUBLAS_ACC)
        IF(TRESIDENTGRAM) THEN
          CALL CPPAW_CUBLAS_ACC_PROFILE_PRESENT_C8_2D &
     &        ('ACC_PRESENT_GRAM_X1','ACC_COPY_GRAM_X1_IN' &
     &        ,NBH,NBH,X1)
          CALL CPPAW_CUBLAS_ACC_PROFILE_PRESENT_C8_2D &
     &        ('ACC_PRESENT_GRAM_X2','ACC_COPY_GRAM_X2_IN' &
     &        ,NBH,NBH,X2)
        END IF
#ENDIF
#ENDIF
!       == PSI_I=PSI_I+PSI_J*X1_JI+PSIINV_J*X2_JI
!$ACC DATA CREATE(PSIINV(1:NGL,1:NDIM,1:NBH)) &
!$ACC& COPYIN(X1(1:NBH,1:NBH),X2(1:NBH,1:NBH)) IF(TRESIDENTGRAM)
!$ACC PARALLEL LOOP COLLAPSE(3) PRESENT(PSI,PSIINV) IF(TRESIDENTGRAM)
        DO IBH1=1,NBH
          DO IDIM=1,NDIM
            DO I=1,NGL
              PSIINV(I,IDIM,IBH1)=PSI(I,IDIM,IBH1)
            ENDDO
          ENDDO
        ENDDO
!$ACC END PARALLEL LOOP
...
#IF DEFINED(CPPVAR_CUBLAS_ACC)
        IF(TRESIDENTGRAM) THEN
          CALL CPPAW_CUBLAS_ACC_PROFILE_PRESENT_C8_3D &
     &        ('ACC_PRESENT_GRAM_PSIINV','ACC_COPY_GRAM_PSIINV_IN' &
     &        ,NGL,NDIM,NBH,PSIINV)
        END IF
#ENDIF
#ENDIF
...
#IF DEFINED(CPPVAR_CUBLAS_ACC)
        IF(TRESIDENTGRAM) THEN
          CALL CPPAW_CUBLAS_ACC_PROFILE_PRESENT_C8_2D &
     &        ('ACC_PRESENT_GRAM_X','ACC_COPY_GRAM_X_IN' &
     &        ,NB,NB,X)
        END IF
#ENDIF
#ENDIF
!       == PSI_I=PSI_I+PSI_J*X1_JI
!$ACC DATA CREATE(PSIINV(1:NGL,1:NDIM,1:NB)) COPYIN(X(1:NB,1:NB)) &
!$ACC& IF(TRESIDENTGRAM)
!$ACC PARALLEL LOOP COLLAPSE(3) PRESENT(PSI,PSIINV) IF(TRESIDENTGRAM)
        DO IBH1=1,NB
          DO IDIM=1,NDIM
            DO I=1,NGL
...
!$ACC END DATA
      ...
      ENDIF
#IF DEFINED(CPPVAR_CUBLAS_ACC)
      IF(TRESIDENTGRAM.AND.TPSIRESIDENTIN) THEN
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
        ACC_COPY_PSI_OUT='ACC_UPDATE_GRAM_'//TRIM(PROFILE_ID)//'_PSI_OUT'
        CALL ACCELPROFILE$ADD(ACC_COPY_PSI_OUT &
     &      ,INT(NGL,KIND=8),INT(NDIM,KIND=8),INT(NBH,KIND=8),0_8 &
     &      ,0.D0,16.D0*REAL(NGL,KIND=8)*REAL(NDIM,KIND=8) &
     &      *REAL(NBH,KIND=8),0.D0)
#ENDIF
!$ACC UPDATE SELF(PSI(1:NGL,1:NDIM,1:NBH))
      END IF
#ENDIF
!$ACC END DATA
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
      CALL ACCELPROFILE$NOW(ACCEL_GRAM_T1)
      CALL ACCELPROFILE$ADD('PAW_GRAM_TRANSFORM' &
     &   ,INT(NGL,KIND=8),INT(NDIM,KIND=8),INT(NBH,KIND=8) &
     &   ,INT(NB,KIND=8),0.D0,0.D0,ACCEL_GRAM_T1-ACCEL_GRAM_T0)
#ENDIF
! 
...
#IF DEFINED(CPPVAR_CUBLAS_ACC)
     ...
     &       ,WAVES_ACC_THIS_PSI0_CLEAR &
     &       ,WAVES_ACC_THIS_PSIM_DELETE &
     &       ,WAVES_ACC_THIS_PSIM_MARK &
     &       ,WAVES_ACC_THIS_PSIM_CLEAR
      USE CPPAW_CUBLAS_ACC_MODULE, ONLY: &
     &        CPPAW_CUBLAS_ACC_PSIM_SWITCH_RESIDENCY_ENABLED
#ENDIF
      IMPLICIT NONE
      ...
      INTEGER(4)             :: NB
#IF DEFINED(CPPVAR_CUBLAS_ACC)
      INTEGER(4)             :: NGL,NBH,NDIML
      INTEGER(4)             :: NGL0,NBH0,NDIM0
      INTEGER(4)             :: NGLM,NBHM,NDIMM
      LOGICAL(4)             :: TPSI0ACC
      LOGICAL(4)             :: TPSIMACC
      LOGICAL(4)             :: TRESIDENTPSIMSWITCH
#ENDIF
     ...
     &      .AND.(.NOT.TSTRESS)
      ...
      DO IKPT=1,NKPTL
        DO ISPIN=1,NSPIN
          CALL WAVES_SELECTWV(IKPT,ISPIN)
          CALL PLANEWAVE$SELECT(GSET%ID)
          TPSI0ACC=THIS%PSI0_ACC_RESIDENT
          TPSIMACC=THIS%PSIM_ACC_RESIDENT
          IF(TPSI0ACC.AND.(.NOT.TRESIDENTPSIMSWITCH)) THEN
            CALL WAVES_ACC_THIS_PSI0_DELETE()
            TPSI0ACC=.FALSE.
          END IF
          IF(TPSIMACC.AND.(.NOT.TRESIDENTPSIMSWITCH)) THEN
            CALL WAVES_ACC_THIS_PSIM_DELETE()
            TPSIMACC=.FALSE.
          END IF
          IF(TPSI0ACC.AND.TRESIDENTPSIMSWITCH) THEN
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
            NGL=THIS%PSI0_ACC_NGL
            NDIML=THIS%PSI0_ACC_NDIM
            NBH=THIS%PSI0_ACC_NBH
            IF(NGL.LE.0) NGL=GSET%NGL
            IF(NDIML.LE.0) NDIML=NDIM
            IF(NBH.LE.0) NBH=THIS%NBH
            CALL ACCELPROFILE$ADD('ACC_PRESENT_SWITCH_PSIM_PSI0' &
     &          ,INT(NGL,KIND=8),INT(NDIML,KIND=8),INT(NBH,KIND=8) &
     &          ,0_8,0.D0,0.D0,0.D0)
#ENDIF
          END IF
          IF(TPSIMACC.AND.TRESIDENTPSIMSWITCH) THEN
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
            ...
            IF(NDIML.LE.0) NDIML=NDIM
            IF(NBH.LE.0) NBH=THIS%NBH
            CALL ACCELPROFILE$ADD('ACC_PRESENT_SWITCH_PSI0_PSIM' &
     &          ,INT(NGL,KIND=8),INT(NDIML,KIND=8),INT(NBH,KIND=8) &
     &          ,0_8,0.D0,0.D0,0.D0)
#ENDIF
          END IF
...
#IF DEFINED(CPPVAR_CUBLAS_ACC)
          TPSI0ACC=THIS%PSI0_ACC_RESIDENT
          NGL0=THIS%PSI0_ACC_NGL
          NDIM0=THIS%PSI0_ACC_NDIM
          NBH0=THIS%PSI0_ACC_NBH
          TPSIMACC=THIS%PSIM_ACC_RESIDENT
          NGLM=THIS%PSIM_ACC_NGL
          NDIMM=THIS%PSIM_ACC_NDIM
          NBHM=THIS%PSIM_ACC_NBH
#ENDIF
          TPSI=>THIS%PSI0
          THIS%PSI0=>THIS%PSIM
          THIS%PSIM=>TPSI
#IF DEFINED(CPPVAR_CUBLAS_ACC)
          ...
          IF(TRESIDENTPSIMSWITCH.AND.TPSIMACC) THEN
            IF(NGLM.LE.0) NGLM=GSET%NGL
            IF(NDIMM.LE.0) NDIMM=NDIM
            IF(NBHM.LE.0) NBHM=THIS%NBH
            CALL WAVES_ACC_THIS_PSI0_MARK(NGLM,NDIMM,NBHM)
          END IF
          IF(TRESIDENTPSIMSWITCH.AND.TPSI0ACC) THEN
            IF(NGL0.LE.0) NGL0=GSET%NGL
            IF(NDIM0.LE.0) NDIM0=NDIM
            IF(NBH0.LE.0) NBH0=THIS%NBH
            CALL WAVES_ACC_THIS_PSIM_MARK(NGL0,NDIM0,NBH0)
          END IF
#ENDIF
!===============================================================================
