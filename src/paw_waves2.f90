...
!$ACC EXIT DATA DELETE(THIS%PSIM(1:NGL,1:NDIM,1:NBH))
      ...
      DO IKPT=1,NKPTL
        DO ISPIN=1,NSPIN
          ...
          IF(THIS%PSI0_ACC_RESIDENT) THEN
            NGL=GSET%NGL
            NBH=THIS%NBH
            IF(ASSOCIATED(THIS%PROJ)) NBH=SIZE(THIS%PROJ,2)
            ACCBYTES=16_8*INT(NGL,KIND=8)*INT(NDIM,KIND=8) &
     &              *INT(NBH,KIND=8)
