...
!     **************************************************************************
     ...
     &     ('CPPAW_CUSOLVER_ZHEGVD_MIN_N',MIN_N_ZHEGVD)
      CALL GET_ENVIRONMENT_VARIABLE('CPPAW_CUSOLVER_ACC_GRAM_CHOLESKY' &
     &                             ,VALUE,STATUS=STATUS)
      IF(STATUS.NE.0) THEN
        CALL GET_ENVIRONMENT_VARIABLE('CPPAW_CUSOLVER_GRAM_CHOLESKY' &
     &                               ,VALUE,STATUS=STATUS)
      END IF
      IF(STATUS.EQ.0) THEN
        VALUE=ADJUSTL(VALUE)
        ...
        END IF
      END IF
      MIN_N_GRAM_CHOLESKY=MAX(MIN_N,4096)
      CALL CPPAW_CUSOLVER_ACC_READ_INT_ENV &
     &   ('CPPAW_CUSOLVER_ACC_GRAM_CHOLESKY_MIN_N' &
     &   ,MIN_N_GRAM_CHOLESKY)
      CALL CPPAW_CUSOLVER_ACC_READ_INT_ENV &
     &   ('CPPAW_CUSOLVER_GRAM_CHOLESKY_MIN_N' &
     &   ,MIN_N_GRAM_CHOLESKY)
      RETURN
...
!
!     ..........................................................................
      LOGICAL(4) FUNCTION CPPAW_CUSOLVER_ACC_SHOULD_USE_GRAM_CHOLESKY(N)
      IMPLICIT NONE
      INTEGER(4),INTENT(IN) :: N
!     **************************************************************************
      CALL CPPAW_CUSOLVER_ACC_INITCONFIG
      CPPAW_CUSOLVER_ACC_SHOULD_USE_GRAM_CHOLESKY=ENABLED &
     &     .AND.GRAM_CHOLESKY_ENABLED.AND.(N.GE.MIN_N_GRAM_CHOLESKY)
      RETURN
      END FUNCTION CPPAW_CUSOLVER_ACC_SHOULD_USE_GRAM_CHOLESKY
!
!     ..........................................................................
...
!
!     ..........................................................................
      SUBROUTINE CPPAW_CUSOLVER_ACC_ENSURE_CUBLAS
      IMPLICIT NONE
      INTEGER(4)                :: ISTAT
      INTEGER(ACC_HANDLE_KIND)  :: STREAM
!     **************************************************************************
      CALL CPPAW_CUSOLVER_ACC_INITCONFIG
      IF(.NOT.ENABLED) RETURN
      IF(.NOT.CUBLAS_HANDLE_READY) THEN
        ISTAT=CUBLASCREATE(CUBLAS_HANDLE)
        IF(ISTAT.NE.0) THEN
          CALL ERROR$MSG('CUBLASCREATE FAILED FOR CUSOLVER HELPER')
          CALL ERROR$I4VAL('STATUS',ISTAT)
          CALL ERROR$STOP('CPPAW_CUSOLVER_ACC_ENSURE_CUBLAS')
        END IF
        CUBLAS_HANDLE_READY=.TRUE.
      END IF
      STREAM=ACC_GET_CUDA_STREAM(ACC_ASYNC_SYNC)
      ISTAT=CUBLASSETSTREAM(CUBLAS_HANDLE,STREAM)
      IF(ISTAT.NE.0) THEN
        CALL ERROR$MSG('CUBLASSETSTREAM FAILED FOR CUSOLVER HELPER')
        CALL ERROR$I4VAL('STATUS',ISTAT)
        CALL ERROR$STOP('CPPAW_CUSOLVER_ACC_ENSURE_CUBLAS')
      END IF
      RETURN
      END SUBROUTINE CPPAW_CUSOLVER_ACC_ENSURE_CUBLAS
!
!     ..........................................................................
...
!
!     ..........................................................................
      SUBROUTINE CPPAW_CUSOLVER_ACC_DSYEVD_PRESENT(N,H,E,U,USED,INFO)
      IMPLICIT NONE
      INTEGER(4),INTENT(IN)    :: N
      REAL(8)   ,INTENT(IN)    :: H(N,N)
...
!     **************************************************************************
      ...
      IF(.NOT.CPPAW_CUSOLVER_ACC_SHOULD_USE_DSYEVD(N)) RETURN
      CALL CPPAW_CUSOLVER_ACC_ENSURE
!$ACC PARALLEL LOOP COLLAPSE(2) PRESENT(H,U)
      DO J=1,N
        DO I=1,N
          U(I,J)=0.5D0*(H(I,J)+H(J,I))
        ENDDO
      ENDDO
...
!$ACC END HOST_DATA
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
      CALL CPPAW_CUSOLVER_ACC_PROFILE_TIME &
     &       ('ACC_SETUP_DSYEVD_PRESENT' &
     &       ,N,MAX(LWORK,0),0,0,ACCEL_T0)
#ENDIF
      IF(ISTAT.NE.0.OR.LWORK.LE.0) THEN
        INFO=ISTAT
...
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
      CALL ACCELPROFILE$NOW(ACCEL_T0)
#ENDIF
!$ACC DATA CREATE(WORK(1:LWORK)) COPY(DEVINFO)
!$ACC HOST_DATA USE_DEVICE(U,E,WORK,DEVINFO)
      ISTAT=CUSOLVERDNDSYEVD(HANDLE,CUSOLVER_EIG_MODE_VECTOR &
     &        ,CUBLAS_FILL_MODE_UPPER,N,U,N,E,WORK,LWORK,DEVINFO)
!$ACC END HOST_DATA
...
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
      ...
      CALL ACCELPROFILE$ADD('CUSOLVER_DSYEVD_PRESENT' &
     &   ,INT(N,KIND=8),INT(LWORK,KIND=8),0_8,0_8 &
...
#ENDIF
      ...
      USED=.TRUE.
      RETURN
      END SUBROUTINE CPPAW_CUSOLVER_ACC_DSYEVD_PRESENT
!
!     ..........................................................................
...
!
!     ..........................................................................
      SUBROUTINE CPPAW_CUSOLVER_ACC_ZGRAM_CHOLESKY_COPY &
     &                                  (N,OVERLAP,X,USED,INFO)
      IMPLICIT NONE
      INTEGER(4),INTENT(IN)    :: N
      COMPLEX(8),INTENT(IN)    :: OVERLAP(N,N)
      COMPLEX(8),INTENT(OUT)   :: X(N,N)
      LOGICAL(4),INTENT(OUT)   :: USED
      INTEGER(4),INTENT(OUT)   :: INFO
      COMPLEX(8),ALLOCATABLE   :: A(:,:)
      COMPLEX(8),ALLOCATABLE   :: WORK(:)
...
!     **************************************************************************
      ...
      INFO=0
      IF(N.LE.0) RETURN
      IF(.NOT.CPPAW_CUSOLVER_ACC_SHOULD_USE_GRAM_CHOLESKY(N)) RETURN
      ALLOCATE(A(N,N))
      A=OVERLAP
      CALL CPPAW_CUSOLVER_ACC_ENSURE
      LWORK=0
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
      CALL ACCELPROFILE$NOW(ACCEL_T0)
#ENDIF
!$ACC DATA COPYIN(A(1:N,1:N))
!$ACC HOST_DATA USE_DEVICE(A)
      ISTAT=CUSOLVERDNZPOTRF_BUFFERSIZE(HANDLE,CUBLAS_FILL_MODE_UPPER &
     &                                  ,N,A,N,LWORK)
!$ACC END HOST_DATA
!$ACC END DATA
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
      CALL CPPAW_CUSOLVER_ACC_PROFILE_TIME &
     &       ('ACC_SETUP_CUSOLVER_ZPOTRF_GRAM',N,MAX(LWORK,0),0,0 &
     &       ,ACCEL_T0)
#ENDIF
      IF(ISTAT.NE.0.OR.LWORK.LE.0) THEN
        INFO=ISTAT
...
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
      CALL ACCELPROFILE$NOW(ACCEL_T0)
#ENDIF
!$ACC DATA COPYIN(A(1:N,1:N)) COPY(X(1:N,1:N),DEVINFO) &
!$ACC& CREATE(WORK(1:LWORK))
!$ACC HOST_DATA USE_DEVICE(A,WORK,DEVINFO)
      ISTAT=CUSOLVERDNZPOTRF(HANDLE,CUBLAS_FILL_MODE_UPPER,N,A,N &
     &                       ,WORK,LWORK,DEVINFO)
!$ACC END HOST_DATA
      ISTAT=MAX(ISTAT,CUDADEVICESYNCHRONIZE())
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
      CALL ACCELPROFILE$NOW(ACCEL_T1)
      CALL ACCELPROFILE$ADD('CUSOLVER_ZPOTRF_GRAM' &
     &   ,INT(N,KIND=8),INT(LWORK,KIND=8),0_8,0_8 &
     &   ,0.D0,0.D0,ACCEL_T1-ACCEL_T0)
#ENDIF
!$ACC UPDATE SELF(DEVINFO)
      IF(ISTAT.EQ.0.AND.DEVINFO.EQ.0) THEN
        CALL CPPAW_CUSOLVER_ACC_ENSURE_CUBLAS
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
        CALL ACCELPROFILE$NOW(ACCEL_T0)
#ENDIF
!$ACC HOST_DATA USE_DEVICE(A,X)
        ISTAT=CUBLASZTRSM(CUBLAS_HANDLE,CUBLAS_SIDE_LEFT &
...
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
        CALL ACCELPROFILE$NOW(ACCEL_T1)
        CALL ACCELPROFILE$ADD('CUBLAS_ZTRSM_GRAM' &
...
!$ACC END DATA
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
      CALL CPPAW_CUSOLVER_ACC_PROFILE_BYTES &
     &     ('ACC_COPY_CUSOLVER_GRAM_CHOLESKY',N,N,0,0 &
     &     ,16.D0*(3.D0*REAL(N,KIND=8)*REAL(N,KIND=8)))
#ENDIF
      DEALLOCATE(WORK)
      DEALLOCATE(A)
      IF(ISTAT.NE.0) THEN
      ...
      END IF
      DO I=1,N
        X(I,I)=X(I,I)-(1.D0,0.D0)
      ENDDO
      USED=.TRUE.
      RETURN
      END SUBROUTINE CPPAW_CUSOLVER_ACC_ZGRAM_CHOLESKY_COPY
!
