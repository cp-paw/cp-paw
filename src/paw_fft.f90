...
LOGICAL(4)                     :: SERIAL3D_ACC_INPUT_PRESENT_HINT=.FALSE.
LOGICAL(4)                     :: SERIAL3D_ACC_OUTPUT_PRESENT_HINT=.FALSE.
...
!     ******************************************************************
     ...
     &     ,INPUT_PRESENT_HINT => SERIAL3D_ACC_INPUT_PRESENT_HINT &
     &     ,OUTPUT_PRESENT_HINT => SERIAL3D_ACC_OUTPUT_PRESENT_HINT
      IMPLICIT NONE
      CHARACTER(*),INTENT(IN)   :: ID
      INTEGER(4),INTENT(IN)     :: NGL
      INTEGER(4),INTENT(IN)     :: NRL
      ...
      LOGICAL(4)                :: WORK_PRESENT
      LOGICAL(4)                :: INPUT_PRESENT
      LOGICAL(4)                :: OUTPUT_PRESENT
      LOGICAL(4)                :: DATA_REGION_NEEDED
      LOGICAL(4)                :: INPUT_HINT
      LOGICAL(4)                :: OUTPUT_HINT
      CHARACTER(128)            :: ENVVAL_CACHE
      INTEGER(4)                :: ENVSTATUS_CACHE
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
      REAL(8)                   :: ACCEL_BYTES
      REAL(8)                   :: ACCEL_INPUT_BYTES
      REAL(8)                   :: ACCEL_MAP_BYTES
      REAL(8)                   :: ACCEL_OUTPUT_BYTES
      REAL(8)                   :: ACCEL_T0
      REAL(8)                   :: ACCEL_T1
#ENDIF
!     ******************************************************************
      ...
      INPUT_HINT=INPUT_PRESENT_HINT
      OUTPUT_HINT=OUTPUT_PRESENT_HINT
      INPUT_PRESENT_HINT=.FALSE.
      OUTPUT_PRESENT_HINT=.FALSE.
     ...
     &                               ,ENVVAL_CACHE,STATUS=ENVSTATUS_CACHE)
        IF(ENVSTATUS_CACHE.EQ.0) THEN
          ...
          IF(LEN_TRIM(ENVVAL_CACHE).GT.0) THEN
            SELECT CASE(ENVVAL_CACHE(1:MIN(LEN_TRIM(ENVVAL_CACHE) &
     &                                    ,LEN(ENVVAL_CACHE))))
            CASE('1','yes','YES','true','TRUE','on','ON')
              TCACHE=.TRUE.
            CASE DEFAULT
      ...
      IF(TCACHE) THEN
        IF(ALLOCATED(WORK)) THEN
          IF(CACHE_NR1.NE.NR1.OR.CACHE_NR2.NE.NR2 &
     &       .OR.CACHE_NR3.NE.NR3) THEN
!$ACC EXIT DATA DELETE(WORK)
        ...
        IF(.NOT.ALLOCATED(WORK)) THEN
          ...
          CACHE_NR2=NR2
          CACHE_NR3=NR3
!$ACC ENTER DATA CREATE(WORK(1:NR1,1:NR2,1:NR3))
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
          CALL ACCELPROFILE$ADD('ACC_CREATE_SERIAL3D_ACC_WORK' &
     &       ,INT(NR1,KIND=8),INT(NR2,KIND=8),INT(NR3,KIND=8) &
     &       ,INT(NGL,KIND=8),0.D0,0.D0,0.D0)
#ENDIF
        END IF
        IF(.NOT.(ACC_IS_PRESENT(IGTOSTRIPE) &
     &       .AND.ACC_IS_PRESENT(ISTRIPETOYZ))) THEN
!$ACC ENTER DATA COPYIN(IGTOSTRIPE(1:NGL),ISTRIPETOYZ(1:NSTRIPEL))
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
          CALL ACCELPROFILE$ADD('ACC_COPY_SERIAL3D_ACC_MAP_CACHE' &
     &       ,INT(NR1,KIND=8),INT(NR2,KIND=8),INT(NR3,KIND=8) &
     &       ,INT(NGL,KIND=8),0.D0 &
     &       ,4.D0*REAL(NGL,KIND=8) &
     &       +4.D0*REAL(NSTRIPEL,KIND=8),0.D0)
#ENDIF
        END IF
      ELSE
        ALLOCATE(WORK(NR1,NR2,NR3))
      END IF
      MAP_PRESENT=ACC_IS_PRESENT(IGTOSTRIPE) &
     &     .AND.ACC_IS_PRESENT(ISTRIPETOYZ)
      WORK_PRESENT=ACC_IS_PRESENT(WORK)
      INPUT_PRESENT=.FALSE.
      OUTPUT_PRESENT=.FALSE.
      DATA_REGION_NEEDED=.TRUE.
      IF(ID.EQ.'GTOR') THEN
        INPUT_PRESENT=ACC_IS_PRESENT(FOFG).OR.INPUT_HINT
        OUTPUT_PRESENT=ACC_IS_PRESENT(FOFR).OR.OUTPUT_HINT
        DATA_REGION_NEEDED=.NOT.(INPUT_PRESENT.AND.OUTPUT_PRESENT &
     &                      .AND.MAP_PRESENT.AND.WORK_PRESENT)
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
        IF(.NOT.DATA_REGION_NEEDED) THEN
          CALL ACCELPROFILE$ADD('ACC_PRESENT_SERIAL3D_DATA' &
     &       ,INT(NR1,KIND=8),INT(NR2,KIND=8),INT(NR3,KIND=8) &
     &       ,INT(NGL,KIND=8),0.D0,0.D0,0.D0)
          CALL ACCELPROFILE$ADD('ACC_PRESENT_SERIAL3D_GTOR_DATA' &
     &       ,INT(NR1,KIND=8),INT(NR2,KIND=8),INT(NR3,KIND=8) &
     &       ,INT(NGL,KIND=8),0.D0,0.D0,0.D0)
        END IF
#ENDIF
!$ACC DATA PRESENT_OR_COPYIN(FOFG(1:NGL)) &
!$ACC&          PRESENT_OR_COPYIN(IGTOSTRIPE(1:NGL) &
!$ACC&                            ,ISTRIPETOYZ(1:NSTRIPEL)) &
!$ACC&          PRESENT_OR_COPYOUT(FOFR(1:NRL)) &
!$ACC&          PRESENT_OR_CREATE(WORK(1:NR1,1:NR2,1:NR3)) &
!$ACC&          IF(DATA_REGION_NEEDED)
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
        CALL ACCELPROFILE$NOW(ACCEL_T0)
#ENDIF
...
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
        ...
        CALL ACCELPROFILE$ADD('ACC_SERIAL3D_GTOR_ZERO' &
     &      ,INT(NR1,KIND=8),INT(NR2,KIND=8),INT(NR3,KIND=8) &
     &      ,INT(NGL,KIND=8),0.D0,0.D0,ACCEL_T1-ACCEL_T0)
        CALL ACCELPROFILE$NOW(ACCEL_T0)
#ENDIF
...
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
        CALL ACCELPROFILE$NOW(ACCEL_T1)
        CALL ACCELPROFILE$ADD('ACC_SERIAL3D_GTOR_SCATTER' &
     &      ,INT(NR1,KIND=8),INT(NR2,KIND=8),INT(NR3,KIND=8) &
     &      ,INT(NGL,KIND=8),0.D0,0.D0,ACCEL_T1-ACCEL_T0)
#ENDIF
...
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
        CALL ACCELPROFILE$NOW(ACCEL_T1)
        CALL ACCELPROFILE$ADD('ACC_SERIAL3D_GTOR_GATHER' &
     &      ,INT(NR1,KIND=8),INT(NR2,KIND=8),INT(NR3,KIND=8) &
     &      ,INT(NGL,KIND=8),0.D0,0.D0,ACCEL_T1-ACCEL_T0)
#ENDIF
!$ACC END DATA
      ELSE IF(ID.EQ.'RTOG') THEN
        INPUT_PRESENT=ACC_IS_PRESENT(FOFR).OR.INPUT_HINT
        OUTPUT_PRESENT=ACC_IS_PRESENT(FOFG).OR.OUTPUT_HINT
        DATA_REGION_NEEDED=.NOT.(INPUT_PRESENT.AND.OUTPUT_PRESENT &
     &                      .AND.MAP_PRESENT.AND.WORK_PRESENT)
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
        IF(.NOT.DATA_REGION_NEEDED) THEN
     ...
     &       ,INT(NR1,KIND=8),INT(NR2,KIND=8),INT(NR3,KIND=8) &
     &       ,INT(NGL,KIND=8),0.D0,0.D0,0.D0)
          CALL ACCELPROFILE$ADD('ACC_PRESENT_SERIAL3D_RTOG_DATA' &
     &       ,INT(NR1,KIND=8),INT(NR2,KIND=8),INT(NR3,KIND=8) &
     &       ,INT(NGL,KIND=8),0.D0,0.D0,0.D0)
        END IF
#ENDIF
!$ACC DATA PRESENT_OR_COPYIN(FOFR(1:NRL)) &
!$ACC&          PRESENT_OR_COPYIN(IGTOSTRIPE(1:NGL) &
!$ACC&                            ,ISTRIPETOYZ(1:NSTRIPEL)) &
!$ACC&          PRESENT_OR_COPYOUT(FOFG(1:NGL)) &
!$ACC&          PRESENT_OR_CREATE(WORK(1:NR1,1:NR2,1:NR3)) &
!$ACC&          IF(DATA_REGION_NEEDED)
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
        CALL ACCELPROFILE$NOW(ACCEL_T0)
...
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
        CALL ACCELPROFILE$NOW(ACCEL_T1)
        CALL ACCELPROFILE$ADD('ACC_SERIAL3D_RTOG_LOAD' &
     &      ,INT(NR1,KIND=8),INT(NR2,KIND=8),INT(NR3,KIND=8) &
     &      ,INT(NGL,KIND=8),0.D0,0.D0,ACCEL_T1-ACCEL_T0)
#ENDIF
...
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
        CALL ACCELPROFILE$NOW(ACCEL_T1)
        CALL ACCELPROFILE$ADD('ACC_SERIAL3D_RTOG_GATHER' &
     &      ,INT(NR1,KIND=8),INT(NR2,KIND=8),INT(NR3,KIND=8) &
     &      ,INT(NGL,KIND=8),0.D0,0.D0,ACCEL_T1-ACCEL_T0)
#ENDIF
!$ACC END DATA
      END IF
      IF(.NOT.TCACHE) DEALLOCATE(WORK)
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
      IF(ID.EQ.'GTOR') THEN
        ACCEL_INPUT_BYTES=16.D0*REAL(NGL,KIND=8)
        ACCEL_OUTPUT_BYTES=16.D0*REAL(NRL,KIND=8)
      ELSE
        ACCEL_INPUT_BYTES=16.D0*REAL(NRL,KIND=8)
        ACCEL_OUTPUT_BYTES=16.D0*REAL(NGL,KIND=8)
      END IF
      ACCEL_MAP_BYTES=4.D0*REAL(NGL,KIND=8) &
     &               +4.D0*REAL(NSTRIPEL,KIND=8)
      IF(INPUT_PRESENT) THEN
        ACCEL_INPUT_BYTES=0.D0
      END IF
      IF(OUTPUT_PRESENT) THEN
        ACCEL_OUTPUT_BYTES=0.D0
      END IF
      IF(MAP_PRESENT) THEN
        ACCEL_MAP_BYTES=0.D0
      END IF
      ACCEL_BYTES=ACCEL_INPUT_BYTES+ACCEL_OUTPUT_BYTES+ACCEL_MAP_BYTES
      CALL ACCELPROFILE$ADD('ACC_COPY_SERIAL3D_ACC_INPUT' &
     & ,INT(NR1,KIND=8),INT(NR2,KIND=8),INT(NR3,KIND=8) &
     & ,INT(NGL,KIND=8),0.D0,ACCEL_INPUT_BYTES,0.D0)
      CALL ACCELPROFILE$ADD('ACC_COPY_SERIAL3D_ACC_OUTPUT' &
     & ,INT(NR1,KIND=8),INT(NR2,KIND=8),INT(NR3,KIND=8) &
     & ,INT(NGL,KIND=8),0.D0,ACCEL_OUTPUT_BYTES,0.D0)
      CALL ACCELPROFILE$ADD('ACC_COPY_SERIAL3D_ACC_MAP_META' &
     & ,INT(NR1,KIND=8),INT(NR2,KIND=8),INT(NR3,KIND=8) &
     & ,INT(NGL,KIND=8),0.D0,ACCEL_MAP_BYTES,0.D0)
...
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
      ...
      CALL ACCELPROFILE$ADD('PW_FFT_GTOR_TOTAL' &
     & ,INT(NR1,KIND=8),INT(NR2,KIND=8),INT(NR3,KIND=8),INT(NGL,KIND=8) &
     & ,5.D0*ACCEL_GRID*LOG(MAX(ACCEL_GRID,1.D0))/LOG(2.D0) &
     & ,32.D0*ACCEL_GRID,ACCEL_T1-ACCEL_T0)
#ENDIF
      RETURN
      END
!
...
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
      CALL ACCELPROFILE$NOW(ACCEL_T1)
      ACCEL_GRID=REAL(NR1,KIND=8)*REAL(NR2,KIND=8)*REAL(NR3,KIND=8)
      CALL ACCELPROFILE$ADD('PW_FFT_RTOG_TOTAL' &
     & ,INT(NR1,KIND=8),INT(NR2,KIND=8),INT(NR3,KIND=8),INT(NGL,KIND=8) &
     & ,5.D0*ACCEL_GRID*LOG(MAX(ACCEL_GRID,1.D0))/LOG(2.D0) &
     & ,32.D0*ACCEL_GRID,ACCEL_T1-ACCEL_T0)
#ENDIF
