...
!     ==========================================================================
      CALL SIMPLELMTOAUGMENTATION$SETI4('NAT',NAT_)
      CALL SIMPLELMTOAUGMENTATION$SETI4('NDIMD',NDIMD_)
      CALL SIMPLELMTOAUGMENTATION$SETI4('LMNXX',LMNXX_)
!     == SETC8A('DENMAT'...) ALLOCATES INTERNAL ARRAYS AN AND SETS TACTIVE=TRUE
      CALL SIMPLELMTOAUGMENTATION$SETC8A('DENMAT',LMNXX_*LMNXX_*NDIMD_*NAT_,DENMAT_)
!
!     ==========================================================================
!     ==  SELECT CHOICES                                                      ==
!     ==========================================================================
...
!
!     ==========================================================================
!     ==  COLLECT ONE-CENTER HAMILTONIAN FROM SIMPLELMTOAUGMENTATION OBJECT   ==
!     ==========================================================================
      CALL SIMPLELMTOAUGMENTATION$GETC8A('DH',LMNXX_*LMNXX_*NDIMD_*NAT_,DH_)
!
!     == DEALLOCATE INTERNAL ARRAYS AND SET OBJECT INACTIVE=====================
      CALL SIMPLELMTOAUGMENTATION$CLEAN()
!!$DO  IAT=1,NAT_
!!$  WRITE(*,FMT='("HONSITE 1: ",20F10.5)')DH_(:,:,:,IAT)
!!$ENDDO
!!$STOP 'FORCED'
!
...
!     == -- DATH (VIA MODULE SIMPLELMTOAUGMENTATIUON)                         ==
!     ==                                                                      ==
...
!       ========================================================================
        IF(HYBRIDSETTING(ISP)%TCV) THEN
          CALL SIMPLELMTO_CVX_ACTONPHI_NEW(IAT,LMNX,NDIMD,DON,EX,HON)
...
!        CALL SIMPLELMTOAUGMENTATION$MPECOMBINEPOT()
        DO NN=1,NND
...
!
!     ...1.........2.........3.........4.........5.........6.........7.........8
      SUBROUTINE SIMPLELMTO_CVX_ACTONPHI(IAT,LHFWEIGHT,ETOT)
!     **************************************************************************
!     **  CORE VALENCE EXCHANGE ENERGY ACTING ON PARTIAL WAVES                **
...
!     **************************************************************************
      ...
      CALL SIMPLELMTOAUGMENTATION$SETI4('IAT',IAT)
!
!     ==========================================================================
!     == COLLECT DATA                                                         ==
!     ==========================================================================
      ...
      CALL SETUP$UNSELECT()
      LMNX=SUM(2*LOX+1)
      CALL SIMPLELMTOAUGMENTATION$GETI4('NDIMD',NDIMD)
      ALLOCATE(DENMAT(LMNX,LMNX,NDIMD))
      CALL SIMPLELMTOAUGMENTATION$GETC8A('DENMAT',LMNX*LMNX*NDIMD,DENMAT)
!
!     ==========================================================================
!     == SCALE ENERGY TERM                                                    ==
!     ==========================================================================
...
!     ==========================================================================
      ...
      DO LN1=1,LNX
        ...
        LMN1=LMN1+2*L1+1
      ENDDO        
      CALL SIMPLELMTOAUGMENTATION$ADDC8A('DH',LMNX*LMNX*NDIMD,DH)
!
!     ==========================================================================
!     == CLOSE DOWN                                                           ==
!     ==========================================================================
      ...
      DEALLOCATE(CVXMAT)
      DEALLOCATE(LOX)
      CALL SIMPLELMTOAUGMENTATION$SETI4('IAT',0)
                              CALL TRACE$POP()
      RETURN
      END
!
...
!
!........1.........2.........3.........4.........5.........6.........7.........8
MODULE SIMPLELMTOAUGMENTATION_MODULE
!*******************************************************************************
...
!**                                                                           **
!**    SIMPLELMTOAUGMENTATION$SETI4('NAT',NAT)
!**    SIMPLELMTOAUGMENTATION$SETI4('NDIMD',NDIMD)
!**    SIMPLELMTOAUGMENTATION$SETI4('LMNXX',LMNXX)
!**    SIMPLELMTOAUGMENTATION$SETC8A('DENMAT',LMNXX_*LMNXX_*NDIMD_*NAT_,DENMAT_)
!**
!**    SIMPLELMTOAUGMENTATION$SETI4('IAT',IAT)
!**    SIMPLELMTOAUGMENTATION$SETI4('IAT',0) !UNSET ATOM INDEX
!**
!**    SIMPLELMTOAUGMENTATION$GETC8A('DENMAT',LMNX*LMNX*NDIMD,DENMAT)
!**    SIMPLELMTOAUGMENTATION$ADDC8A('DH',LMNX*LMNX*NDIMD,DH)
!**
!**    SIMPLELMTOAUGMENTATION$GETRHO(GID,NR,LMRX,NDIMD,RHO_ALL)
!**    SIMPLELMTOAUGMENTATION$ADDPOT(GID,NR,LMRX,NDIMD,-POT_ALL*HFSCALE)
!**
!**    SIMPLELMTOAUGMENTATION$MPECOMBINEPOT()
!**    SIMPLELMTOAUGMENTATION$GETC8A('DH',LMNXX_*LMNXX_*NDIMD_*NAT_,DH_)
!**    SIMPLELMTOAUGMENTATION$CLEAN()
!**
!*******************************************************************************
IMPLICIT NONE
LOGICAL(4),SAVE        :: TINI=.FALSE.
...
END MODULE SIMPLELMTOAUGMENTATION_MODULE
!
!     ...1.........2.........3.........4.........5.........6.........7.........8
      SUBROUTINE SIMPLELMTOAUGMENTATION_INITIALIZE()
!     **************************************************************************
!     **************************************************************************
      USE SIMPLELMTOAUGMENTATION_MODULE, ONLY : TINI,NAT,NDIMD,LMNXX
      IMPLICIT NONE
!     **************************************************************************
      IF(TINI) RETURN
      CALL ATOMLIST$NATOM(NAT)
      IF(NDIMD.EQ.0) THEN
        ...
        CALL ERROR$STOP('SIMPLELMTOAUGMENTATION_INITIALIZE')
      END IF
      IF(LMNXX.EQ.0) THEN
...
!
!     ...1.........2.........3.........4.........5.........6.........7.........8
      SUBROUTINE SIMPLELMTOAUGMENTATION$CLEAN()
!     **************************************************************************
!     ** FREE MEMORY. NEEDS TO BE ACTIVATED BY SETTING A DENSITY MATRIX
!     **************************************************************************
      USE SIMPLELMTOAUGMENTATION_MODULE, ONLY : TINI,TACTIVE,IAT,DENMAT,DATH
      IMPLICIT NONE
!     **************************************************************************
      IF(.NOT.TINI) CALL SIMPLELMTOAUGMENTATION_INITIALIZE()
...
!
!     ...1.........2.........3.........4.........5.........6.........7.........8
      SUBROUTINE SIMPLELMTOAUGMENTATION$GETI4(ID,VAL)
!     **************************************************************************
!     **************************************************************************
      USE SIMPLELMTOAUGMENTATION_MODULE, ONLY : TINI,NAT,LMNXX,NDIMD,IAT
      IMPLICIT NONE
      CHARACTER(*),INTENT(IN) :: ID
      INTEGER(4)  ,INTENT(OUT):: VAL
!     **************************************************************************
      ...
      ELSE
        CALL ERROR$MSG('ID NOT RECOGNIZED')
        CALL ERROR$CHVAL('ID',ID)
        CALL ERROR$STOP('SIMPLELMTOAUGMENTATION$SETI4')
      END IF
      RETURN
      END
!
!     ...1.........2.........3.........4.........5.........6.........7.........8
      SUBROUTINE SIMPLELMTOAUGMENTATION$SETI4(ID,VAL)
!     **************************************************************************
!     **************************************************************************
      USE SIMPLELMTOAUGMENTATION_MODULE, ONLY : TINI,NAT,LMNXX,NDIMD,IAT
      IMPLICIT NONE
...
!
!     ...1.........2.........3.........4.........5.........6.........7.........8
      SUBROUTINE SIMPLELMTOAUGMENTATION$SETC8A(ID,LEN,VAL)
!     **************************************************************************
!     **************************************************************************
      USE SIMPLELMTOAUGMENTATION_MODULE, ONLY : TINI,TACTIVE &
...
!     **************************************************************************
      ...
      IF(ID.EQ.'DENMAT') THEN
        IF(LMNXX*LMNXX*NDIMD*NAT.NE.LEN) THEN
          ...
          CALL ERROR$CHVAL('ID',ID)
          CALL ERROR$STOP('SIMPLELMTOAUGMENTATION$SETC8A')
        END IF
        IF(TACTIVE) THEN
          CALL ERROR$MSG('DENSITY MATRIX MUST NOT BE CHANGED IN ACTIVE STATE')
          CALL ERROR$MSG('CALL SIMPLELMTOAUGMENTATION$CLEAN TO RESET OBJECT')
          CALL ERROR$CHVAL('ID',ID)
          CALL ERROR$STOP('SIMPLELMTOAUGMENTATION$SETC8A')
        END IF 
...
!
!     ...1.........2.........3.........4.........5.........6.........7.........8
      SUBROUTINE SIMPLELMTOAUGMENTATION$ADDC8A(ID,LEN,VAL)
!     **************************************************************************
!     **************************************************************************
      USE SIMPLELMTOAUGMENTATION_MODULE, ONLY : TINI,TACTIVE,IAT &
     &                                   ,NAT,LMNXX,NDIMD,DENMAT,DATH
      IMPLICIT NONE
      CHARACTER(*),INTENT(IN) :: ID
      INTEGER(4)  ,INTENT(IN) :: LEN
...
!     **************************************************************************
      ...
      IF(ID.EQ.'DH') THEN
        IF(.NOT.TACTIVE) THEN
          CALL ERROR$MSG('SIMPLELMTOAUGMENTATION OBJECT IS NOT ACTIVE')
          CALL ERROR$STOP('SIMPLELMTOAUGMENTATION$ADDC8A')
        END IF 
        IF(IAT.EQ.0) THEN
...
!
!     ...1.........2.........3.........4.........5.........6.........7.........8
      SUBROUTINE SIMPLELMTOAUGMENTATION$GETC8A(ID,LEN,VAL)
!     **************************************************************************
!     **************************************************************************
      USE SIMPLELMTOAUGMENTATION_MODULE, ONLY : TINI,TACTIVE,IAT &
...
!     **************************************************************************
      ...
      IF(ID.EQ.'DH') THEN
        IF(.NOT.TACTIVE) THEN
          CALL ERROR$MSG('SIMPLELMTOAUGMENTATION OBJECT IS NOT ACTIVE')
          CALL ERROR$CHVAL('ID',ID)
          CALL ERROR$STOP('SIMPLELMTOAUGMENTATION$GETC8A')
        END IF 
        IF(LMNXX*LMNXX*NDIMD*NAT.NE.LEN) THEN
          CALL ERROR$MSG('INCONSISTENT ARRAY SIZE')
          CALL ERROR$MSG('SET VARIABLE "ISP" FIRST')
...
!
!     ...1.........2.........3.........4.........5.........6.........7.........8
      SUBROUTINE SIMPLELMTOAUGMENTATION$GETRHO(GID,NR,LMRX,NDIMD_,RHO)
!     **************************************************************************
...
!     **************************************************************************
      ...
      IF(IAT.EQ.0) THEN
        ...
        CALL ERROR$STOP('SIMPLELMTOAUGMENTATION$GETRHO')
      END IF
      IF(.NOT.TACTIVE) THEN
        CALL ERROR$MSG('SIMPLELMTOAUGMENTATION OBJECT IS NOT ACTIVE')
        CALL ERROR$STOP('SIMPLELMTOAUGMENTATION$GETRHO')
...
!     **************************************************************************
      ...
      IF(.NOT.TACTIVE) THEN
        CALL ERROR$MSG('SIMPLELMTOAUGMENTATION OBJECT IS NOT ACTIVE')
        CALL ERROR$STOP('SIMPLELMTOAUGMENTATION$MPECOMBINEPOT')
      END IF 
...
!
!     ...1.........2.........3.........4.........5.........6.........7.........8
      SUBROUTINE SIMPLELMTOAUGMENTATION$ADDPOT(GID,NR,LMRX,NDIMD_,POT)
!     **************************************************************************
!     ** CALCULATES MATRIX ELEMENT OF THE POTENTIAL WITH PARTIAL WAVES        **
!     ** AND ADDS THEM TO THE MODULE-ARRAY DATH                               **
!     **************************************************************************
      USE SIMPLELMTOAUGMENTATION_MODULE, ONLY : TINI,TACTIVE,NDIMD,DATH,IAT
      IMPLICIT NONE
      INTEGER(4),INTENT(IN)  :: GID
      INTEGER(4),INTENT(IN)  :: NR
      INTEGER(4),INTENT(IN)  :: LMRX
...
!     **************************************************************************
      ...
      IF(IAT.EQ.0) THEN
        CALL ERROR$MSG('ATOM INDEX NOT SET')
        CALL ERROR$STOP('SIMPLELMTOAUGMENTATION$ADDPOT')
      END IF
      IF(.NOT.TACTIVE) THEN
        CALL ERROR$MSG('SIMPLELMTOAUGMENTATION OBJECT IS NOT ACTIVE')
        CALL ERROR$STOP('SIMPLELMTOAUGMENTATION$ADDPOT')
...
!      CALL SETUP$GETR8A('AECORE',NR,AECORE)
      ...
      CALL SIMPLELMTO_EXPECT(GID,NR,NDIMD,LNX,LOX,LMNX,LMRX,POT,AEPHI,DATH1)
!PRINT*,'DATH FROM ADDPOT',IAT,DATH1
