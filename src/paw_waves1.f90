...
TYPE WVSET_TYPE  !==============================================================
  ...
  LOGICAL(4)         :: PSI0_ACC_RESIDENT=.FALSE.
...
LOGICAL(4)  :: HPSI_RTOG_ACC_ACTIVE=.FALSE.
...
#IF DEFINED(CPPVAR_CUBLAS_ACC)
     ...
     &      ,CPPAW_CUBLAS_ACC_HPSI_PROP_RESIDENCY_ENABLED &
...
#IF DEFINED(CPPVAR_CUBLAS_ACC)
      ...
      LOGICAL(4)             :: TKEEPHPSIPROP
      LOGICAL(4)             :: TKEEPPSI0ORTHO
#ENDIF
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
...
#IF DEFINED(CPPVAR_CUBLAS_ACC)
     ...
     &                   .AND.THIS%HPSI_ACC_RESIDENT
          TKEEPHPSIPROP=THPSIRESIDENT &
     &       .AND.CPPAW_CUBLAS_ACC_HPSI_PROP_RESIDENCY_ENABLED() &
     &       .AND.(.NOT.TSTRESS).AND.(.NOT.TSTRAIGHTEN)
...
#ENDIF
#IF DEFINED(CPPVAR_CUBLAS_ACC)
      TKEEPPSI0ORTHO=CPPAW_CUBLAS_ACC_PSI0_ORTHO_RESIDENCY_ENABLED()
      DO IKPT=1,NKPTL
        DO ISPIN=1,NSPIN
          CALL WAVES_SELECTWV(IKPT,ISPIN)
          CALL PLANEWAVE$SELECT(GSET%ID)
...
#IF DEFINED(CPPVAR_CUBLAS_FP64_EMULATION)
      IF(CPPAW_CUBLAS_ACC_FP64_CHECK_ENABLED()) THEN
        WRITE(*,FMT='("CPPAW FP64 FORCE ACTIVE ",L1)') TFORCE
        IF(TFORCE) THEN
          DO IAT=1,NAT
            WRITE(*,FMT='("CPPAW FP64 FORCE ",I6,3(1X,ES24.16))') &
     &          IAT,FORCE(:,IAT)
          ENDDO
        END IF
        WRITE(*,FMT='("CPPAW FP64 STRESS ACTIVE ",L1)') TSTRESS
        IF(TSTRESS) THEN
          DO IAT=1,3
            WRITE(*,FMT='("CPPAW FP64 STRESS ",I2,3(1X,ES24.16))') &
     &          IAT,STRESS(IAT,:)
          ENDDO
        END IF
      END IF
...
#IF DEFINED(CPPVAR_CUBLAS_ACC)
     ...
     &      ,CPPAW_CUBLAS_ACC_SETUP_PSIM_RESIDENCY_ENABLED &
...
#IF DEFINED(CPPVAR_CUBLAS_ACC)
     ...
     &        CPPAW_CUBLAS_ACC_SETUP_PSI_RESIDENCY_ENABLED()
          TSETUPPSIMRESIDENT= &
     &        CPPAW_CUBLAS_ACC_SETUP_PSIM_RESIDENCY_ENABLED() &
     &        .AND.CPPAW_CUBLAS_ACC_PSIM_PHASE_RESIDENCY_ENABLED()
...
#ENDIF
     ...
     &                          ,'PSI0')
#IF DEFINED(CPPVAR_CUBLAS_ACC)
          IF(TSETUPPSIMRESIDENT.AND.(.NOT.THIS%PSIM_ACC_RESIDENT)) THEN
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
            CALL ACCELPROFILE$ADD('ACC_COPY_SETUP_PSIM_IN' &
     &          ,INT(NGL,KIND=8),INT(NDIM,KIND=8),INT(NBH,KIND=8),0_8 &
     &          ,0.D0,16.D0*REAL(NGL,KIND=8)*REAL(NDIM,KIND=8) &
     &          *REAL(NBH,KIND=8),0.D0)
#ENDIF
...
#ENDIF
      ...
      OFFDENBATCHSIZE=64
      CALL GET_ENVIRONMENT_VARIABLE('CPPAW_GPU_RESIDENCY_STACK',ENVVAL &
     &                             ,STATUS=ENVSTAT)
      IF(ENVSTAT.NE.0) THEN
        CALL GET_ENVIRONMENT_VARIABLE('CPPAW_CUBLAS_ACC_RESIDENCY_STACK' &
  ...
  101 CONTINUE
      ...
      IF(.NOT.TOFFDENBLAS) THEN
        TOFFDENCUBLAS=.FALSE.
        TOFFDENBATCH=.FALSE.
        TOFFDENDEVICEPACK=.FALSE.
        TOFFDENDEVICEACCUM=.FALSE.
      END IF
      IF(.NOT.TOFFDENCUBLAS) THEN
        TOFFDENBATCH=.FALSE.
        TOFFDENDEVICEPACK=.FALSE.
        TOFFDENDEVICEACCUM=.FALSE.
      END IF
      IF(.NOT.TOFFDENBATCH) THEN
        TOFFDENDEVICEPACK=.FALSE.
        TOFFDENDEVICEACCUM=.FALSE.
      END IF
      IF(.NOT.TOFFDENDEVICEPACK) THEN
        TOFFDENDEVICEACCUM=.FALSE.
      END IF
!
!     ==========================================================================
...
!$ACC&                  ,FORCE_LAMBDA2(1:NBH,1:NBH))
     ...
     &                   *REAL(NDIM*NBH,KIND=8)
              TKEEPPSI0FORHPSI=TKEEPPSI0FORHPSI &
...
!     ********************P.E. BLOECHL, TU-CLAUSTHAL (2005)*********************
     ...
     &                        ,WAVES_SELECTWV,HPSI_RTOG_ACC_ACTIVE
#IF DEFINED(CPPVAR_CUBLAS_ACC)
...
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
            CALL CPPAW_CUBLAS_ACC_PROFILE_PRESENT_C8_3D &
     &          ('ACC_PRESENT_HPSI_PSI0','ACC_COPY_HPSI_PSI0_IN' &
     &          ,NGL,NDIM,NBH,THIS%PSI0)
#ENDIF
...
!===============================================================================
#IF DEFINED(CPPVAR_CUBLAS_ACC)
            HPSI_RTOG_ACC_ACTIVE=THPSIRESIDENT
#ENDIF
		         CALL WAVES_VPSI(GSET,NGL,NDIM,NBH,NRL,THIS%PSI0,RHO(1,ISPIN) &
		     &                   ,THIS%HPSI)
#IF DEFINED(CPPVAR_CUBLAS_ACC)
            HPSI_RTOG_ACC_ACTIVE=.FALSE.
#ENDIF
...
!     **                                                                      **
!     *******************************************P.E. BLOECHL, (1991)***********
      USE WAVES_MODULE, ONLY : GSET_TYPE,TBUCKET,HPSI_RTOG_ACC_ACTIVE
...
#IF DEFINED(CPPVAR_CUBLAS_ACC)
      LOGICAL(4)             :: TACC_HPSI_FINISH
      LOGICAL(4)             :: TACC_HPSI_PRESENT
      LOGICAL(4)             :: TACC_HPSI_RTOG_RESIDENT
      LOGICAL(4)             :: TACC_HPSI_RTOG_DEVICE
      LOGICAL(4)             :: TACC_VPSI_INTERNAL
      LOGICAL(4)             :: TACC_PSIOFR_DEVICE
      INTEGER(4)             :: TACC_PSIOFR_DEVICE_COUNT
#ENDIF
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
      REAL(8)                :: ACCEL_T0
      REAL(8)                :: ACCEL_T1
...
#ENDIF
#IF DEFINED(CPPVAR_CUBLAS_ACC)
      TACC_HPSI_FINISH=CPPAW_CUBLAS_ACC_HPSI_RESIDENCY_ENABLED() &
     &                .AND.ACC_IS_PRESENT(PSI)
      TACC_HPSI_RTOG_RESIDENT=TACC_HPSI_FINISH &
     &     .AND.HPSI_RTOG_ACC_ACTIVE &
     &     .AND.CPPAW_CUBLAS_ACC_VPSI_HPSI_RTOG_RESIDENCY_ENABLED()
      TACC_HPSI_RTOG_DEVICE=TACC_HPSI_RTOG_RESIDENT
      TACC_VPSI_INTERNAL=TACC_HPSI_FINISH.AND.(NDIM.EQ.1) &
     &     .AND.CPPAW_CUBLAS_ACC_VPSI_INTERNAL_RESIDENCY_ENABLED()
      TACC_PSIOFR_DEVICE_COUNT=0
      IF(TACC_HPSI_RTOG_RESIDENT.AND.(.NOT.ACC_IS_PRESENT(HPSI))) THEN
!$ACC ENTER DATA CREATE(HPSI(1:NGL,1:NDIM,1:NBH))
      END IF
#ENDIF
!
!     ==========================================================================
...
#IF DEFINED(CPPVAR_CUBLAS_ACC)
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
      IF(TACC_VPSI_INTERNAL) THEN
        CALL ACCELPROFILE$ADD('ACC_COPY_VPSI_V_IN' &
     &      ,INT(NRL,KIND=8),1_8,0_8,0_8 &
     &      ,0.D0,8.D0*REAL(NRL,KIND=8),0.D0)
      END IF
#ENDIF
!$ACC DATA CREATE(PSIOFR(1:NRL,1:NDIM)) &
!$ACC&     COPYIN(V(1:NRL,1:1)) IF(TACC_VPSI_INTERNAL)
#ENDIF
      IF(NDIM.EQ.1) THEN
        DO IB=1,NBH
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
...
#ENDIF
#IF DEFINED(CPPVAR_CUBLAS_ACC)
          TACC_PSIOFR_DEVICE=.FALSE.
#ENDIF
          CALL PLANEWAVE$FFT('GTOR',NDIM,NGL,PSI(:,:,IB),NRL,PSIOFR)
#IF DEFINED(CPPVAR_CUBLAS_ACC)
          TACC_PSIOFR_DEVICE=TACC_VPSI_INTERNAL &
     &                       .AND.LAST_FFT_ACC_MAP_USED
          IF(TACC_PSIOFR_DEVICE) THEN
            TACC_PSIOFR_DEVICE_COUNT=TACC_PSIOFR_DEVICE_COUNT+1
          END IF
#ENDIF
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
          CALL ACCELPROFILE$NOW(ACCEL_T1)
...
#IF DEFINED(CPPVAR_CUBLAS_ACC)
          IF(TACC_PSIOFR_DEVICE) THEN
!$ACC PARALLEL LOOP PRESENT(PSIOFR,V)
            DO IR=1,NRL
...
#IF DEFINED(CPPVAR_CUBLAS_ACC)
          IF(TACC_PSIOFR_DEVICE.AND.(.NOT.LAST_FFT_ACC_MAP_USED)) THEN
            CALL ERROR$MSG('RTOG ACCMAP WAS NOT USED AFTER DEVICE VPSI')
            CALL ERROR$STOP('WAVES_VPSI')
          END IF
          IF(TACC_HPSI_RTOG_RESIDENT) THEN
            TACC_HPSI_RTOG_DEVICE=TACC_HPSI_RTOG_DEVICE &
     &                            .AND.LAST_FFT_ACC_MAP_USED
          END IF
#ENDIF
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
...
!$ACC END DATA
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
      IF(TACC_PSIOFR_DEVICE_COUNT.GT.0) THEN
        CALL ACCELPROFILE$ADD('ACC_PRESENT_VPSI_PSIOFR' &
     &      ,INT(NRL,KIND=8),1_8,INT(TACC_PSIOFR_DEVICE_COUNT,KIND=8) &
     &      ,0_8,0.D0,0.D0,0.D0)
      END IF
#ENDIF
#ENDIF
      DEALLOCATE(PSIOFR)
#IF DEFINED(CPPVAR_CUBLAS_ACC)
      IF(TACC_HPSI_FINISH) THEN
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
        CALL CPPAW_CUBLAS_ACC_PROFILE_PRESENT_C8_3D &
     &      ('ACC_PRESENT_VPSI_PSI','ACC_COPY_VPSI_PSI_IN' &
     &      ,NGL,NDIM,NBH,PSI)
        IF(ACC_IS_PRESENT(HPSI).AND.TACC_HPSI_RTOG_DEVICE) THEN
          CALL ACCELPROFILE$ADD('ACC_PRESENT_VPSI_HPSI' &
     &        ,INT(NGL,KIND=8),INT(NDIM,KIND=8),INT(NBH,KIND=8),0_8 &
     &        ,0.D0,0.D0,0.D0)
        ELSE IF(ACC_IS_PRESENT(HPSI)) THEN
          CALL ACCELPROFILE$ADD('ACC_UPDATE_VPSI_HPSI_IN' &
     &        ,INT(NGL,KIND=8),INT(NDIM,KIND=8),INT(NBH,KIND=8),0_8 &
     &        ,0.D0,16.D0*REAL(NGL,KIND=8)*REAL(NDIM,KIND=8) &
     &             *REAL(NBH,KIND=8),0.D0)
        ELSE
          CALL ACCELPROFILE$ADD('ACC_COPY_VPSI_HPSI_IN' &
     &        ,INT(NGL,KIND=8),INT(NDIM,KIND=8),INT(NBH,KIND=8),0_8 &
     &        ,0.D0,16.D0*REAL(NGL,KIND=8)*REAL(NDIM,KIND=8) &
     &             *REAL(NBH,KIND=8),0.D0)
        END IF
#ENDIF
        TACC_HPSI_PRESENT=ACC_IS_PRESENT(HPSI)
        IF(TACC_HPSI_PRESENT.AND.TACC_HPSI_RTOG_DEVICE) THEN
!         HPSI was produced directly by the device-side RTOG mapping.
        ELSE IF(TACC_HPSI_PRESENT) THEN
!$ACC UPDATE DEVICE(HPSI(1:NGL,1:NDIM,1:NBH))
        ELSE
!$ACC ENTER DATA COPYIN(HPSI(1:NGL,1:NDIM,1:NBH))
        END IF
      END IF
#ENDIF
!
...
#IF DEFINED(CPPVAR_CUBLAS_ACC)
      IF(TACC_HPSI_FINISH) THEN
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
        CALL ACCELPROFILE$ADD('ACC_COPY_VPSI_G2_IN' &
     &      ,INT(NGL,KIND=8),1_8,0_8,0_8 &
     &      ,0.D0,8.D0*REAL(NGL,KIND=8),0.D0)
#ENDIF
!$ACC DATA PRESENT(HPSI(1:NGL,1:NDIM,1:NBH) &
!$ACC&            ,PSI(1:NGL,1:NDIM,1:NBH)) COPYIN(G2(1:NGL))
!$ACC PARALLEL LOOP COLLAPSE(3)
        DO IB=1,NBH
          DO IDIM=1,NDIM
            DO IG=1,NGL
              HPSI(IG,IDIM,IB)=HPSI(IG,IDIM,IB) &
     &                         +0.5D0*G2(IG)*PSI(IG,IDIM,IB)
            ENDDO
          ENDDO
        ENDDO
!$ACC END PARALLEL LOOP
...
#IF DEFINED(CPPVAR_CUBLAS_ACC)
        IF(TACC_HPSI_FINISH) THEN
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
          CALL ACCELPROFILE$ADD('ACC_COPY_VPSI_BUCKET_IN' &
     &        ,INT(NGL,KIND=8),1_8,0_8,0_8 &
     &        ,0.D0,8.D0*REAL(NGL,KIND=8),0.D0)
#ENDIF
!$ACC DATA PRESENT(HPSI(1:NGL,1:NDIM,1:NBH) &
!$ACC&            ,PSI(1:NGL,1:NDIM,1:NBH)) &
!$ACC& COPYIN(GSET%BUCKET(1:NGL))
!$ACC PARALLEL LOOP COLLAPSE(3)
          DO IB=1,NBH
            DO IDIM=1,NDIM
              DO IG=1,NGL
...
#IF DEFINED(CPPVAR_CUBLAS_ACC)
     ...
     &        ,CPPAW_CUBLAS_ACC_PROJECTION_STACK_ENABLED &
     &        ,CPPAW_CUBLAS_ACC_PROJECTION_PRESENT &
     &        ,CPPAW_CUBLAS_ACC_PROJECTION_STACK_PRESENT &
     &        ,CPPAW_CUBLAS_ACC_PROFILE_PRESENT_C8_3D
#ENDIF
      IMPLICIT NONE
      TYPE(MAP_TYPE) ,INTENT(IN) :: MAP
      ...
      COMPLEX(8)     ,ALLOCATABLE:: PROPSISTACK(:,:) !(NPRO,NDIM*NB)
      COMPLEX(8)     ,ALLOCATABLE:: EIGR(:)    !(NGL)
      ...
      LOGICAL(4)                 :: TUSESTACKPROJ
      LOGICAL(4)                 :: TSTACKUNIFORM
      LOGICAL(4)                 :: TSUPER
      REAL(8)                    :: PROJFLOPS
      REAL(8)                    :: GWEIGHT
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
...
#IF DEFINED(CPPVAR_CUBLAS_ACC)
      ...
      IF(TUSERESIDENTPROJ) THEN
        ...
        CALL PLANEWAVE$GETR8('GWEIGHT',GWEIGHT)
        IF(TSUPER) CALL PLANEWAVE$GETI4('NGAMMA',NGAMMA)
      END IF
      DO IAT=1,MAP%NAT
        ISP=MAP%ISP(IAT)
        LMNX=MAP%LMNX(ISP)
...
!       == DEVICE-RESIDENT PROJECTION PATH =====================================
        ...
        TUSESTACKPROJ=CPPAW_CUBLAS_ACC_PROJECTION_STACK_ENABLED() &
     &                 .AND.TUSEGPUEXPANDPRO
        IF(TUSESTACKPROJ) THEN
          TSTACKUNIFORM=NPRO.EQ.LMNXX*MAP%NAT
          DO IAT=1,MAP%NAT
            ISP=MAP%ISP(IAT)
            TSTACKUNIFORM=TSTACKUNIFORM.AND.(MAP%LMNX(ISP).EQ.LMNXX)
          ENDDO
          TUSESTACKPROJ=TSTACKUNIFORM
        END IF
#IF DEFINED(CPPVAR_ACCEL_PROFILE)
...
!$ACC& CREATE(PROPSIACC(1:LMNXX,1:NDIM*NB))
          IF(TUSESTACKPROJ) THEN
            ALLOCATE(PROPSISTACK(NPRO,NDIM*NB))
!$ACC ENTER DATA CREATE(PROPSISTACK(1:NPRO,1:NDIM*NB))
            CALL CPPAW_CUBLAS_ACC_PROJECTION_STACK_PRESENT(NGL,NDIM,NB &
     &          ,NPRO,LMNXX,NAT,ACC_PRO_CACHE_PRO,PSI,GWEIGHT &
     &          ,PROPSISTACK,TSUPER,NGAMMA,PROPSI)
!$ACC EXIT DATA DELETE(PROPSISTACK(1:NPRO,1:NDIM*NB))
            DEALLOCATE(PROPSISTACK)
          ELSE
            DO IAT=1,MAP%NAT
              ISP=MAP%ISP(IAT)
              LNX=MAP%LNX(ISP)
     ...
     &            ,GWEIGHT,PROPSIACC,TSUPER,NGAMMA,PROPSI)
              IPRO=IPRO+LMNX
            ENDDO
          END IF
!$ACC END DATA
...
!$ACC& CREATE(PROPSIACC(1:LMNXX,1:NDIM*NB))
     ...
     &                          ,GSET%YLM,EIGR,PRO)
            CALL CPPAW_CUBLAS_ACC_PROJECTION_PRESENT(NGL,NDIM,NB,LMNX &
     &              ,LMNXX,IPRO,NPRO,PRO,PSI,GWEIGHT,PROPSIACC &
...
#ENDIF
#ENDIF
!$ACC DATA PRESENT_OR_COPYIN(PSI(1:NGL,1:NDIM,1:NB)) IF(TUSECUBLASPROJ)
        DO IAT=1,MAP%NAT
          ISP=MAP%ISP(IAT)
          LNX=MAP%LNX(ISP)
          LMNX=MAP%LMNX(ISP)
...
!       == END OF BLOCKED CODE SEGMENT =========================================
      ...
      IF(ALLOCATED(PROPSISTACK)) DEALLOCATE(PROPSISTACK)
...
#IF DEFINED(CPPVAR_CUBLAS_ACC)
     ...
     &        ,CPPAW_CUBLAS_ACC_ADDPRO_STACK_ENABLED &
     &        ,CPPAW_CUBLAS_ACC_ZGEMM_NN_PRESENT &
     &        ,CPPAW_CUBLAS_ACC_ADDPRO_STACK_PRESENT &
     &        ,CPPAW_CUBLAS_ACC_PROFILE_PRESENT_C8_3D &
...
#IF DEFINED(CPPVAR_CUBLAS_ACC)
     ...
     &       CPPAW_CUBLAS_ACC_ADDPRO_CACHE_CONTEXT_ENABLED(PROFILE_ID)
        TUSESTACKADDPRO= &
     &       CPPAW_CUBLAS_ACC_ADDPRO_STACK_ENABLED(PROFILE_ID)
        IF(TUSESTACKADDPRO) THEN
          TSTACKUNIFORM=NPRO.EQ.LMNXX*MAP%NAT
          DO IAT=1,MAP%NAT
            ISP=MAP%ISP(IAT)
            TSTACKUNIFORM=TSTACKUNIFORM.AND.(MAP%LMNX(ISP).EQ.LMNXX)
          ENDDO
          TUSESTACKADDPRO=TSTACKUNIFORM
        END IF
#ENDIF
...
!$ACC& CREATE(PROPSI1(1:LMNXX*NDIM*NB))
          IF(TUSESTACKADDPRO) THEN
            ALLOCATE(PROPSISTACK(NPRO,NDIM*NB))
!$ACC ENTER DATA CREATE(PROPSISTACK(1:NPRO,1:NDIM*NB))
            CALL CPPAW_CUBLAS_ACC_ADDPRO_STACK_PRESENT(NGL,NDIM,NB,NPRO &
     &          ,LMNXX,NAT,ACC_PRO_CACHE_PRO,PROPSI,PROPSISTACK,PSI)
!$ACC EXIT DATA DELETE(PROPSISTACK(1:NPRO,1:NDIM*NB))
            DEALLOCATE(PROPSISTACK)
          ELSE
            DO IAT=1,MAP%NAT
